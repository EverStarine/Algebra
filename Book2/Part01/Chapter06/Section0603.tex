...
设 \({}_SP_R\) 为双模，\(M\) 为左 \(R\) 模，\(N\) 为左 \(S\) 模。采用\eqref{b2:eq:hom-left-action}的左 \(R\) 作用，则有交换群同构
...
\(\operatorname{Hom}_S\Bigl(F(M),N\Bigr)\cong\operatorname{Hom}_R\Bigl(M,G(N)\Bigr)\)。一般地，一对方向相反的函子若带有这种在两变量上自然的 Hom 集双射，就称构成\term[bansui]{伴随}[adjunction]；\(F\) 称为 \(G\) 的\term[zuobansui]{左伴随}[left adjoint]，\(G\) 称为 \(F\) 的\term[youbansui]{右伴随}[right adjoint]，记作 \(F\dashv G\)。
...
限制标量 \(\operatorname{Res}_{\varphi}\) 的左伴随为 \(S\otimes_R-\)，右伴随为
\[
