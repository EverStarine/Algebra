...
设 \(M\) 为右 \(R\) 模，\(K\subseteq N\) 为左 \(R\) 模的子模。令 \(H\subseteq M\otimes_RN\) 为所有 \(m\otimes k\)、\(k\in K\) 生成的子群，则自然映射诱导同构
\[
...
设 \(I\) 为 \(R\) 的双边理想，\(N\) 为左 \(R\) 模，\(M\) 为右 \(R\) 模。存在自然同构
\[
...
上述集合配以下加法与 \(\Sigma^{-1}R\) 标量作用，称为 \(N\) 在 \(\Sigma\) 处的\term[mojubu-hua]{局部化}[localization of a module]：
\[
\frac ns+\frac{n'}t=\frac{tn+sn'}{st},\qquad
\frac av\frac ns=\frac{an}{vs}.
\]
其中 \(\Sigma^{-1}R\) 是第一卷定理13.2.2所构造的交换环局部化；本节的 \(R\) 交换且 \(\Sigma\) 含 \(1\)、乘法封闭，正好满足该构造的条件。
\end{definition}
