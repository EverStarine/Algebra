...
一个 \(k\) 上的\term[xiangliang-kongjian]{向量空间}[vector space]是交换加法群 \((V,+)\)，配有标量乘法 \(k\times V\to V\)，使任意 \(a,b\in k\) 和 \(u,v\in V\) 满足
\[
a(u+v)=au+av,\qquad (a+b)v=av+bv,\qquad
(ab)v=a(bv),\qquad 1v=v.
\]
若子集 \(U\subseteq V\) 包含 \(0\)，且对加法与标量乘法封闭，就称为 \(V\) 的\term[zi-kongjian]{子空间}[subspace]。
\end{definition}
...
设 \(S\subseteq V\)。\term[xianxing-shengcheng-kongjian]{线性生成空间}[linear span] \(\operatorname{span}_kS\) 是 \(S\) 中向量的全部有限线性组合；约定空和为 \(0\)。线性生成空间也称线性闭包。\termidx[xianxing-bibao]{线性闭包}若它等于 \(V\)，称 \(S\) 生成 \(V\)。向量族 \((v_i)_{i\in I}\) 称为\term[xianxing-wuguan]{线性无关}[linearly independent]的，是指每个有限关系
\[
\sum_{i\in F}a_iv_i=0\quad(F\subseteq I\text{ 有限})
\]
都迫使所有 \(a_i=0\)。既线性无关又生成 \(V\) 的向量族称为 \(V\) 的一组\term[ji]{基}[basis]。存在有限生成集的空间称为\term[youxian-wei]{有限维}[finite-dimensional]空间；否则称为无限维空间。
\end{definition}
...
若 \(v_1,\ldots,v_n\) 生成 \(V\)，而 \(u_1,\ldots,u_m\) 线性无关，则 \(m\le n\)，且可用这些 \(u_i\) 替换生成组中的 \(m\) 个向量，得到仍生成 \(V\) 的 \(n\) 个向量。
\end{lemma}
\begin{proof}
...
向量空间中的每个线性无关子集都可扩充为一组基。若空间由有限集 \(S\) 生成，可以只从 \(S\) 中选取所需的新向量。
\end{theorem}
\begin{proof}
...
同一向量空间的任意两组基等势。其共同基数称为 \(V\) 的\term[weishu]{维数}[dimension]，记作 \(\dim_kV\)。特别地，\(\dim_k0=0\)。
\end{theorem}
\begin{proof}
...
把 \(U\) 的一组基 \(B_U\) 扩充为 \(V\) 的基 \(B_U\sqcup C\)，则 \((c+U)_{c\in C}\) 是 \(V/U\) 的基。令 \(W=\operatorname{span}_kC\)，每个 \(v\in V\) 唯一写为 \(u+w\)，其中 \(u\in U,w\in W\)。记这种分解为 \(V=U\oplus W\)，称 \(W\) 为 \(U\) 的一个\term[bu-kongjian]{补空间}[complement]。
\end{proposition}
\begin{proof}
...
设 \(V,W\) 为 \(k\) 向量空间。映射 \(f:V\to W\) 若满足 \(f(au+bv)=af(u)+bf(v)\)，就称为\term[xianxing-yingshe]{线性映射}[linear map]。全部这样的映射构成向量空间 \(\operatorname{Hom}_k(V,W)\)，加法和标量乘法逐点定义。双射的线性映射称为\term[xianxing-tonggou]{线性同构}[linear isomorphism]，记 \(V\cong W\)。
\end{definition}
...
线性映射 \(f:V\to W\) 的\term[he]{核}[kernel]、\term[xiang]{像}[image]及\term[zhi]{秩}[rank]分别为
\[
...
对 \(U\le V\) 及线性映射 \(f:V\to W\)，存在满足 \(\bar f q=f\) 的线性映射 \(\bar f:V/U\to W\)，当且仅当 \(U\subseteq\ker f\)。存在时它唯一，且 \(\bar f(v+U)=f(v)\)。
\end{proposition}
\begin{proof}
...
映射 \(v+\ker f\mapsto f(v)\) 给出同构 \(V/\ker f\cong\operatorname{im}f\)。
\end{proposition}
\begin{proof}
