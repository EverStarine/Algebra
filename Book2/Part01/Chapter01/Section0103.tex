...
设 \(V\) 是域 \(k\) 上的向量空间。若线性映射的定义域和值域都是 \(V\)，就称它为 \(V\) 上的\term[xianxing-suanzi]{线性算子}[linear operator]。记
...
设 \(V\) 是域 \(k\) 上的 \(n\) 维向量空间，\(N,T\in\operatorname{End}_k(V)\)，\(\lambda\in k\)，则核链 \(\ker N^r\) 至迟在 \(r=n\) 时稳定。因此 \(G_\lambda(T)=\ker(T-\lambda I_V)^n\)。
\end{proposition}
\begin{proof}
...
设 \(V\) 是域 \(k\) 上的向量空间，\(T\in\operatorname{End}_k(V)\)，\(p\in k[t]\)。若 \(p(T)=0\)，就称 \(p\) \term[linghua]{零化}[annihilate]算子 \(T\)。如果存在非零零化多项式，就把其中次数最小的首一多项式称为 \(T\) 的\term[zuixiao-duoxiangshi]{最小多项式}[minimal polynomial]，记作 \(m_T(t)\)。
\end{definition}
\symidx{\(m_T\)：线性算子的最小多项式}

\begin{proposition}\label{b2:prop:minimal-polynomial-divisibility}
设 \(V\ne0\) 是域 \(k\) 上的有限维向量空间，\(T\in\operatorname{End}_k(V)\)，则 \(T\) 有唯一的最小多项式 \(m_T\)，且对任意 \(p\in k[t]\) 有
\[
...
\]
若 \(k[T]v=V\)，就称 \(v\) 为 \(T\) 的\term[xunhuan-xiangliang]{循环向量}[cyclic vector]。
\end{definition}

\begin{proposition}\label{b2:prop:cyclic-basis}
设 \(V\ne0\) 是域 \(k\) 上的有限维向量空间，\(T\in\operatorname{End}_k(V)\)，\(0\ne v\in V\)。令 \(m_{T,v}\) 为零化 \(v\) 的首一最小多项式，\(d=\deg m_{T,v}\)。此时 \(v,Tv,\ldots,T^{d-1}v\) 是 \(k[T]v\) 的基。若 \(v\) 是循环向量，则 \(m_T=m_{T,v}\)，从而 \(\deg m_T=\dim_k V\)。
\end{proposition}
\begin{proof}
