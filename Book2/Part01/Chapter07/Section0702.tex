...
设 \(R\) 是环，\(E\) 是左 \(R\) 模。\(E\) 内射，当且仅当对每个左理想 \(I\subseteq R\)，每个左 \(R\) 模同态 \(f:I\to E\) 都能延拓为 \({}_RR\to E\)。后一条件也可写成：对每个这样的 \(I,f\)，存在 \(e\in E\)，使所有 \(a\in I\) 都满足 \(f(a)=ae\)。
\end{theorem}
...
设 \(D\) 是交换群，并按整数倍看成左 \(\mathbb Z\) 模，则 \(D\) 内射，当且仅当它可除。
\end{theorem}
\begin{proof}
...
设 \(R\) 是环，\(D\) 是作为 \(\mathbb Z\) 模内射的交换群，则
\[
