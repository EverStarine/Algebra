...
左 \(R\) 模 \(M\) 称为\term[pingtanmo]{平坦模}[flat module]，如果对每个右 \(R\) 模的单同态 \(u:A\to B\)，诱导的交换群同态
\[
u\otimes\operatorname{id}_M:A\otimes_R M\longrightarrow B\otimes_R M
\]
仍为单射。右模平坦的定义相应使用它与左模的张量积，不能在非交换环上省略这一侧别。
\end{definition}

\begin{proposition}\label{b2:prop:flat-exact}
左模 \(M\) 平坦，当且仅当 \(-\otimes_R M\) 将每个右模短正合列变成交换群的短正合列。
\end{proposition}
\begin{proof}
...
自由模是平坦模；平坦模的任意直和及直和因子仍平坦。因此每个投射模都平坦。
\end{theorem}
\begin{proof}
...
设左模 \(M\) 是一族子模 \((M_\lambda)\) 的并，且任意两项都包含在某个共同的较大项中。若每个 \(M_\lambda\) 平坦，则 \(M\) 平坦。
\end{lemma}
\begin{proof}
...
设 \(R\) 为交换环，\(M\) 为 \(R\) 模。则 \(M\) 平坦，当且仅当对每个有限生成理想 \(I\subseteq R\)，映射
\[
