...
设 \(F:\mathcal C\to\mathcal D\) 是范畴间的函子。\(F\) 把 \(\mathcal C\) 中的同构送到 \(\mathcal D\) 中的同构。若 \(F\) 全忠实，则对 \(\mathcal C\) 中的态射 \(f\)，\(F(f)\) 可逆蕴含 \(f\) 可逆；对对象 \(X,Y\)，\(F(X)\cong F(Y)\) 蕴含 \(X\cong Y\)。
\end{proposition}
\begin{proof}
...
设 \(\mathcal C,\mathcal D\) 是范畴，函子 \(F:\mathcal C\rightarrow\mathcal D\) 是范畴等价，当且仅当它全、忠实且本质满。反向构造使用选择公理，并在预先固定的足够大集合论宇宙中选取对象代表。
\end{theorem}
\begin{proof}
...
设 \(F:\mathcal C\to\mathcal D\) 是范畴等价。\(F\) 把 \(\mathcal C\) 的初始对象、终止对象分别送到 \(\mathcal D\) 的初始对象、终止对象，并把一个已存在的积或余积及其指定映射送到相应的积或余积。
\end{proposition}
\begin{proof}
