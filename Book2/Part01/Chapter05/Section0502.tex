...
设 \(k\) 是域，\(\mathbf{Vect}_k\) 是 \(k\) 向量空间及其线性映射的范畴，\(D^2(V)=V^{**}\)、\(D^2(f)=f^{**}\)。评价映射 \(j_V(v)(\lambda)=\lambda(v)\) 组成自然变换 \(j:\operatorname{Id}_{\mathbf{Vect}_k}\Rightarrow D^2\)。限制在有限维向量空间范畴上，它是自然同构。
\end{proposition}
\begin{proof}
