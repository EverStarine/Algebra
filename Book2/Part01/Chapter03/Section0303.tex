...
设 \(R\) 是环，\(A,B,C\) 是左 \(R\) 模，\(u:A\to B\)、\(v:B\to C\) 是模同态。若
\[
...
设 \(0\to A\xrightarrow{i}B\xrightarrow{p}C\to0\) 是左 \(R\) 模的短正合列，则 \(i\) 给出 \(A\cong\ker p\)，\(p\) 给出同构
\[
...
设 \(A,B,C\) 是左 \(R\) 模，\(p:B\to C\)、\(i:A\to B\) 是同态。若同态 \(s:C\to B\) 满足 \(ps=\operatorname{id}_C\)，就称 \(s\) 为 \(p\) 的一个\term[jie-mian]{截面}[section]。若同态 \(r:B\to A\) 满足 \(ri=\operatorname{id}_A\)，就称 \(r\) 为 \(i\) 的一个\term[suohui]{缩回}[retraction]。这两种等式分别说 \(s\) 是 \(p\) 的右逆、\(r\) 是 \(i\) 的左逆。
\end{definition}

\begin{theorem}[短正合列的分裂]\label{b2:thm:split-exact-sequence}
设 \(0\to A\xrightarrow{i}B\xrightarrow{p}C\to0\) 是左 \(R\) 模的短正合列。记 \(\iota_A:A\to A\oplus C\) 为第一分量嵌入，\(\pi_C:A\oplus C\to C\) 为第二分量投影。以下条件等价：
\begin{equivalences}
...
设 \(0\to A\to B\xrightarrow{p}F\to0\) 是左 \(R\) 模的短正合列。若 \(F\) 是自由 \(R\) 模，则该列分裂。
\end{corollary}
...
设 \(0\to A\xrightarrow{i}B\xrightarrow{p}C\to0\) 与 \(0\to A'\xrightarrow{i'}B'\xrightarrow{p'}C'\to0\) 是左 \(R\) 模的短正合列，且同态 \(\alpha:A\to A'\)、\(\beta:B\to B'\)、\(\gamma:C\to C'\) 满足 \(\beta i=i'\alpha\)、\(p'\beta=\gamma p\)。若 \(\alpha,\gamma\) 都单射，则 \(\beta\) 单射；若 \(\alpha,\gamma\) 都满射，则 \(\beta\) 满射。特别地，若 \(\alpha,\gamma\) 都是同构，则 \(\beta\) 也是同构。
\end{lemma}
