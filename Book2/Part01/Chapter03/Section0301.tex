...
设 \(M\) 是左 \(R\) 模，\((e_i)_{i\in I}\) 是其中一族元素。若每个 \(m\in M\) 都能唯一写成
\[
...
设 \(F\) 有基 \((e_i)_{i\in I}\)。对任意左 \(R\) 模 \(N\) 及任意一族 \((n_i)_{i\in I}\subseteq N\)，存在唯一模同态 \(f:F\to N\) 满足 \(f(e_i)=n_i\)。其公式为
\[
...
设 \(F\) 自由，\(q:M\to N\) 满射。每个模同态 \(f:F\to N\) 都可以写成 \(qg\)，其中 \(g:F\to M\) 为模同态。
\end{proposition}
\begin{proof}
...
设 \(R\) 是非零交换环。如果 \(R^{(I)}\cong R^{(J)}\)，则 \(|I|=|J|\)。这个公共基数称为自由模的\term[ziyoumozhichi]{秩}[rank]，记为 \(\operatorname{rank}_R F\)。
\end{theorem}
\begin{proof}
