...
设 \(I\) 是集合，\((M_i)_{i\in I}\) 是一族左 \(R\) 模。它们的\term[mo-zhiji]{直积}[direct product of modules]为
\[
...
L_i\cap\sum_{j\ne i}L_j=0.
\]
成立时，每个 \(m\in M\) 都有唯一的有限表达 \(m=\sum_i\ell_i\)、\(\ell_i\in L_i\)，称 \(M\) 为这些子模的\term[nei-zhihe]{内直和}[internal direct sum]，也记 \(M=\bigoplus_iL_i\)。
\end{proposition}
\begin{proof}
...
设 \(N\) 为左 \(R\) 模，并对每个 \(i\in I\) 给定同态 \(f_i:N\to M_i\)。则存在唯一同态
\[
...
设 \(N\) 为左 \(R\) 模，并对每个 \(i\in I\) 给定同态 \(g_i:M_i\to N\)。则存在唯一同态
\[
