...
设 \(R\) 是环。一个\term[zuomo]{左 \(R\) 模}[left R-module]是交换群 \((M,+)\)，连同映射
\[
R\times M\longrightarrow M,\qquad (r,m)\longmapsto rm,
\]
使任意 \(r,s\in R\) 和 \(m,n\in M\) 满足
\[
...
\]
最后一条称为幺性条件。这里的加法交换群已经包括加法结合律、零元、负元与交换律。
\end{definition}
...
一个\term[youmo]{右 \(R\) 模}[right R-module]是交换群 \((M,+)\)，连同右侧标量乘法 \((m,r)\mapsto mr\)，满足
\[
...
\ProseParagraphBreak
也可以不换底集合而改变标量环。环 \(R\) 的\term[fanhuan]{反环}[opposite ring]记为 \(R^{\mathrm{op}}\)，加法与 \(R\) 相同，乘法定义为 \(r* s=sr\)。一个右 \(R\) 模成为左 \(R^{\mathrm{op}}\) 模的方法是 \(r\cdot m=mr\)：确有
\[
...
设 \(V\) 为 \(k\) 向量空间，\(T:V\to V\) 为线性算子。对 \(f(t)=\sum_{j=0}^d a_jt^j\)，定义
\[
...
给交换群 \(A\) 一个幺性左 \(R\) 模结构，等价于给一个保幺环同态
\[
