...
设 \(f:M\to N\) 是左 \(R\) 模之间的同态。定义其\term[he]{核}[kernel]、\term[xiang]{像}[image]及\term[yuhe]{余核}[cokernel]为
\[
...
设 \(f:M\to N\) 是左 \(R\) 模之间的同态。它诱导 \(R\) 模同构
\[
...
设 \(M\) 是左 \(R\) 模，\(L\le M\)，商映射为 \(q:M\to M/L\)。\(M\) 中包含 \(L\) 的子模 \(U\) 与 \(M/L\) 的子模 \(W\) 通过
\[
...
设 \(M\) 是左 \(R\) 模。若 \(U,V\le M\)，则 \(U+V=\{\,u+v\mid u\in U,\ v\in V\,\}\) 与 \(U\cap V\) 均为子模，并有 \(R\) 模同构
\[
