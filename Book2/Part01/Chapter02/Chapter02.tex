...
\ProseParagraphBreak
生成元记录怎样得到全部元素，关系记录表达为何不唯一；元素的零化子与全模零化子分别记录局部的零作用和对整个模无效的标量。整环上的挠元形成子模，模掉它们得到无挠商，但挠模未必具有一个共同的非零零化标量。直和与直积则把有限性体现为支集条件：任意一族坐标映射可以合成映入直积的映射，任意一族来自各因子的映射可以合成映出直和的映射。下一章将以这些具体构造为基础，系统讨论自由模、生成与关系以及正合列。
\end{chaptersummary}
