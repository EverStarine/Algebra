...
设 \(V\ne0\) 是域 \(k\) 上的有限维向量空间，\(T\in\operatorname{End}_k(V)\)。假设 \(T\) 的最小多项式 \(m_T\) 在 \(k\) 上分裂，写成
\[
...
e_d=\overline1.
\]
算子在这组基下的矩阵为 Jordan 块 \(J_d(\lambda)\)，其对角元均为 \(\lambda\)，紧邻对角线的上方元素均为 \(1\)，其余元素为 \(0\)。等价地，若 \(V\) 是 \(k\) 向量空间，\(T\in\operatorname{End}_k(V)\)，且循环子空间由 \(v\in V\) 生成并满足 \(m_{T,v}=(t-\lambda)^d\)，则其相应基为
\[
...
设 \(V\) 是域 \(k\) 上的有限维向量空间，\(T\in\operatorname{End}_k(V)\) 的特征多项式在 \(k\) 上分裂，\(\lambda\in k\) 是其特征值。记 \(c_\lambda(0)=0\)，\(c_\lambda(r)=\dim_k\ker(T-\lambda I_V)^r\)（\(r\ge1\)）。此时属于 \(\lambda\)、大小恰为 \(r\) 的 Jordan 块数是
\[
...
设 \(V\) 是域 \(k\) 上的有限维向量空间，\(T\in\operatorname{End}_k(V)\)，则 \(T\) 在 \(k\) 上可对角化，当且仅当其最小多项式 \(m_T\) 是 \(k[t]\) 中互不相同的一次因子的乘积；\(V=0\) 时将空乘积视为 \(1\)。
\end{corollary}
\begin{proof}
...
设 \(k\subseteq L\) 为域扩张，\(A\in M_n(k)\)，其在 \(k[t]\) 中的首一不变因子为 \(f_1,\ldots,f_s\)。把 \(A\) 看作 \(L\) 上的矩阵后，首一不变因子仍为这些多项式，只是把系数看成 \(L\) 中的元素。因此最小多项式和特征多项式也保持不变。
\end{proposition}
\begin{proof}
...
设 \(k\subseteq L\) 为域扩张，\(A\in M_n(k)\)，\(q^e\) 是 \(A\) 在 \(k[t]\) 中的一个初等因子，其中 \(q\in k[t]\) 首一不可约，\(e\ge1\)。若在 \(L[t]\) 中有分解
\[
