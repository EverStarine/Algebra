...
设 \(V,W\) 是域 \(k\) 上的向量空间，\(T:V\to V\)、\(S:W\to W\) 是线性算子；\(V_T,W_S\) 表示分别由 \(t\cdot v=Tv\)、\(t\cdot w=Sw\) 得到的 \(k[t]\) 模。映射 \(u:V_T\to W_S\) 是 \(k[t]\) 模同态，当且仅当 \(u\) 是 \(k\) 线性映射并满足 \(uT=Su\)。因此两个同维算子的矩阵相似，当且仅当对应 \(k[t]\) 模同构。
\end{proposition}
\begin{proof}
