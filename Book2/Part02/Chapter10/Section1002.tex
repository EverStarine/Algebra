...
设 \(k\) 为域，\(V\) 为 \(k\) 向量空间。如果映射 \(q:V\to k\) 满足 \(q(av)=a^2q(v)\) 对所有 \(a\in k,v\in V\) 成立，且映射
\begin{equation}\label{b2:eq:quadratic-polar}
\beta_q(u,v)=q(u+v)-q(u)-q(v)
\end{equation}
是 \(k\) 双线性的，就称 \(q\) 为 \(V\) 上的\term[erci-xing]{二次型}[quadratic form]，称 \(\beta_q\) 为它的\term[jihua-xing]{极化型}[polar form]。
\end{definition}
...
设 \(k\) 为特征二的域，\(V\) 为以 \(e_1,\ldots,e_n\) 为基的有限维 \(k\) 向量空间，\(q:V\to k\) 为二次型，\(\beta_q\) 为其极化型，则
\[
