...
设 \(U,V\) 为域 \(k\) 上的向量空间。一个\term[shuangxianxing-peidui]{双线性配对}[bilinear pairing]是映射 \(b:U\times V\to k\)，它在固定任意一个变量后，对另一个变量都是 \(k\) 线性的。它的左根空间与右根空间分别为
\[
...
\]
它们的核就是上述两个根空间。若 \(L_b\) 是同构，配对称为\term[wanmei-peidui]{完美配对}[perfect pairing]。

\begin{proposition}\label{b2:prop:finite-perfect-pairing}
有限维配对非退化，当且仅当 \(L_b\) 与 \(R_b\) 都是同构；这也等价于其中任一个是同构。此时 \(\dim U=\dim V\)。
\end{proposition}
\begin{proof}
...
双线性型 \(b\) 称为\term[duichen-xing]{对称型}[symmetric bilinear form]，如果 \(b(u,v)=b(v,u)\) 对所有 \(u,v\) 成立；称为\term[jiaoti-xing]{交替型}[alternating bilinear form]，如果 \(b(v,v)=0\) 对所有 \(v\) 成立。满足 \(b(u,v)=-b(v,u)\) 的型称为\term[fanduichen-xing]{反对称型}[skew-symmetric bilinear form]。
\end{definition}
...
设 \(b\) 是有限维 \(V\) 上的非退化对称型或交替型，\(W\le V\)。则
\[
...
\]
其中符号 \(\perp\) 表示两个分量互相正交的直和；\(W^\perp\) 上的限制型也非退化。最后这项分解只要求 \(W\) 上非退化，即使 \(V\) 上的型退化，分解本身仍成立。
\end{theorem}
\begin{proof}
...
若新基的列在旧基中的坐标组成 \(P\in\operatorname{GL}_n(k)\)，则型的新矩阵为
\[
B'=P^{\mathsf T}BP.
\]
