...
对任意交换 \(R\) 代数 \(A\) 及 \(R\) 线性映射 \(f:M\rightarrow A\)，存在唯一 \(R\) 代数同态
\[
...
商映射 \(q_d:M^{\otimes d}\rightarrow\operatorname{Sym}_R^d(M)\) 的核由所有 \(t-P_\sigma t\)、\(t\in M^{\otimes d}\)、\(\sigma\in S_d\) 生成。因而从 \(\operatorname{Sym}_R^d(M)\) 到 \(N\) 的线性映射，与对变量的任意排列保持不变的多线性映射 \(M^d\rightarrow N\) 一一对应。
\end{proposition}
\begin{proof}
...
若 \(M\) 有有限基 \(e_1,\ldots,e_r\)，则存在唯一分次 \(R\) 代数同构
\[
...
令 \(S_d\) 按\cref{b2:prop:multiple-tensor-functoriality}的公式作用在 \(M^{\otimes d}\) 上。其不变子模为
\[
(M^{\otimes d})^{S_d}
...
若 \(d!\) 在 \(R\) 中可逆，则\term[duichen-hua]{对称化}[symmetrization]算子
\[
