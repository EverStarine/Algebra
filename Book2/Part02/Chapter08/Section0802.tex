...
设 \(R\) 为交换环。一个含单位元的\term[jiehe-daishu]{结合代数}[associative algebra] \(A\) 是含单位元的环，连同保单位元环同态 \(R\rightarrow Z(A)\)。它因此是 \(R\) 模，乘法对两个变量分别 \(R\) 线性；结构同态不要求单射。\(R\) 代数同态指与结构同态相容的保单位元环同态。
...
对 \(R\) 模 \(M\)，令 \(M^{\otimes0}=R\)、\(M^{\otimes1}=M\)，并记
\[
...
对任意含单位元的结合 \(R\) 代数 \(A\) 及 \(R\) 线性映射 \(f:M\rightarrow A\)，存在唯一 \(R\) 代数同态
\[
...
若 \(M\) 自由，基为 \((x_i)_{i\in I}\)，则 \(T_R(M)\) 以所有有限字
\[
1,\quad x_{i_1},\quad x_{i_1}x_{i_2},\quad\ldots
\]
为 \(R\) 模的一组基，乘法为连接字。给定任意 \(R\) 代数 \(A\) 中的一族元素 \((a_i)_{i\in I}\)，存在唯一代数同态将每个 \(x_i\) 送到 \(a_i\)。这个代数称为这些生成元上的\term[ziyou-jiehe-daishu]{自由结合代数}[free associative algebra]，记为 \(R\langle x_i\mid i\in I\rangle\)。
\end{proposition}
\begin{proof}
...
从上述商代数到 \(R\) 代数 \(B\) 的同态，与满足全部关系 \(r_\alpha(b_i)=0\) 的元素族 \((b_i)\subseteq B\) 一一对应。
\end{proposition}
\begin{proof}
...
在分次 \(R\) 代数 \(A=\bigoplus_{d\ge0}A_d\) 中，由齐次元素族生成的双边理想 \(J\) 满足
\[
