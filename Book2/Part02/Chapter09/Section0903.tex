...
若 \(M\) 为有 \(n\) 个基向量的有限自由 \(R\) 模，称具有一个基向量的自由模
\[
\det M\coloneqq\bigwedge^nM
\]
为 \(M\) 的\term[hanglieshi-xian]{行列式线}[determinant line]。\(M=0\) 且取空基时，约定 \(\det M=R\)。
\end{definition}
...
设 \(A\in M_{m\times n}(R)\)、\(B\in M_{n\times r}(R)\)。对大小均为 \(p\) 的递增指标集 \(I\subseteq\{1,\ldots,m\}\)、\(J\subseteq\{1,\ldots,r\}\)，有
