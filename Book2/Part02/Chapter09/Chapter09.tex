...
反过来，若 \(a_0\) 可逆，令 \(z=a_0^{-1}\eta\)。则 \(z^{n+1}=0\)，有限几何级数给出
\[
...
设 \(a_1\) 为交换环 \(R\) 的单位，在 \(K_\bullet(a_1,\ldots,a_n;R)\) 上定义 \(h(\xi)=a_1^{-1}e_1\wedge\xi\)。证明 \(dh+hd=\operatorname{id}\)，并据此证明这条复形正合。说明这只是一个附加了单位条件的特例。
\end{exercise}
\begin{solution}
收缩的乘法公式与 \(d(e_1)=a_1\) 给出
\[
d\Bigl(a_1^{-1}e_1\wedge\xi\Bigr)
=\xi-a_1^{-1}e_1\wedge d\xi,
\]
故 \(dh+hd=\operatorname{id}\)。对任意 \(d\xi=0\)，得到 \(\xi=d(h\xi)\)，因此核包含在前一箭头的像中；反向包含由 \(d^2=0\)，所以正合。零次处也如此，此时 \((a_1,\ldots,a_n)=R\)，末端余核为零。一般序列未必有单位，无法据此定义 \(a_1^{-1}\)，所以此构造不能代替一般正合性理论。
\end{solution}
...
\[
d_1=\begin{pmatrix}a&b&c\end{pmatrix},\quad
d_2=\begin{pmatrix}-b&-c&0\\a&0&-c\\0&a&b\end{pmatrix},\quad
d_3=\begin{pmatrix}c\\-b\\a\end{pmatrix}.
\]
\(d_1d_2\) 的三项为 \(-ab+ba,-ac+ca,-bc+cb\)，均为零；\(d_2d_3\) 的三项为 \(-bc+cb,ac-ca,-ab+ba\)，也均为零。两次检查明确使用了标量环的交换性。它们只说明序列为复形，没有检验各核是否都由前一项生成。
\end{solution}
