...
设 \(R\) 为交换环，\(M\) 为 \(R\) 模，\(\lambda:M\to R\) 为 \(R\) 线性映射，并约定 \(\bigwedge^pM=0\) 对 \(p<0\) 成立。存在唯一次数为 \(-1\) 的 \(R\) 线性算子
\[
...
令 \(M=R^n\)，选基 \(e_1,\ldots,e_n\)，并指定 \(\lambda(e_i)=a_i\in R\)。取 \(K_p=\bigwedge^pR^n\)、\(\mathrm{d}_p=\iota_\lambda\)，得到
\[
...
\]
设 \(R\) 为交换环。一列 \(R\) 模与 \(R\) 线性箭头若满足每两个相邻箭头的复合为零，就称为一个\term[lianfuxing]{链复形}[chain complex]；此处的条件 \(\mathrm{d}_{p-1}\mathrm{d}_p=0\) 已由上一命题证明。给定元素 \(a_1,\ldots,a_n\in R\)，上面由 \(K_p=\bigwedge^pR^n\) 和 \(\mathrm{d}_p=\iota_\lambda\)、\(\lambda(e_i)=a_i\) 构造的复形称为序列 \(a_1,\ldots,a_n\) 的\term[Koszul-fuxing]{Koszul 复形}[Koszul complex]，记为 \(K_\bullet(a_1,\ldots,a_n;R)\)。\symidx{\(K_\bullet(a_1,\ldots,a_n;R)\)：Koszul 复形}它在低次上的公式是
\[
\mathrm{d}_1(e_i)=a_i,\qquad \mathrm{d}_2(e_i\wedge e_j)=a_ie_j-a_je_i.
\]
因而末端余核为 \(R/(a_1,\ldots,a_n)\)，而 \(\mathrm{d}_2\) 记录了最直接的两两关系。
