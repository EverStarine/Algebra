...
设 \(P\) 为左 \(R\) 模、\(N\le P\) 为子模。若对每个子模 \(L\le P\)，等式 \(L+N=P\) 都蕴含 \(L=P\)，则称 \(N\) 为 \(P\) 的\term[duoyu-zimo]{多余子模}[superfluous submodule]，记为 \(N\ll P\)。若左 \(R\) 模的满同态 \(p:P\to M\) 满足 \(\ker p\ll P\)，则称 \(p\) 为\term[benzhi-manshe]{本质满射}[essential epimorphism]。
\end{definition}
...
设 \(M\) 为左 \(R\) 模。若左 \(R\) 模的满同态 \(p:P\to M\) 的来源 \(P\) 投射，且 \(\ker p\ll P\)，则称 \(p\) 为 \(M\) 的一个\term[toushe-fugai]{投射覆盖}[projective cover]。定义中保留这个满同态，而不仅记录 \(P\) 的同构类型。
\end{definition}
...
设 \(R\) 为含幺环、\(I\subseteq R\) 为幂零双边理想。若 \(\bar e_1,\ldots,\bar e_m\in R/I\) 满足
\[
...
设 \(k\) 为域、\(A\) 为有限维结合含幺 \(k\) 代数，\(J=J(A)\)。有限维左 \(A\) 模 \(P\) 的子模 \(N\) 多余，当且仅当 \(N\subseteq JP\)。因此，对有限生成投射左 \(A\) 模的满射 \(p:P\to M\)，以下条件等价：它是投射覆盖；\(\ker p\subseteq JP\)；诱导映射 \(P/JP\to M/JM\) 是同构。
\end{proposition}
\begin{proof}
...
设 \(A\) 为含幺代数、\(e\in A\) 为非零幂等元。若 \(e\) 不能写成两个非零正交幂等元之和，则称它为\term[benyuan-midengyuan]{本原幂等元}[primitive idempotent]。这里的正交要求两个方向的乘积都为零。
\end{definition}

\begin{proposition}\label{b2:prop:primitive-projective-indecomposable}
设 \(A\) 为含幺代数、\(e\in A\) 为非零幂等元。左 \(A\) 模 \(Ae\) 有限生成且投射；它不可分解，当且仅当 \(e\) 本原。
\end{proposition}
\begin{proof}
...
设 \(k\) 为域、\(A\) 为有限维结合含幺 \(k\) 代数，\(J=J(A)\)。取 \(A/J\) 的本原幂等元 \(\bar e\)，并取其在 \(A\) 中的幂等元提升 \(e\)，则 \(e\) 本原，\(Ae/Je\) 是简单左 \(A\) 模，且自然满射
\[
...
设 \(k\) 为域、\(A\) 为有限维结合含幺 \(k\) 代数，\(J=J(A)\)。从全部简单左 \(A\) 模的同构类中各选一个代表 \(S_1,\ldots,S_s\)，并给每个 \(S_i\) 选定投射覆盖 \(p_i:P_i\to S_i\)，则每个有限维左 \(A\) 模 \(M\) 都有投射覆盖。若
\[
...
设 \(k\) 为域、\(A\) 为有限维结合含幺 \(k\) 代数，\(J=J(A)\)。从全部简单左 \(A\) 模的同构类中各选一个代表 \(S_1,\ldots,S_s\)，并给每个 \(S_i\) 选定投射覆盖 \(p_i:P_i\to S_i\)，则每个有限生成投射左 \(A\) 模都是 \(P_1,\ldots,P_s\) 的有限直和。不可分解投射模恰为这些 \(P_i\) 的同构类型，而
\[
P\longmapsto P/JP
\]
...
设 \(k\) 为域、\(A\) 为有限维结合含幺 \(k\) 代数，\(M\) 为有限维左 \(A\) 模。若 \(p:P\to M\) 与 \(q:Q\to M\) 是 \(M\) 的两个投射覆盖，则存在 \(A\) 模同构 \(a:P\to Q\)，使 \(qa=p\)。这个同构本身未必唯一。
\end{theorem}
\begin{proof}
...
设 \(k\) 为域、\(A\) 为有限维结合含幺 \(k\) 代数，\(M\) 为左 \(A\) 模。对有限生成投射左 \(A\) 模的满射 \(p:P\to M\)，它是投射覆盖，当且仅当每个满足 \(pu=p\) 的 \(A\) 模自同态 \(u:P\to P\) 都可逆。若 \(p\) 是覆盖，且 \(q:Q\to M\) 是任意有限生成投射左 \(A\) 模的满射，则 \(P\) 是 \(Q\) 的直和因子。
\end{proposition}
\begin{proof}
