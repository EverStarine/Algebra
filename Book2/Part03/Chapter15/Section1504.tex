...
设 \(A\) 为含幺代数、\(e\in A\) 为幂等元。若 \(AeA=A\)，则称 \(e\) 为\term[man-midengyuan]{满幂等元}[full idempotent]。例如 \(M_n(k)\) 中取 \(e=E_{11}\)，角代数就是 \(k\)；上三角矩阵代数和 \(k[t]/(t^m)\) 则已经是基本代数。基本性并不要求根为零。
...
\]
按左右两个顶点区分根的第一层。设 \(k\) 为域、\(B\) 为有限维基本 \(k\) 代数，且 \(B/J(B)\cong k^s\)。取完整正交本原幂等元组 \(e_1,\ldots,e_s\)，并记 \(J=J(B)\)。为每个空间 \(e_j(J/J^2)e_i\) 选一组 \(k\) 基，每个基向量记录为一条从 \(i\) 指向 \(j\) 的箭头。由此得到的允许重边与圈的有向图称为\term[jiantu]{箭图}[quiver]。这里的箭头方向专门配合左模约定。
