...
模 \(M\) 的\term[mo-de-gen]{根}[radical of a module]记为 \(\operatorname{rad}M\)，定义为全部极大真子模的交。它的\term[jizuo]{基座}[socle]记为 \(\operatorname{Soc}M\)，定义为全部简单子模之和；若没有简单子模，该和为零。对零模约定 \(\operatorname{rad}0=\operatorname{Soc}0=0\)。
\end{definition}
...
\]
商 \(M/JM\) 半单，而且每个从 \(M\) 到半单模的同态都唯一通过这个商分解。
\end{theorem}
\begin{proof}
...
令 \(\operatorname{rad}^0M=M\)、\(\operatorname{rad}^{r+1}M=\operatorname{rad}(\operatorname{rad}^rM)\)。所得降列称为\term[gen-Loewy-lie]{根 Loewy 列}[radical Loewy series]。另令 \(\operatorname{Soc}^0M=0\)，并令 \(\operatorname{Soc}^{r+1}M\) 是 \(M/\operatorname{Soc}^rM\) 的基座在 \(M\) 中的原像，得到\term[jizuo-Loewy-lie]{基座 Loewy 列}[socle Loewy series]。
\end{definition}
...
使 \(J^\ell M=0\) 的最小非负整数 \(\ell\) 称为模的\term[Loewy-changdu]{Loewy 长度}[Loewy length]。由命题，它也等于使 \(\operatorname{Soc}^\ell M=M\) 的最小整数。零模长度为零；非零半单模长度为一。
