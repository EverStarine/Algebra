...
双边理想 \(I\) 称为\term[mi-ling-lixiang]{幂零理想}[nilpotent ideal]，如果存在整数 \(N\ge1\) 使 \(I^N=0\)。其中 \(I^N\) 是全部 \(N\) 个 \(I\) 中元素的乘积所生成的加法子群。最小的这样的正整数称为其\term[miling-zhishu]{幂零指数}[nilpotency index]。
\end{definition}
...
有限维代数 \(A\) 的商代数 \(A/J\) 是半单代数。每个被 \(J\) 零化的有限维左 \(A\) 模都是半单模。
\end{proposition}
\begin{proof}
...
\ProseParagraphBreak
双边理想的幂给出\term[gen-lvguo]{根滤过}[radical filtration]
\[
...
有限维代数 \(A\) 的 Jacobson 根 \(J\) 幂零。若 \(A\ne0\)，可以取 \(N\le\dim_k A\) 使 \(J^N=0\)。因此 \(J\) 是 \(A\) 中最大的幂零双边理想。
\end{theorem}
\begin{proof}
