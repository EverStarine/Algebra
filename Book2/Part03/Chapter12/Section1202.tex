...
设 \(R\) 为含幺环。若非零左 \(R\) 模 \(S\) 只有 \(0,S\) 两个子模，则称 \(S\) 为\term[jiandan-mo]{简单模}[simple module]。对左 \(R\) 模 \(M\)，取有限子模链
\[
...
设 \(M\) 为有限长度左 \(R\) 模。\(M\) 的每个子模和商模都有合成列。若 \(M\) 有一条包含 \(n\) 个简单商层的合成列，则所构造的子模与商模合成列各至多有 \(n\) 层。
\end{lemma}
\begin{proof}
...
有限长度左 \(R\) 模 \(M\) 的任意两条合成列具有相同的层数，并且其合成因子在计入重数后，除排列次序外逐个同构。
\end{theorem}
\begin{proof}
...
对含幺环 \(R\) 上的有限长度左模 \(M\)，模的\term[mo-Jordan-Holder-dingli]{Jordan–Hölder 定理}[Jordan–Hölder theorem for modules]使以下记号有意义：
\[
...
左 \(R\) 模 \(M\) 有限长度，当且仅当它同时为 Noether 模和 Artin 模。
\end{theorem}
\begin{proof}
...
在左 \(R\) 模的短正合列 \(0\rightarrow A\rightarrow B\rightarrow C\rightarrow0\) 中，\(B\) 有限长度当且仅当 \(A,C\) 都有限长度。此时
\[
\ell(B)=\ell(A)+\ell(C),\qquad
[B:S]=[A:S]+[C:S]
\]
对每个简单左 \(R\) 模 \(S\) 都成立，其中 \(\ell\) 表示合成列长度，\([X:S]\) 表示 \(S\) 在 \(X\) 的合成列中出现的重数。
\end{proposition}
\begin{proof}
...
设 \(M\) 为有限长度左 \(R\) 模。对自同态 \(f:M\rightarrow M\)，单射、满射、可逆这三个条件等价。更一般地，
\[
\ell(M)=\ell(\ker f)+\ell(\operatorname{im}f)
\]
对任何到左 \(R\) 模 \(N\) 的同态 \(f:M\to N\) 成立。
\end{proposition}
\begin{proof}
...
设 \(k\) 为域、\(A\) 为有限维结合含幺 \(k\) 代数。对左 \(A\) 模 \(M\)，以下三个条件等价：\(M\) 有限生成；\(M\) 作为 \(k\) 向量空间有限维；\(M\) 作为 \(A\) 模有限长度。若 \(S_1,\ldots,S_r\) 为一条合成列的因子，则
\[
