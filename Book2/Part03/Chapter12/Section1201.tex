...
\(M\) 满足升链条件，当且仅当每个非空子模集合在包含关系下有极大元。把升链、极大元分别换成降链、极小元，也有同样的等价。
\end{proposition}
\begin{proof}
...
设 \(A\subseteq B\)，\(q:B\rightarrow B/A\) 为商映射。若 \(L\subseteq L'\subseteq B\) 满足
\[
...
若 \(0\rightarrow A\rightarrow B\rightarrow C\rightarrow0\) 为短正合列，则 \(B\) 为 Noether 模，当且仅当 \(A,C\) 均为 Noether 模；对 Artin 模也有相同结论。
\end{theorem}
\begin{proof}
...
若左正则模 \({}_RR\) 是 Noether 模或 Artin 模，就分别称 \(R\) 为\term[zuo-Noether-huan]{左 Noether 环}[left Noetherian ring]或\term[zuo-Artin-huan]{左 Artin 环}[left Artinian ring]。相应地，对右正则模 \(R_R\) 定义右 Noether 环和右 Artin 环。它们分别要求左理想链或右理想链满足相应条件。
\end{definition}
...
\(R\) 是左 Noether 环，当且仅当每个有限生成左 \(R\) 模都为 Noether 模。把 Noether 换成 Artin，同样成立。
\end{proposition}
\begin{proof}
