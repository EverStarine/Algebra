...
设 \(R\) 为含幺环。若 \(R\) 半单，则每个左 \(R\) 模既投射又内射。反过来，若每个左 \(R\) 模都投射，或每个左 \(R\) 模都内射，则 \(R\) 半单。
\end{proposition}
\begin{proof}
...
对非零含单位元的环 \(R\)，有
\[
\bigcap_{S\text{ 简单左 }R\text{ 模}}\operatorname{Ann}_R(S)
...
若含幺环 \(R\) 的全部简单左模的零化子之交为零，则称 \(R\) 具有\term[banbenyuan-xing]{半本原性}[semiprimitivity]。第十四章将把这个交系统地作为 Jacobson 根研究；本节只使用已经说明的零化子条件，不调用后面的根理论。
