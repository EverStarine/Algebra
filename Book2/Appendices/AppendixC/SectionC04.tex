...
设 \(R\) 为非零交换含幺环，\(\operatorname{Spec}R\) 为其素理想谱。对 \(f\in R\) 记
\[
D(f)=\{\,\mathfrak p\in\operatorname{Spec}R\mid f\notin\mathfrak p\,\}.
\]
这些集合满足 \(D(1)=\operatorname{Spec}R\)、\(D(f)\cap D(g)=D(fg)\)。把主开集 \(D(f)\)（\(f\in R\)）作为开集的一组基，得到 \(\operatorname{Spec}R\) 上的\term[Zariski-tuopu]{Zariski 拓扑}[Zariski topology]。在 \(D(f)\) 上允许 \(f\) 可逆，对应的代数操作是 \(R_f=R[1/f]\) 以及 \(P_f=R_f\otimes_RP\)。
\symidx{\(D(f)\)：素理想谱的主开集}
...
\]
变换，并在三重交叠上满足 \(g_{ij}g_{jk}=g_{ik}\)。这里约定 \([p]_i=g_{ij}[p]_j\)。对交换环 \(R\) 上的有限生成投射模 \(P\)，这些局部自由模及其可逆坐标变换给出\term[xiangliang-cong]{向量丛}[vector bundle]的局部描述；本节只使用从已经给定的 \(P\) 导出的这一模型。不同连通部分可以有不同的局部秩。
...
设 \(R\) 为非零交换含幺环、\(P\) 为忠实有限生成投射 \(R\) 模，\(E=\operatorname{End}_R(P)\)，则左模范畴之间的函子
\[
R\text{-}\mathbf{Mod}\longrightarrow E\text{-}\mathbf{Mod},
\qquad N\longmapsto P\otimes_RN
\]
是等价，拟逆可写为 \(\operatorname{Hom}_E(P,-)\)，也可写为 \(P^*\otimes_E-\)，其中 \(P^*=\operatorname{Hom}_R(P,R)\)。这里 \(P\) 的左 \(E\) 作用为求值，\(P^*\) 的右 \(E\) 作用为 \(\lambda e=\lambda\circ e\)。
\end{proposition}
\begin{proof}
