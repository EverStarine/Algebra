...
二次型既决定几何中的正交关系，也能规定结合代数中的乘法。本附录证明 Clifford 代数的有序基定理，计算低维偶代数；在已经建立的 Witt 群上加入张量积，得到 Witt 环及其初步不变量；随后把对称型、交替型和 Hermitian 型解释为矩阵代数上的伴随对合，说明为什么相似型给出相同的伴随运算，并计算一个右四元数模上的 Hermitian 例子。最后通过自旋群的平面计算，说明代数构造怎样产生旋转，以及底域为什么会影响这种构造的像。
\end{chapterabstract}
...
\ProseParagraphBreak
Witt 环在忽略双曲分量以后，仍保留张量积及各向异性部分。维数奇偶和判别式是容易计算的不变量，二元型的进一步区别则由范数群决定。低维偶 Clifford 代数可以补充判别式的信息，但也未必单独决定型。对合从另一个方向记录型：矩阵转置的各种共轭形式对应双线性型，第二类对合必须保留中心上的非平凡共轭；同一个四元数代数也能配备不同类型的对合。相似型给出相同或共轭的伴随对合，故对合并不记录全部等距信息；除环上的具体公式还须明确左右模和标量次序。
...
比较实三维型 \(q=\langle1,1,1\rangle\) 与 \(-q=\langle-1,-1,-1\rangle\) 的判别式、符号差及偶 Clifford 代数。说明偶 Clifford 代数的同构类为何不能单独决定二次型的等距类。
\end{exercise}
\begin{solution}
两型的行列式分别为一与负一，故判别式平方类分别为负一与一；符号差分别为三与负三，所以不等距。但\cref{b2:prop:B-low-clifford}的公式中，两个四元数参数分别是首个系数与另外两个系数的乘积再取负；三个系数同时变号不改变这两个参数。因此两者的偶 Clifford 代数都为 \((-1,-1)_{\mathbb R}=\mathbb H\)。结合\cref{b2:exm:B-discriminant-even-clifford}，这说明判别式与偶代数确能记录不同的信息，但任取其中一个都不是一般的完整分类数据。
\end{solution}

\begin{exercise}\label{b2:ex:B-similarity-not-isometry}
在 \(\mathbb Q^2\) 上取型 \(q=\langle1,1\rangle\) 与 \(r=\langle3,3\rangle\)。证明它们给出相同的伴随对合，并有相同的等距子群，却不是等距的二次型。
\end{exercise}
\begin{solution}
两者的 Gram 矩阵分别为 \(I_2\) 与 \(3I_2\)，所以伴随公式都为 \(X\mapsto X^{\mathsf T}\)。矩阵条件 \(P^{\mathsf T}(3I_2)P=3I_2\) 与 \(P^{\mathsf T}P=I_2\) 等价，因此两个等距子群也相同。若两型等距，\(r\) 中取值为三的向量会给出有理数解 \(x^2+y^2=3\)。\cref{b2:exm:B-rational-norm-obstruction}已经用模三整除证明这个方程无有理解，所以两型不等距。这里相同的是各自保持型的群，并不是两个型之间存在等距映射。
\end{solution}
...
\end{exercise}
\begin{solution}
Gram 矩阵为单位，故 \(T^\dagger=T^*=\operatorname{diag}(\bar a,\bar b)\)，从而 \(T^\dagger T=\operatorname{diag}(N(a),N(b))\)。逐坐标代入配对，得到等距当且仅当 \(N(a)=N(b)=1\)。相似条件为 \(h(Tx,Ty)=c\,h(x,y)\)、\(c\in\mathbb R^\times\)，当且仅当 \(N(a)=N(b)=c\)。非零实四元数的范数为正，故这个乘子必为正。例如 \(a=i,b=j\) 给出等距变换，\(a=1+i,b=1+j\) 则给出乘子为二的相似变换。
\end{solution}
