...
复结合代数 \(A\) 上的\term[gonge-xianxing-duihe]{共轭线性对合}[conjugate-linear involution]是映射 \(a\mapsto a^*\)，满足
\[
(a+b)^*=a^*+b^*,\quad (\lambda a)^*=\bar\lambda a^*,\quad
(ab)^*=b^*a^*,\quad (a^*)^*=a.
\]
若 \(A\) 具有完备范数，且 \(\|ab\|\le\|a\|\cdot\|b\|\)，称为\term[Banach-daishu]{Banach 代数}[Banach algebra]。再配以上述对合并满足
\begin{equation}\label{b2:eq:cstar-identity}
\|a^*a\|=\|a\|^2\qquad(a\in A),
\end{equation}
便称为 \term[Cxing-daishu]{\(C^*\)-代数}[C*-algebra]。这里不默认存在单位元。保持乘法、复线性结构和对合的映射称为 \term[xing-tongtai]{\(*\)-同态}[*-homomorphism]；需要保持单位元时另行注明。
\end{definition}
...
\(B(H)\) 在算子范数下是含幺 \(C^*\)-代数；它的每个范数闭且对伴随封闭的复子代数也是 \(C^*\)-代数，未必含有 \(I_H\)。
\end{proposition}
\begin{proof}
...
\ProseParagraphBreak
有限秩算子组成 \(B(H)\) 的双边 \(*\)-理想；其范数闭包记为 \(\mathcal K(H)\)，称为\term[jinsuanzi-daishu]{紧算子代数}[algebra of compact operators]。在 Hilbert 空间上，这与把有界集送到相对紧集的算子一致，见 Blackadar\cite[\nopp I.8.1.1; I.8.1.5]{Blackadar2006OperatorAlgebras}。若 \(H\) 无限维，任何有限秩 \(F\) 都有一个单位核向量，故 \(\|I-F\|\ge1\)，所以 \(I\notin\mathcal K(H)\)。它也没有别的单位：若 \(e\) 对全部秩一算子作左单位，逐一作用于其像向量就得到 \(e\xi=\xi\) 对所有 \(\xi\)，从而 \(e=I\)。
...
非零含幺复 Banach 代数中，不存在 \(x,d\) 满足 \(dx-xd=1\)。因此复 Weyl 代数没有取值于 \(B(H)\)、\(H\ne0\)，的含幺代数表示。
\end{proposition}
\begin{proof}
