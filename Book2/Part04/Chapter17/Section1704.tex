...
\]
定义的含幺 \(k\) 代数称为\term[siyuanshu-daishu]{四元数代数}[quaternion algebra]，记为 \((a,b)_k\)。具体地，它是自由结合代数 \(k\langle i,j\rangle\) 按这些关系生成的双边理想所取的商。
\end{definition}
...
\(Q=(a,b)_k\) 以 \(1,i,j,ij\) 为 \(k\) 基，并且是次数二的中心单代数。若扩域 \(K/k\) 含有 \(\alpha\) 满足 \(\alpha^2=a\)，则有分裂同构
\[
Q_K\longrightarrow M_2(K),\qquad
...
把式\eqref{b2:eq:quaternion-norm}的标量记为 \(N(q)\)，称为\term[siyuanshu-fanshu]{四元数范数}[quaternion norm]。则
\[
N(qr)=N(q)N(r),\qquad
...
四元数代数 \(Q\) 或者是除代数，或者同构于 \(M_2(k)\)。它分裂，当且仅当其范数二次型有非平凡零点，即存在 \(q\ne0\) 使 \(N(q)=0\)。
\end{proposition}
\begin{proof}
...
设 \(a\) 不是 \(k\) 中的平方，\(L=k(\alpha)\)、\(\alpha^2=a\)。则
\[
