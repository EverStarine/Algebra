...
设 \(S\subseteq R\) 含一、乘法封闭，且全部元素正则。若对任意 \(a\in R,s\in S\)，存在 \(t\in S,b\in R\) 满足 \(at=sb\)，称 \(S\) 满足\term[you-Ore-tiaojian]{右 Ore 条件}[right Ore condition]，也称为本节意义下的右 Ore 分母集。

左 Ore 条件要求对任意 \(a,s\)，存在 \(t\in S,b\in R\) 使 \(ta=bs\)，它对应左分式 \(s^{-1}a\)。左条件就是反环中的右条件，不因两侧分母都是正则元素就自动相同。
\end{definition}
...
对右 Ore 分母集 \(S\)，存在环 \(Q=RS^{-1}\) 及单射 \(R\rightarrow Q\)，使 \(S\) 中每个元素可逆，且每个 \(Q\) 元素都为 \(as^{-1}\)。对任意保单位元环同态 \(f:R\rightarrow B\)，若全部 \(f(s)\) 可逆，则存在唯一同态
\[
...
在 \(Q=RS^{-1}\) 中，任意有限组 \(q_1,\ldots,q_m\) 有共同右分母：存在 \(s\in S\) 使全部 \(q_i s\in R\)。若 \(I\) 为 \(R\) 的右理想，则
\[
