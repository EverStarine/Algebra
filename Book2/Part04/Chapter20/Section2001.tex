...
代数 \(A\) 的一个非负\term[lvguo]{滤过}[filtration]是一列 \(k\) 子空间
\[
0=F_{-1}A\subseteq F_0A\subseteq F_1A\subseteq\cdots,
\quad \bigcup_{n\ge0}F_nA=A,
\]
满足 \(1\in F_0A\) 及 \(F_iA\,F_jA\subseteq F_{i+j}A\)。负指标的滤过项均约定为零。相容的模滤过满足 \(\bigcup_nF_nM=M\)、\(F_nM=0\) 对 \(n<0\)，以及 \(F_iA\,F_jM\subseteq F_{i+j}M\)。
\end{definition}

对有统一整数下界的模滤过，平移模的指标便可使用本节的论证。这里的穷尽性保证每个元素最终出现，下界则保证降次归纳能到达零；两项条件各有用途。

\ProseParagraphBreak
分次给出滤过 \(F_nA=\bigoplus_{i\le n}A_i\)。反过来，滤过不一定来自这样一个与乘法相容的直和。例如关系 \(dx-xd=1\) 混合二次与零次，但按字长不超过 \(n\) 所生成的子空间仍构成滤过。后面将严格构造这个例子。
...
滤过代数的\term[bansui-fenci-daishu]{伴随分次代数}[associated graded algebra]为
\[
\operatorname{gr}_F A=\bigoplus_{n\ge0}F_nA/F_{n-1}A,
\]
乘法由 \((a+F_{i-1}A)(b+F_{j-1}A)=ab+F_{i+j-1}A\) 给出。模的伴随分次 \(\operatorname{gr}_F M\) 同样定义。若非零 \(a\) 首次属于 \(F_nA\)，称 \(n\) 为其滤过次数，\(\sigma(a)=a+F_{n-1}A\) 为其\term[zhufuhao]{主符号}[principal symbol]。
\end{definition}
...
设 \(M\) 带有上述相容、穷尽且非负的滤过。如果 \(\operatorname{gr}_F M\) 由有限个齐次元素 \(\bar m_i\)、次数 \(d_i\)，作为 \(\operatorname{gr}_F A\) 模生成，则任意代表 \(m_i\in F_{d_i}M\) 生成 \(M\)，并有
\[
F_nM=\sum_i(F_{n-d_i}A)m_i.
\]
\end{theorem}
...
若 \(\operatorname{gr}_F A\) 是左 Noether 环，则 \(A\) 也是左 Noether 环。若 \(\operatorname{gr}_F M\) 是 Noether 左 \(\operatorname{gr}_F A\) 模，则 \(M\) 是 Noether 左 \(A\) 模。右侧版本也成立。
\end{theorem}
\begin{proof}
...
若 \(\operatorname{gr}_F A\) 没有零因子，则 \(A\) 没有零因子，且非零元素的乘积次数等于次数之和。
\end{proposition}
\begin{proof}
