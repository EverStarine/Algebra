...
设 \(k\) 为域，\(A\) 为由 \(x_1,\ldots,x_n\) 生成的含幺结合 \(k\) 代数，\(n\geq1\)。以这些生成元的字长不超过 \(m\) 的乘积张成 \(F_mA\)，并假设全部交换子 \([x_i,x_j]=x_ix_j-x_jx_i\) 均属于 \(F_1A\)，则有满同态
\[
...
在 Weyl 代数中，交换子 \(\mathrm{d}x-x\mathrm{d}\) 比乘积 \(\mathrm{d}x\) 的次数低。现在暂时不保留两个乘积，只把它们的差作为一项双线性运算。结合律会使这种运算满足额外的恒等式；这些恒等式正是下面定义的依据。
...
\]
若它满足交替性 \([u,u]=0\) 对所有 \(u\in\mathfrak g\) 成立，以及对任意 \(u,v,w\in\mathfrak g\) 成立的\term[Jacobi-hengdengshi]{Jacobi 恒等式}[Jacobi identity]
...
\]
则称 \(f\) 为\term[Lie-daishu-tongtai]{Lie 代数同态}[Lie algebra homomorphism]。若线性子空间 \(I\subseteq\mathfrak g\) 满足 \([u,a]\in I\) 对所有 \(u\in\mathfrak g\)、\(a\in I\) 成立，则称 \(I\) 为 \(\mathfrak g\) 的\term[Lie-lixiang]{Lie 理想}[ideal of a Lie algebra]。交替性使 \([a,u]\) 也属于 \(I\)。此时
\[
[u+I,v+I]=[u,v]+I
\]
...
设 \(k\) 为域，\(\mathfrak h\) 为以 \(x,d,z\) 为基、满足 \([d,x]=z\)、\([z,x]=[z,d]=0\) 的三维 Heisenberg Lie 代数，\(j:\mathfrak h\rightarrow U(\mathfrak h)\) 为自然映射，则有 \(k\) 代数同构
\[
U(\mathfrak h)/\Bigl(j(z)-1\Bigr)\cong A_1(k),
\]
将 \(j(x),j(d)\) 的商类分别送到 Weyl 生成元 \(x,\mathrm{d}\)。因此左 \(A_1(k)\) 模也就是中心元素 \(z\) 作用为恒等算子的 \(\mathfrak h\) 表示。
\end{proposition}
\begin{proof}
在 \(A_1(k)^-\) 中指定 \(x\mapsto x,d\mapsto\mathrm{d},z\mapsto1\)。Weyl 关系 \([\mathrm{d},x]=1\) 及单位元的中心性核对了\eqref{b2:eq:heisenberg-brackets}；双线性保证保持任意两向量的括号。\cref{b2:thm:enveloping-universal}于是给出 \(U(\mathfrak h)\to A_1(k)\)，且 \(j(z)-1\) 映为零，故可下降到所述商代数。

反过来，记 \(\bar x,\bar d\) 分别为 \(j(x),j(d)\) 在该商中的像，它们满足 \(\bar d\bar x-\bar x\bar d=1\)。由\cref{b2:prop:algebra-relations-universal}，赋值 \(x\mapsto\bar x,\mathrm{d}\mapsto\bar d\) 唯一给出从 \(A_1(k)\) 返回该商的同态。该商由 \(\bar x,\bar d\) 生成，因为 \(j(z)\) 的像已为一。两个复合均固定对应生成元和单位元，所以互为逆映射。最后，\cref{b2:cor:lie-representations-enveloping-modules}把关系 \(j(z)-1=0\) 对应为 \(z\) 作用为恒等算子，得到表示的说明。
\end{proof}
...
设 \(k\) 为域，\(\mathfrak g\) 为有限维 \(k\) Lie 代数，\(x_1,\ldots,x_n\) 为其有序基，\(U(\mathfrak g)\) 为泛包络代数，\(j:\mathfrak g\rightarrow U(\mathfrak g)\) 为自然映射。按 \(j(x_i)\) 的乘积长度滤过 \(U(\mathfrak g)\)，则自然映射 \(\operatorname{Sym}_k(\mathfrak g)\rightarrow\operatorname{gr}U(\mathfrak g)\) 为同构；等价地，全部 \(j(x_1)^{a_1}\cdots j(x_n)^{a_n}\)、\(a_i\geq0\)，构成 \(U(\mathfrak g)\) 的 \(k\) 基。特别地，\(j\) 单射。
\end{claim}
