...
\end{optionalexercise}
\begin{solution}
设 \(x,\mathrm{d}\) 的作用矩阵为 \(X,D\)。关系给出 \(DX-XD=I\)，取迹得到 \(0=\dim V\) 在 \(k\) 中的像。这在特征零排除正维数，在特征 \(p\) 要求 \(p\mid\dim V\)。
...
\[
A_1(k)/(x^p,\mathrm{d}^p)\cong M_p(k).
\]
\end{optionalexercise}
\begin{solution}
上一题的作用零化 \(x^p,\mathrm{d}^p\)，故从该商得到到 \(\operatorname{End}_k\Bigl(k[X]/(X^p)\Bigr)\) 的同态。商由 \(x^i\mathrm{d}^j\)、\(0\le i,j<p\)，线性生成，所以维数至多为 \(p^2\)。
...
\end{optionalexercise}
\begin{solution}
由\cref{b2:thm:weyl-center-simplicity}，中心为 \(k[x^p,\mathrm{d}^p]\)。将两个指数分别除以 \(p\)，便把每个单项式唯一写为中心单项式乘 \(x^i\mathrm{d}^j\)、\(0\le i,j<p\)。展开中心系数后，普通正规基的独立性说明这些剩余单项式也是中心上的基。特征零时中心为 \(k\)，而 Weyl 代数具有无限多个独立正规单项式，所以在中心上不有限生成。
\end{solution}
...
\]
\(\lambda=0\) 时这个商为 \(k[X,D]\)，由对称代数的多项式解释得到所给基。\(\lambda\ne0\) 时，Weyl 生成元 \(x,\mathrm{d}\) 分别送到 \(\bar x,\lambda^{-1}\bar d\)，反向将 \(\bar x,\bar d\) 送到 \(x,\lambda\mathrm{d}\)，得到与 \(A_1(k)\) 的互逆同构。\cref{b2:thm:weyl-normal-basis}于是给出相同的基结论，不需要使用一般 PBW。
...
在 \(K=k(x)\)、\(\delta=\mathrm{d}/\mathrm{d}x\) 上令 \(L=\partial^2-x\)。分别写出 \(\partial\) 在左模 \(M_L\) 的基 \(e_0=1+DL,e_1=\partial+DL\) 中的作用坐标式，以及标量解 \(y\) 的列向量 \((y,\delta y)^{\mathsf T}\) 所满足的系统。
\end{optionalexercise}
\begin{solution}
