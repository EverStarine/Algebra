...
**第一卷正文三编初稿已完成**：第一编第1—9章，36节、72题全解；第二编第10—13章，16节、59题全解；第三编“域扩张与 Galois 理论”第14—22章，37节、137题全解。合计22章、89节和268题，各编导读、章摘要与目标、正文证明及小结均已写入。可直接阅读 [Book1.pdf](Book1.pdf)。目前完成写作阶段的核对与编译，仍待正式审阅；第一卷附录及后记、第二至五卷仍待撰写。
...
正文和原已分节附录共设 1,603 个小节，五卷依次为 289、279、426、266、343 个。附录节包括纲要原列的 35 节，以及根据其余 15 个附录主题补齐的 60 节。
...
按[参考资料目录](参考资料目录.md)查找各卷参考书及辅助材料。本地 `参考文献原文件/` 仿照分析学项目分设 `Shared/`、`Book1/` 至 `Book5/`，已从 textbook 复制40份资料、从作者及机构官网补充14份PDF。第一批8条及审查包增补20条记录尚未取得全文。文件版本、来源、阅读范围和SHA256记录在该目录的 `来源记录.md` 与 `catalog.json`。

五卷共用的正式书目主文件为 [Shared/References.bib](Shared/References.bib)，按正式发表资格及书籍五年／论文三年的通常门槛筛选后，现有52条，本次无例外（新增1882年正式发表的Lindemann论文）。原88条候选中37条已移出Bib，只保留在内部JSON／Markdown目录，见[入库资格清理](书目入库资格清理.md)。教材仍只打印实际引用的条目。资料目录分别注明资料键、用途、文件状态和是否进入正式Bib；本地参考文件不上传公开仓库。外部审查接入过程见[书目审查接入记录](书目审查接入记录.md)，其中88条入库的旧处理已由本次资格清理纠正；实际写作按[核心论证来源表](核心论证来源表.md)落实原文定位。

本地资料完整性可用 `python ./scripts/Check-ReferenceLibrary.py` 核验；未复制全文的其他工作区会提示资料库未安装。数据库和排版检查按[书目维护注意事项](书目维护注意事项.md)执行。
