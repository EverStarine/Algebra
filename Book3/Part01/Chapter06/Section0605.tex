...
设 \(A\) 为整闭 Noether 整环，\(K=\operatorname{Frac}(A)\)，\(L/K\) 为有限可分扩张。则 \(A\) 在 \(L\) 中的整闭包 \(B\) 是有限 \(A\) 模。
\end{lemma}
\begin{proof}
...
设 \(k\) 为特征 \(p>0\) 的域，\(K=k(x_1,\ldots,x_d)\)，\(L/K\) 为有限扩张。在同一个代数闭包内，可以选取 \(q=p^e\)、有限纯不可分扩张 \(k'/k\) 及元素 \(y_j\) 满足 \(y_j^q=x_j\)，使
\[
...
设 \(k\) 为任意域，\(A=k[x_1,\ldots,x_d]\)，\(K=\operatorname{Frac}(A)\)。对任意有限扩张 \(L/K\)，\(A\) 在 \(L\) 中的整闭包是有限 \(A\) 模。
\end{theorem}
\begin{proof}
...
设 \(C\ne0\) 为约化 Noether 环，极小素理想为 \(\mathfrak p_1,\ldots,\mathfrak p_r\)，\(K_i=\operatorname{Frac}(C/\mathfrak p_i)\)。则自然映射给出同构
...
设 \(k\) 为任意域。
\begin{enumerate}
\item 若 \(C\) 为有限型 \(k\) 整环，\(L'/\operatorname{Frac}(C)\) 为有限域扩张，则 \(C\) 在 \(L'\) 中的整闭包是有限 \(C\) 模。特别地，\(C\) 的正规化有限。
\item 若 \(C\) 为约化有限型 \(k\) 代数，则它在总分式环中的正规化是有限 \(C\) 模。
\end{enumerate}
\end{theorem}
\begin{proof}
