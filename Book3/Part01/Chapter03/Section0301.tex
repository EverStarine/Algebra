...
\(\sum_{i=1}^b a_i m_i\)，其中 \(a_i\in R\)，则称 \(M\) 为\term[youxian-shengcheng-mo]{有限生成模}[finitely generated module]，也简称有限模。若存在非负整数 \(a,b\) 及 \(R\) 模正合列
\[
R^a\xrightarrow{A}R^b\longrightarrow M\longrightarrow0,
\]
则称 \(M\) 为\term[youxian-zhanshi-mo]{有限展示模}[finitely presented module]。该正合列给出一个有限展示：\(R^b\) 的基给出生成元，矩阵 \(A\) 的列生成它们的全部关系。
\end{definition}
