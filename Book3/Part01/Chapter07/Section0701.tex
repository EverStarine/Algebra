...
记 \(\mathbb A_k^n=k^n\)。形如 \(X=Z_k(I)\)、\(I\subseteq k[x_1,\ldots,x_n]\) 为理想，的子集称为\term[fangshe-daishu-ji]{仿射代数集}[affine algebraic set]。以这些子集为闭集，得到 \(\mathbb A_k^n\) 的 Zariski 拓扑；每个代数集赋予子空间拓扑。本书允许代数集为空或可约；非空不可约的代数集称为\term[fangshe-daishu-cu]{仿射代数簇}[affine variety]。
\end{definition}
...
非空代数集 \(X\subseteq k^n\) 不可约，当且仅当 \(I(X)\) 素，也当且仅当 \(k[X]\) 为整环。每个代数集有有限个不可约分支，它们对应 \(k[X]\) 的极小素理想。
\end{proposition}
\begin{proof}
...
代数集 \(X\) 连通，当且仅当 \(k[X]\) 没有除零与一以外的幂等元。
\end{theorem}
\begin{proof}
