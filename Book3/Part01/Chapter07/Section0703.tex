...
若 \(A=k[x_1,\ldots,x_n]/I\)，则
\[
K\otimes_kA\cong K[x_1,\ldots,x_n]/I K[x_1,\ldots,x_n].
\]
若 \(A,B\) 是以两组不同变量表示的有限型 \(k\) 代数，则它们的张量积由两组变量及两组原关系共同表示。
\end{proposition}
\begin{proof}
...
对有限型 \(k\) 代数 \(A\) 及域扩张 \(K/k\)，一个 \(K\) 值点是 \(k\) 代数同态 \(A\to K\)。当 \(K=k\) 时称为 \(k\) 有理点；当 \(K\) 为代数闭域时称为以 \(K\) 为值的\term[jihe-dian]{几何点}[geometric point]。
\end{definition}
