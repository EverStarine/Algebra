...
设 \(A\) 是交换含幺整闭整环，\(K=\operatorname{Frac}(A)\)，\(L/K\) 为域扩张，\(b\in L\) 在 \(A\) 上整。则 \(b\) 在 \(K\) 上的首一最小多项式属于 \(A[T]\)。进一步，若 \(A\subseteq B\subseteq L\) 为整环的整扩张，\(\mathfrak p\in\operatorname{Spec}A\)，而 \(b\in\mathfrak pB\)，则此最小多项式的首项以外的全部系数，都属于 \(\mathfrak p\)。
\end{lemma}
\begin{proof}
