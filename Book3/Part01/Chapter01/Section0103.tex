...
若 \(R\) 为交换含幺 Noether 环，则 \(\operatorname{Spec}R\) 的闭集降链最终稳定，并且它是有限多个极大不可约闭子集之并。它们恰为 \(V(\mathfrak p_1),\ldots,V(\mathfrak p_r)\)，其中 \(\mathfrak p_i\) 是 \(R\) 的全部极小素理想；每个素理想都包含其中至少一个。零环对应空分解。
\end{proposition}
\begin{proof}
...
\ProseParagraphBreak
设 \(R\) 为交换含幺环，\(\mathfrak p,\mathfrak q\) 为 \(R\) 的素理想。若 \(\mathfrak p\subseteq\mathfrak q\)，则称 \(\mathfrak q\) 是 \(\mathfrak p\) 的\term[teshuhua]{特殊化}[specialization]，并称 \(\mathfrak p\) 是 \(\mathfrak q\) 的\term[yibanhua]{一般化}[generalization]；包含方向在这里不能颠倒。一般点保存的是整条不可约分支的共同代数关系，闭点则可能带有一个较大的剩余域。第六、七章将在代数闭基域的有限型情形把闭点解释为通常的坐标点。
