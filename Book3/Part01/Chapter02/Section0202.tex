...
设 \(R\) 为交换含幺环，\(S\subseteq R\) 为含一的乘法集，\(I\) 为 \(R\) 的理想。\(I\) 关于 \(S\) 的\term[chengfaji-baohe]{饱和}[saturation with respect to a multiplicative set]为
\[
I:S^\infty\coloneqq\{\,r\in R\mid sr\in I\text{ 对某个 }s\in S\text{ 成立}\,\}.
\]
若 \(I:S^\infty=I\)，则称 \(I\) 关于 \(S\) 饱和。记号中的 \(\infty\) 仅提示允许乘上分母；这里没有把 \(S\) 当作理想取幂。
\end{definition}
