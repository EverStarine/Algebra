...
\]
自然映射 \(\iota_M:M\rightarrow\widehat M\) 把 \(x\) 送到 \((x+I^nM)_n\)。若此映射为同构，称 \(M\) 为 \(I\) 进完备模；此处“完备”包含分离性。
...
对任意交换环 \(A\)，\term[xingshi-miji-shu-huan]{形式幂级数环}[formal power series ring] \(A[[x_1,\ldots,x_r]]\) 的元素是全部系数族
\(\sum_{\alpha\in\mathbb N^r}a_\alpha x^\alpha\)。加法逐系数定义，乘法定义为
\[
...
\]
对固定的多重指标 \(\gamma\)，内层和只有有限项，故不需要分析意义的收敛。
\end{definition}
...
设 \(A\) 为交换环。级数 \(f\in A[[x_1,\ldots,x_r]]\) 可逆，当且仅当其常数项 \(a_0\) 在 \(A\) 中可逆。
\end{proposition}
\begin{proof}
