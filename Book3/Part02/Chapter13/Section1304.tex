...
设 \(R\) 为交换 Noether 环、\(I\subseteq R\) 为理想，\(M,N\) 为有限生成 \(R\) 模，所有完备化均关于 \(I\) 进行。则
\[
\widehat{M\otimes_RN}
...
设 \((R,\mathfrak m,k)\) 为交换 Noether 局部环，\(B=\widehat R\) 为极大理想进完备化。则 \(B\) 是 Noether 局部环，其极大理想为 \(\mathfrak n=\mathfrak mB\)，剩余域自然等于 \(k\)。对每个 \(r\ge1\) 有
\[
