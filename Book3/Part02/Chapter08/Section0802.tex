...
设 \(k\) 为域，\(I\subseteq k[x_1,\ldots,x_n]\) 为理想，并固定全局单项式序。\(I\) 的 Gröbner 基 \(G\) 若各元素首一，并且每个 \(g\in G\) 的任何单项式都不被其他 \(h\in G\setminus\{g\}\) 的首单项式整除，称为\term[yuehua-Groebner-ji]{约化 Gröbner 基}[reduced Gröbner basis]。零理想的约化基为空，单位理想的约化基为 \(\{1\}\)。
\end{definition}
