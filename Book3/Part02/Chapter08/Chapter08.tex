...
理想的有限生成性并不直接解决成员判定。本章从单项式序和多元除法出发，完整建立 Gröbner 基、正规式、Buchberger 判据及算法、约化基唯一性，再用消去计算投影闭包、理想交商和饱和。计算过程中始终保存可核验的多项式恒等式。最后以标准单项式和乘法矩阵研究零维商代数，区分向量空间维数、点数与重数，并说明换序和单变量表示的适用条件。
\end{chapterabstract}
...
\item 通过扩展变量计算消去、交、理想商和饱和，同时保留成员表示。
...
\ProseParagraphBreak
消去把投影和理想运算化为扩展多项式环中的计算。几何投影通常只得到闭包；参数分母、特殊取值和不能提升的点仍须另行检查。两向生成元表示证明理想相等，S 多项式的约化证明 Gröbner 性，二者承担不同任务。

\ProseParagraphBreak
零维商代数把问题带入有限维线性代数。标准单项式数计算代数维数，局部因子的长度记录重数，乘法矩阵保留坐标之间的全部关系。后续维数与 Hilbert 函数将把这种有限计数推广为多项式增长；本章的精确首项方法也将用于泛自由性的证明。
\end{chaptersummary}
...
设 \(k\) 代数闭，\(A=k[x_1,\ldots,x_n]/I\) 非零且有限维，\(T_i\) 为坐标乘法矩阵。证明零点 \(a\in Z_k(I)\) 恰对应一组共同特征值：存在非零 \(v\)，使 \(T_iv=a_iv\) 对所有 \(i\) 成立。这是 Sturmfels 的《Solving Systems of Polynomial Equations》中\cite[Theorem 2.6, pp. 17--18]{Sturmfels2002PolynomialEquations}的结论；本题要求包括非约化情形。
\end{exercise}
\begin{solution}
若有这样的 \(v\)，则任意 \(f\in I\) 满足 \(0=f(T_1,\ldots,T_n)v=f(a)v\)，所以 \(f(a)=0\)。反过来，对零点 \(a\) 取相应的局部 Artin 因子 \(A_{\mathfrak m_a}\)。其极大理想 \(\mathfrak n\) 幂零；若 \(\mathfrak n=0\)，取 \(v=1\)，否则取最后一个非零幂 \(\mathfrak n^r\) 中的非零 \(v\)，使 \(\mathfrak n v=0\)。由于 \(x_i-a_i\in\mathfrak n\)，有 \(x_iv=a_iv\)。把这个 \(v\) 放入该局部因子、其余因子取零，就得到 \(A\) 中所需非零向量。此证明不要求 \(I\) 为根理想。
\end{solution}

\begin{exercise}\label{b3:ex:08-simultaneous-diagonalization}
在\cref{b3:ex:08-joint-eigenvalues}的条件下，证明所有 \(T_i\) 能同时对角化，当且仅当 \(A\) 约化。此结论见\cite[Corollary 2.7, p. 18]{Sturmfels2002PolynomialEquations}，这里要求写出两个方向的证明。
\end{exercise}
\begin{solution}
若 \(A\) 约化，局部 Artin 分解及代数闭性给出 \(A\simeq k^D\)。取各因子的一作为基，所有乘法算子同时为对角矩阵。反过来，若 \(T_i\) 同时对角化，每个 \(T_a\) 都是它们的多项式，也为对角矩阵。若 \(a\) 幂零，则 \(T_a\) 是幂零对角矩阵，只能为零；乘法表示单射，故 \(a=0\)。因此 \(A\) 约化。
\end{solution}
