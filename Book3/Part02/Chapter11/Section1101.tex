...
\]
则称此分解为 \(A\) 的一个\term[zheng-fenci]{正分次}[positive grading]。属于单个 \(A_n\) 的元素称为次数 \(n\) 的\term[qici-yuansu]{齐次元素}[homogeneous element]。若 \(A=A_0[A_1]\)，且 \(A_1\) 是有限 \(A_0\) 模，称此分次为\term[biaozhun-fenci]{标准分次}[standard grading]。若 \(A\) 模 \(M\) 具有直和分解 \(M=\bigoplus_{n\in\mathbb Z}M_n\)，满足 \(A_iM_j\subseteq M_{i+j}\)，则称 \(M\) 为分次 \(A\) 模。分次同态在本章均指保持次数的模同态。
\end{definition}
