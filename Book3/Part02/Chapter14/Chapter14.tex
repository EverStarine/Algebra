...
设 \(k\) 为域、\(n\ge2\)、\(B=k[t]/(t^n)\)。计算 \(\Omega_{B/k}\) 的 \(k\) 维数，并计算余法映射 \((t^n)/(t^{2n})\to B\,dt\) 的核。
\end{exercise}
\begin{solution}
微分展示为 \(\Omega_{B/k}=B\,dt/(nt^{n-1}dt)\)。若 \(n\ne0\) 在 \(k\) 中，维数为 \(n-1\)；若 \(n=0\)，维数为 \(n\)。余法模以 \(t^n\) 的类为一个 \(B\) 自由基，映射等同于乘 \(nt^{n-1}\)。当 \(n\ne0\) 时，其核由 \(t\) 生成；当 \(n=0\) 时，整个余法模都是核。两种情形中都不能给该正合列的左端添加零。
\end{solution}

\begin{exercise}\label{b3:ex:14-laurent-differentials}
设 \(k\) 为域，\(B=k[x^{\pm1},y^{\pm1}]\)。证明 \(dx/x,dy/y\) 为 \(\Omega_{B/k}\) 的自由基，计算任意整数 \(a,b\) 对应的 \(d(x^ay^b)\)。
\end{exercise}
\begin{solution}
多项式微分局部化后，\(dx,dy\) 仍为自由基；分别乘单位 \(x^{-1},y^{-1}\) 得到所述新基。由 \(d(x^{-1})=-x^{-2}dx\) 及整数幂的归纳，
\[
d(x^ay^b)=x^ay^b\left(a\frac{dx}{x}+b\frac{dy}{y}\right).
\]
...
\end{exercise}
\begin{solution}
点为 \((a,a^2)\)，Jacobian 行为 \((0,1)\)，所以切空间是 \(\Bigl\{(v,0):v\in k\Bigr\}\)，维数一。方程对 \(y\) 的偏导仍为单位，给出非零约束。事实上 \(B\cong k[x]\)，微分关系只是 \(dy=0\)，余下 \(dx\) 自由；不能只检查某一个偏导数。
\end{solution}
...
\end{exercise}
\begin{solution}
总有 \(\Omega_{B/k}=(B\,dx\oplus B\,dy)/(2x\,dx+2y\,dy)\)。特征不为二时，在每个有理点 \((a,b)\) 上，\(a,b\) 不同时为零，故切空间维数一。特征二时，关系多项式为 \((x+y-1)^2\)，微分关系为零，所以微分模自由秩二，所有有理点的切空间维数二。约化商是 \(k[x,y]/(x+y-1)\cong k[x]\)，它在对应点的切空间维数一。这说明改变特征可能把一个方程变成重方程。
\end{solution}
...
\end{exercise}
\begin{solution}
有 \(\Omega_{L/k}=L\,d\alpha\)、\(\Omega_{E/k}=E\,d\beta\)、\(\Omega_{E/L}=E\,d\beta\)。第一箭头将 \(1\otimes d\alpha\) 送到 \(d(\beta^p)=0\)，故为零；第二箭头将 \(d\beta\) 送到 \(d\beta\)，为同构。因此该列为
...
\end{exercise}
\begin{solution}
首一除法给出基 \(1,t\)，故自由平坦。微分关系为 \((2t-1)dt=0\)，而在 \(B\) 中
\((2t-1)^2=4t^2-4t+1=5\)。倒置五后 \(2t-1\) 为单位，所以微分模为零，有限呈示条件也成立。模五后，\(t^2-t-1=(t-3)^2\)，纤维为 \(\mathbb F_5[\varepsilon]/(\varepsilon^2)\)，其微分模为 \(\Bigl(\mathbb F_5[\varepsilon]/(\varepsilon)\Bigr)d\varepsilon\ne0\)，故不非分歧。
\end{solution}
...
\end{exercise}
\begin{solution}
关系 \(t\) 来自基环，其相对微分为零，所以 \(\Omega_{B/A}=B\,dx\)。但乘 \(t\) 在 \(A\) 上单射，在非零模 \(B\) 上为零，故不平坦。取 \(C=k[t,x]/(t^2)\)、\(J=(t)\)，则 \(C/J=B\)。恒等同态 \(B\to C/J\) 没有 \(A\) 代数提升，因为那将要求 \(C\) 中非零的 \(t\) 为零。可见即使微分模非零且自由，也不能单凭这一条件断言光滑。
\end{solution}
