...
\]
右边的包含关系可直接由 \(t=y^2-x^3-F_t\) 核验，反向也由各生成元的表达式得到。因此异常点只能出现在参数 \(t=0\) 的原点；其余参数的每个几何点都有一维切空间。若只求异常点集，取根理想得到 \((x,y,t)\)；保留原理想则还能看见 \(x^2=0\) 的加厚信息。这个计算把消去参数、求根理想和求切空间区分为三个实际任务。
...
\]
因为 \(\mathrm{d}(u^p-s)=0\)。这里相对微分纤维非零，完全来自剩余域扩张的不可分性。若扩域到 \(K=k(s^{1/p})\)，则
\[
