...
\end{corollary}
\begin{proof}
对 \(P=A[X]\) 的商应用余法正合列，\(\Omega_{P/A}\) 为以 \(\mathrm{d}X_i\) 为基的自由模。若 \(f=\sum_i g_if_i\)，在 \(B\) 上有 \(\mathrm{d}f=\sum_i g_i\,\mathrm{d}f_i\)，因为含 \(f_i\,\mathrm{d}g_i\) 的项消失。因此全部微分关系由各 \(\mathrm{d}f_i\) 生成，给出所列有限矩阵。
\end{proof}

左端不一定单射。例如在特征 \(p\) 的域上取 \(B=k[t]\)、\(J=(t^p)\)。模 \(J/J^2\) 是非零的循环 \(B/J\) 模，而 \(\delta(t^p)=p t^{p-1}\mathrm{d}t=0\)，所以整个余法映射为零。
...
改变基环意味着将更多元素视为常数，所以末端微分模是中间微分模的商。首箭头则可能不单射。例如 \(A=k\)、\(B=k[t^p]\subseteq C=k[t]\)、\(\operatorname{char}k=p\) 时，\(B\) 作为多项式环有非零微分 \(\mathrm{d}(t^p)\)，但其在 \(\Omega_{C/k}\) 中的像为零。
...
\qquad \mathrm{d}(b/s)=\frac{\mathrm{d}b}{s}-\frac{b\,\mathrm{d}s}{s^2}.
\]
...
\end{proposition}
\begin{proof}
从右侧到左侧的映射由 \(1\otimes \mathrm{d}b\mapsto \mathrm{d}(1\otimes b)\) 定义。反向先在纯张量上定义
\[
D(a'\otimes b)=(a'\otimes1)\otimes \mathrm{d}b.
\]
由于 \(\mathrm{d}(ab)=a\,\mathrm{d}b\) 对 \(a\in A\) 成立，此公式保持张量平衡关系，因此对任意有限和良定义。它零化 \(A'\)，并由 \(\mathrm{d}(bc)=b\,\mathrm{d}c+c\,\mathrm{d}b\) 满足乘法法则，所以对应 \(\Omega_{B'/A'}\) 到右侧的映射。两复合固定所有 \(\mathrm{d}(1\otimes b)\) 或 \(1\otimes \mathrm{d}b\)，而这些元素分别生成两侧，故互为逆。
\end{proof}
