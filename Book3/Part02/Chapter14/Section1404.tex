...
设 \(A\to B\) 为交换环同态。若 \(B\) 可写成
\(A[X_1,\ldots,X_n]/(f_1,\ldots,f_s)\)，其中生成元和关系均有限，则称 \(B\) 为有限呈示 \(A\) 代数。对这样的同态，若它形式光滑，则称其\term[guanghua]{光滑}[smooth]；若它形式非分歧，则称其\term[feifenqi]{非分歧}[unramified]；若 \(B\) 作为 \(A\) 模平坦且 \(\Omega_{B/A}=0\)，则称其\term[pingzhan]{平展}[étale]。
\end{definition}
