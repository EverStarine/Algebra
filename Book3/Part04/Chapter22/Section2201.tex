...
\[
0\longrightarrow M\longrightarrow E^0\xrightarrow{d^0}E^1
\xrightarrow{d^1}E^2\longrightarrow\cdots.
\]
...
第二卷第7.2节的足够多内射模定理已保证这种消解存在：先嵌入 \(E^0\)，再把余核嵌入 \(E^1\)，逐次进行。映射 \(M\to N\) 向两份内射消解的延拓也可逐次构造：每次先在前一微分的像上规定映射，再用目标的内射性延拓。两种延拓之差按同样办法逐次写成 \(dh+hd\)，所以在上同调上相同。这证明上述模与所选消解无关，并确定其函子性。
...
设 \(R\) 为交换环，\(f_1,\ldots,f_r\in R\)，\(M\) 为 \(R\) 模。取两项上链复形 \(C^\bullet(f;R)=[R\to R_f]\)，次数为零和一。令
\[
