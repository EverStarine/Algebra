...
设 \((R,\mathfrak m,k)\) 为非零交换 Artin 局部环。则下列条件等价：\(R\) 为 Gorenstein 环；\(R\) 为内射 \(R\) 模；\(\dim_k\operatorname{Soc}(R)=1\)。
\end{proposition}
\begin{proof}
...
设 \(S\) 为 \(n\) 维交换 Noether 完备正则局部环，\(R=S/I\)，并取有限秩自由 \(S\) 消解 \(P_\bullet\to R\)。把
\[
...
\(C[n]^j=C^{j+n}\)、\(d_{C[n]}=(-1)^nd_C\)，所以该模型的上同调为
\[
