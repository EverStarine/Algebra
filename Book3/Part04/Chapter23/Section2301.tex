...
这一定义立即给出唯一性：两个具有该性质的对象之间各有唯一的相容局部同态，复合由唯一性等于恒等映射。存在性则不是形式推论；它需要把带有指定剩余点的平展邻域组织起来。下面给出构造与性质的证明概要。
...
\begin{proofsketch}
考虑所有指明一个剩余点的平展邻域 \((B,\mathfrak n)\)：\(R\to B\) 为有限呈示平展同态，\(\mathfrak n\cap R=\mathfrak m\)，且 \(\kappa(\mathfrak n)=k\)。把局部环 \(B_{\mathfrak n}\) 依这些邻域的相容映射取滤过余极限，所得环记为 \(R^{\mathrm h}\)。两个邻域可由张量积再选取对应的剩余点共同细化；平行映射的相容性也可在进一步的平展邻域上检验。因此极限仍为局部环，极大理想是 \(\mathfrak mR^{\mathrm h}\)，剩余域是 \(k\)。每个 \(B_{\mathfrak n}\) 在 \(R\) 上平坦，滤过余极限仍平坦；局部平坦映射的剩余域非零，所以该平坦性忠实。

若一个多项式模极大理想有单根，就把所需根加入到一个标准平展邻域；其指定剩余点给出 \(R^{\mathrm h}\) 中的根。用同样方法处理任意互素首一因式分解，便得到 Hensel 性。对 Hensel 局部环 \(S\) 上的局部映射 \(R\to S\)，每个指明剩余点的平展邻域在 \(S\) 中有唯一相容的提升；在标准平展图上这是单根的存在唯一性，一般平展图可在该点附近化为这种图。取余极限得到唯一的 \(R^{\mathrm h}\to S\)，这就是所需泛性质。

若 \(R\) 为 Noether 环，平展邻域的形式平展性使 \(R/\mathfrak m^n\to B_{\mathfrak n}/\mathfrak m^nB_{\mathfrak n}\) 对每个 \(n\) 为同构：底环模 \(\mathfrak m^n\) 后极大理想幂零，从指定的剩余点逐层唯一提升，就得到这一同构及其逆。取余极限后得到定理中的各阶商同构。

Noether 性还需要理想的控制。平展邻域的细化定理说，\(R^{\mathrm h}\) 中任意有限组元素均包含在一个标准平展局部邻域 \(R'\) 中，而且 \(R^{\mathrm h}\) 也是 \(R'\) 的 Hensel 化。具体可取
\(R'=R[a]_{(\mathfrak m,a)}\)，其中 \(a\) 满足首一方程 \(f(a)=0\)、\(f(0)\in\mathfrak m\)、\(f'(0)\notin\mathfrak m\)；这里的局部化在指定剩余根零处进行。此有限邻域构造见永田雅宜\cite[(43.9), pp. 182--183]{Nagata1975LocalRings}。故 \(R'\) Noether，其极大理想为 \(\mathfrak mR'\)，且 \(R'\to R^{\mathrm h}\) 忠实平坦。若一个元素位于所有 \(\mathfrak m^nR^{\mathrm h}\) 中，在容纳它的 \(R'\) 中由忠实平坦收缩也位于所有 \(\mathfrak m^nR'\) 中，Krull 交定理使它为零。因而 \(R^{\mathrm h}\) 分离，其完备化由各阶商同构等于 \(\widehat R\)。

设 \(J=(b_1,\ldots,b_s)R^{\mathrm h}\)，且 \(c\in J\widehat R\cap R^{\mathrm h}\)。在共同邻域 \(R'\) 中容纳 \(c,b_1,\ldots,b_s\)，它的完备化也是 \(\widehat R\)。Noether 环完备化的忠实平坦性给出
\(c\in(b_1,\ldots,b_s)\widehat R\cap R'=(b_1,\ldots,b_s)R'\)，故 \(c\in J\)。因此每个有限生成理想都等于其完备化扩张的收缩。任一有限生成理想升链在 Noether 环 \(\widehat R\) 中扩张后稳定，收缩后原升链也稳定；这等价于 \(R^{\mathrm h}\) Noether。这里省略平展邻域范畴的细化及有限邻域构造的技术证明，其余有限性步骤已展开；整体构造及泛性质见\cite[\S43, (43.3), (43.5), (43.8)--(43.10), pp. 180--183]{Nagata1975LocalRings}。后面的光滑与平展证明不以本定理为前提。
\end{proofsketch}
