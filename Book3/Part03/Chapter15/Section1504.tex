...
设 \(R\) 为交换环，\(N,M\) 为 \(R\) 模，\(F_\bullet\rightarrow N\rightarrow0\) 为自由消解。复形 \(F_\bullet\otimes_RM\) 的第 \(i\) 个同调模记为 \(\operatorname{Tor}_i^R(N,M)\)，称为第 \(i\) 个 \(\operatorname{Tor}\) 模。
\end{definition}
...
\[
d(u\otimes v)=d_Pu\otimes v+(-1)^p u\otimes d_Qv.
\]
...
固定 \(P\) 后，第二变量的短正合列张量 \(P_i\) 仍逐项正合，圈的提升、取微分和投回子复形给出长正合列；第一变量由平衡同构得到。若 \(aN=0\)，消解上的乘法 \(a\) 提升零映射，逐级提升可构造同伦 \(a\operatorname{id}=dh+hd\)，张量后仍成立，所以它在同调上为零。若 \(aM=0\)，乘法已在张量复形上为零。最后，若 \(M\) 平坦，把自由消解分成核与像之间的短正合列，逐条张量仍正合，所以正次数同调消失；若 \(N\) 平坦，交换两个变量即可。
\end{proof}

\begin{corollary}\label{b3:cor:koszul-tor}
设 \(R\) 为交换环，\(x_1,\ldots,x_r\) 是 \(R\) 正则序列，\(I=(x_1,\ldots,x_r)\)。对任意 \(R\) 模 \(M\)，有
\[
