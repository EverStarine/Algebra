...
\item 在选读的样条计算中，把分片多项式的拼接条件写成矩阵，求出全部解及次数受限的维数。
\end{itemize}
\end{learninggoals}
...
\input{Part03/Chapter21/Section2106}

\begin{chaptersummary}
自由消解逐层记录生成元之间的关系。分次极小性使微分在剩余域上变为零，因而各层各次的自由秩恰为 Tor 的分次维数。Hilbert 合冲定理进一步说明，在 \(n\) 个变量的多项式环上，有限分次模的极小自由消解不会超过 \(n\) 层。

\ProseParagraphBreak
计算时，模项序与 Schreyer 构造给出全部核关系，单位微分项则指出可以删去的可缩部分。正确的记录既要有微分相乘为零的等式，也要有像等于核的依据。Hilbert 级数能由各层次数的交错和恢复，但无法反过来决定 Betti 表；初始理想的单项式计数也必须与原理想的消解信息分开。选读的样条实例把光滑拼接化为整除关系，进一步说明核、合冲模和次数计数怎样共同回答一个具体问题。
\end{chaptersummary}
...
设 \(k\) 为域，\(S=k[x_1,\ldots,x_n]\)，\(F,G,U,V,T\) 为尺寸相容的 \(S\) 系数矩阵，满足 \(G=FU\)、\(F=GV\)，且 \(T\) 的列生成 \(\ker G\)。说明为什么只给出 \(UT\) 的列未必生成 \(\ker F\)，并给出一个具体例子。
\end{exercise}
\begin{solution}
...
在 \(S=k[x,y]\) 上求 \(x^2u+xyv=x^3\) 的全部多项式解，并说明把右端改为 \(x\) 时为何无解。
\end{exercise}
\begin{solution}
...
用直线 \(x=0\) 与 \(x=1\) 将平面分成三条闭区域，要求分片多项式作 \(C^r\) 拼接，\(r\ge0\)。求全部解的唯一参数表示，以及每片全次数不超过 \(d\) 时的维数。
