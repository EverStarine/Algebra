...
设 \(R\) 为交换 Noether 正规整环，\(K=\operatorname{Frac}(R)\)，\(M\) 为有限无挠 \(R\) 模，\(V=M\otimes_RK\)。在 \(V\) 中自然识别后，有
\[
...
设 \(R\) 为交换 Noether 正规整环，\(K\) 为其分式域。对有限除子
...
设 \(R\) 为交换 Noether 正规整环。秩一有限反射 \(R\) 模的同构类在
\[
[M]\mathbin{\star}[N]=[(M\otimes_RN)^{**}]
\]
下组成交换群，单位为 \([R]\)，逆为 \([M^*]\)。选取分式理想表示 \(I\subset K\) 后，映射
\[
