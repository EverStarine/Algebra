...
深度达到模维数时，参数系的每一步降维都可由非零因子完成。这个条件允许许多奇点存在，但约束了关联素理想及参数序列的行为，比单看维数更有内容。
...
这里 \(\dim_RM=\dim(R/\operatorname{Ann}_R M)\)。它未必等于 \(\dim R\)：例如 \(R=k[x,y]_{(x,y)}\) 上的 \(M=R/(x)\) 的深度和维数都为一，所以 \(M\) 是 CM 模。给定交换 Noether 局部环 \((R,\mathfrak m)\) 上的非零有限 \(R\) 模 \(M\)，若 \(\operatorname{depth}_RM=\dim R\)，则称 \(M\) 为\term[jidaCohenMacaulaymo]{极大 Cohen–Macaulay 模}[maximal Cohen--Macaulay module]。深度不超过模维数、模维数不超过环维数，保证这种模确实为 CM 模。
