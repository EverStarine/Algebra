...
\[
z\in Z^nP_s,\qquad x\in X^{n-1},\qquad f_s(z)=d_Xx,
\]
添一个次数 \(n-1\) 的自由生成元 \(e_{z,x}\)，规定
\(d e_{z,x}=z\)、\(f_{s+1}(e_{z,x})=x\)。因为 \(dz=0\) 且 \(f_s(z)=dx\)，复形条件与映射条件同时成立。这定义 \(P_{s+1}\)，其分次模是 \(P_s\) 与所添自由模的直和，商微分为零。取并得到半自由复形 \(P\) 及映射 \(f:P\to X\)。

每个 \(X\) 的同调类在 \(P_0\) 已有余圈代表，因而 \(H(f)\) 满。若 \(z\in Z^nP\) 映成 \(X\) 中的边界，则 \(z\) 只含有限个生成元，属于某个 \(P_s\)；选定 \(x\) 使 \(f(z)=dx\)，下一层就添入 \(e_{z,x}\) 使 \(z\) 成为边界。因此 \(H(f)\) 单，\(f\) 是拟同构。

为证 K-投射，设 \(A\) 正合，\(g:P\to A\) 是复形映射。逐层构造次数 \(-1\) 的映射 \(h\)，使 \(g=dh+hd\)。若 \(h\) 已在旧层定义，对次数 \(n\) 的新生成元 \(e\)，\(de\) 属于旧层，而且
\[
d_A\bigl(g(e)-h(de)\bigr)=g(de)-d_Ah(de)=0,
\]
后一等式用旧层上的同伦公式及 \(d^2e=0\)。由 \(A\) 正合，可选 \(h(e)\in A^{n-1}\) 使其微分为 \(g(e)-h(de)\)。自由性允许同时线性延拓到本层。逐层取并即得全局同伦。把 \(A\) 换成任意移位，得到所有 Hom 同调消失。
...
设 \(R\) 为环。若 \(P\) 为 K-投射左 \(R\)-模复形，则对任意左模复形 \(Y\)，有
\[
\operatorname{Hom}_{K(R)}(P,Y)\simeq\operatorname{Hom}_{D(R)}(P,Y).
\]
若 \(I\) 为 K-内射复形，则对任意 \(X\)，有对偶同构
