...
\end{enumerate}
记 \(\mathcal D^{[a,b]}=\mathcal D^{\ge a}\cap\mathcal D^{\le b}\)。则这对子范畴构成 \(\mathcal D\) 上的一个\term[tjiegou]{t-结构}[t-structure]。其中的交 \(\mathcal H=\mathcal D^{\le0}\cap\mathcal D^{\ge0}\) 称为它的\term[xin]{心}[heart]。
\end{definition}

\begin{theorem}\label{b4:thm:t-truncation-adjunction}
设 \(\mathcal D\) 带有上述 t-结构。包含 \(\mathcal D^{\le n}\to\mathcal D\) 有右伴随 \(\tau^{\le n}\)，包含 \(\mathcal D^{\ge n+1}\to\mathcal D\) 有左伴随 \(\tau^{\ge n+1}\)。每个对象有截断三角
\[
...
在带 t-结构的三角范畴中，\(\mathcal D^{\le n}\) 恰是 \(\mathcal D^{\ge n+1}\) 的左正交类，\(\mathcal D^{\ge n+1}\) 恰是 \(\mathcal D^{\le n}\) 的右正交类。二者均对好三角中的扩张、有限直和及直和因子封闭。
\end{proposition}
\begin{proof}
...
设 \(\mathcal D\) 带有 t-结构。若每个对象 \(X\) 都属于某个 \(\mathcal D^{\ge a}\cap\mathcal D^{\le b}\)，其中整数 \(a,b\) 可以依赖 \(X\)，则称该 t-结构\term[youjietjiegou]{有界}[bounded]。若
\[
...
设 \(\mathcal D\) 带 t-结构，\(a\le b\)。两种复合 \(\tau^{\ge a}\tau^{\le b}\) 与 \(\tau^{\le b}\tau^{\ge a}\) 自然同构，所得对象属于 \(\mathcal D^{\ge a}\cap\mathcal D^{\le b}\)。
\end{proposition}
\begin{proof}
