...
\]
映射 \(r(b,a,b')=a\) 和 \(s(a)=(0,a,-fa)\) 给出它与 \(A[1]\) 的同伦等价。确实 \(rs=1\)，而 \(1-sr=\mathrm{d}h+h\mathrm{d}\)，其中 \(h(b,a,b')=(0,0,b)\)。该锥的最后投影与 \(s\) 复合为 \(-f[1]\)，故旋转三角是好三角。逆向旋转由同一构造和平移得到：\(\operatorname{Cone}(f[r])\cong\operatorname{Cone}(f)[r]\) 的同构为 \((b,a)\mapsto(b,(-1)^ra)\)。

为核对 (TR3)，先取两个标准锥三角，设前两个分量为 \(a:A\to A'\)、\(b:B\to B'\)。同伦交换表示可以选择 \(H\) 使
\(bf-f'a=\mathrm{d}H+H\mathrm{d}\)。定义
\[
...
\[
 (\mathrm{d}h+h\mathrm{d})(c,a,b,a')=(0,a,-fa,a')=(1-sr)(c,a,b,a').
\]
