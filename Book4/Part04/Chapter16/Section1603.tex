...
\]
右端用\cref{b4:def:hom-complex}的交换群复形。次数 \(n\) 的余圈条件为 \(d_Df=(-1)^nfd_C\)，恰是 \(C\to D[n]\) 的复形映射条件；余边与零伦映射只相差一个可逆的整体符号 \((-1)^n\)，所以商群相同。这既解释了公式，也说明此处必须取 Hom 复形的上同调，而不能只看某一个次数的普通 Hom 群。
...
\(d_B=\left(\begin{smallmatrix}d_A&t\\0&d_C\end{smallmatrix}\right)\)。条件 \(d_B^2=0\) 给出 \(d_At+td_C=0\)，故 \(-t:C\to A[1]\) 为复形映射。自然映射
\[
 r:\operatorname{Cone}(j)\to C,\quad r((a,c),a')=c,
 \qquad s(c)=((0,c),-tc)
\]
满足 \(rs=1\)。取 \(h((a,c),a')=((0,0),a)\)，便有 \(1-sr=dh+hd\)。于是 \(r\) 为同伦等价，其与锥的包含复合是 \(q\)，而锥的投影与 \(s\) 复合是 \(-t\)。把锥三角经此等价运送，得到 \(\delta=-t\)。
\end{proof}
