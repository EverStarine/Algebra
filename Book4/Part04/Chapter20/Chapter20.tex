...
\item 分清有限计算的完整证明，以及紧对象刻画和导出 Morita 证明概要中的构造与深层输入。
\end{itemize}
\end{learninggoals}
...
\ProseParagraphBreak
完美复形具有有界有限生成投射模型，既能通过有限次构造从正则模得到，又能用与任意直和交换的紧性刻画。倾斜对象进一步要求全部非零移位的自身态射消失。三顶点计算把这两个条件落实为一个短消解及一张端同态矩阵；导出 Morita 定理则把这样的计算转化为导出等价。紧对象的概要用望远镜与有限因子分解提取有限模型，Morita 的概要经 DG 端同态代数和双模替换构造等价，均注明省略步骤及来源。锥映射中的同伦选择、导出张量中的额外同调，说明为什么继续研究时有必要保留增强以及更多相容资料。
\end{chaptersummary}
