...
普通导出态射只保留同伦类，无法记录态射之间的同伦及其复合。保留完整 Hom 复形以后，乘法与微分的符号必须满足分次 Leibniz 法则。
...
设 \(k\) 为交换环。给定一族对象；对每两个对象 \(X,Y\)，给定 \(k\)-模上链复形 \(\mathcal E(X,Y)\)；给定双线性、保持总次数的复合，以及零次余圈 \(1_X\in\mathcal E(X,X)^0\)。若复合结合，\(1_X\) 是单位，并且对次数 \(p,q\) 的齐次态射 \(f:X\to Y\)、\(g:Y\to Z\) 满足 \(\mathrm d(gf)=(\mathrm dg)f+(-1)^qg(\mathrm df)\)，则称这些数据构成一个\term[weifenfencifanchou]{微分分次范畴}[differential graded category]，简称 DG 范畴。
