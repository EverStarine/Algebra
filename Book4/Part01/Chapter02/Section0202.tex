...
设 \(J\) 为小范畴，\(\mathcal C\) 为范畴。函子 \(D:J\rightarrow\mathcal C\) 称为一个 \(J\) 形\term[tubiao]{图表}[diagram]。以 \(T\in\mathcal C\) 为顶点的\term[zhui]{锥}[cone]是一族态射 \(\lambda_j:T\rightarrow D(j)\)，满足对每个 \(\alpha:j\rightarrow k\)，
\[
D(\alpha)\lambda_j=\lambda_k.
\]
若 \(\lambda_j:T\rightarrow D(j)\)、\(\lambda_j':T'\rightarrow D(j)\) 是两个锥，它们之间的态射为满足 \(\lambda_j'h=\lambda_j\) 的 \(h:T\rightarrow T'\)。

\ProseParagraphBreak
以 \(T\) 为顶点的\term[yuzhui]{余锥}[cocone]是一族态射 \(\mu_j:D(j)\rightarrow T\)，满足 \(\mu_kD(\alpha)=\mu_j\)。余锥之间的态射 \(h:T\rightarrow T'\) 满足 \(h\mu_j=\mu_j'\)。
\end{definition}
...
设 \(J\) 为小范畴，\(D:J\rightarrow\mathcal C\) 为图表。该图表的锥范畴中的终对象称为 \(D\) 的\term[jixian]{极限}[limit]，记为 \((\lim_JD,(\pi_j))\)。具体地，每个锥 \((T,(\lambda_j))\) 唯一来自一个态射 \(h:T\rightarrow\lim_JD\)，使 \(\pi_jh=\lambda_j\)。
...
设 \(J\) 为小范畴，\(D,D':J\rightarrow\mathcal C\) 为图表。一个图表的任意两个极限之间存在唯一的保持结构映射的同构；余极限也如此。若 \(D,D'\) 的极限存在，则自然变换 \(\eta:D\Rightarrow D'\) 唯一诱导态射
\[
...
设 \(J\) 为小范畴，\(D:J\rightarrow\mathcal C\) 为图表。假设下列两个积和所需的等化子存在：
\[
...
\]
其中在 \(\alpha\) 对应的余积项上，两个态射分别为 \(\iota_kD(\alpha)\) 与 \(\iota_j\)。
\end{theorem}
\begin{proof}
...
具有所有小积及等化子的范畴是完备的；具有所有小余积及余等化子的范畴是余完备的。把“小”改为“有限”，结论仍成立。因而 \(\mathbf{Set}\)、\(\mathbf{Grp}\)、\(\mathbf{CRing}\) 和 \(R\text{-}\mathbf{Mod}\) 均具有所有小极限；其中 \(\mathbf{Set}\) 与 \(R\text{-}\mathbf{Mod}\) 也具有所有小余极限。
\end{corollary}
\begin{proof}
