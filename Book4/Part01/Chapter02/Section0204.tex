...
设 \(F:\mathcal C\rightarrow\mathcal D\) 为函子，\(J\) 为小范畴，\(D:J\rightarrow\mathcal C\) 为有极限的图表。若 \(F\) 把其极限锥送到 \(FD\) 的极限锥，则称 \(F\) 保持这个极限。当 \(\lim_JFD\) 也已选定时，这等价于由锥 \(F(\pi_j)\) 诱导的比较映射
\[
...
保持所有已经存在的小极限。具体地，若 \(D:J\rightarrow\mathcal C\) 有极限，则
\[
\mathcal C(X,\lim_JD)
\ \cong\
...
设 \(I,J\) 为小范畴，\(D:I\times J\rightarrow\mathcal C\) 为图表。若 \(\mathcal C\) 具有所需的小极限，则存在保持全部结构映射的自然同构
\[
...
设 \(R,S\) 为含幺环。若函子 \(F:R\text{-}\mathbf{Mod}\rightarrow S\text{-}\mathbf{Mod}\) 在每个 Hom 群上保持加法，则称 \(F\) 为加性函子。若加性函子 \(F\) 把每个短正合列送到短正合列，则称它为正合函子。
\end{definition}
