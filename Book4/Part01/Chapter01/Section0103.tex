...
设 \(F:\mathcal C\rightarrow\mathcal D\) 为全忠实函子。若态射 \(f:X\rightarrow Y\) 的像 \(F(f)\) 可逆，则 \(f\) 可逆；若对象 \(FX,FY\) 同构，则 \(X,Y\) 同构。
\end{proposition}
\begin{proof}
...
设 \(F:\mathcal C\rightarrow\mathcal D\) 为函子。若存在函子
...
若改以 \(p:A\rightarrow X\) 为对象，以满足 \(hp=q\) 的 \(h:X\rightarrow Y\) 为态射，则得到余切片范畴 \(A/\mathcal C\)。
\end{definition}
...
以上数据确实组成范畴，向 \(\mathcal A,\mathcal B\) 的两个投影是函子。若
