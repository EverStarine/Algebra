...
模的第一同构定理把源端的商与目标端的像联系起来。交换范畴要求这种联系对每个态射成立，因此比只有态射加法的范畴具有更强的正合结构。
...
设 \(\mathcal A\) 是具有全部核与余核的加性范畴。对 \(\mathcal A\) 中的态射 \(f:X\to Y\)，记其核态射为 \(k_f:K_f\to X\)、余核态射为 \(q_f:Y\to Q_f\)，定义
\[
\operatorname{im}f=\ker q_f,\qquad
\operatorname{coim}f=\operatorname{coker}k_f,
\]
分别称为\term[xiang]{像}[image]与\term[yuxiang]{余像}[coimage]。记它们的结构态射为 \(i_f:\operatorname{im}f\to Y\)、\(p_f:X\to\operatorname{coim}f\)，以 \(\bar f:\operatorname{coim}f\to\operatorname{im}f\) 表示唯一满足 \(i_f\bar f p_f=f\) 的典范态射。若对每个态射 \(f\)，此 \(\bar f\) 都是同构，则称 \(\mathcal A\) 为\term[jiaohuan-fanchou]{交换范畴}[abelian category]，亦称 Abel 范畴。
\end{definition}
...
设 \(\mathcal A\) 为交换范畴，\(X,Y,Z\) 为其对象，\(f:X\to Y\)、\(g:Y\to Z\) 为其态射。若 \(gf=0\)，且 \(\operatorname{im}f\) 与 \(\ker g\) 是 \(Y\) 的同一个子对象，则称序列 \(X\xrightarrow fY\xrightarrow gZ\) 在 \(Y\) 处\term[zhenghe]{正合}[exact]。这里子对象相同，是指二者之间有与嵌入 \(Y\) 相容的同构。序列
\[
