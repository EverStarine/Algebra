...
\ProseParagraphBreak
保持极限是一项必要条件，并非任意情况下都足以推出存在左伴随。证明存在伴随还要为每个对象构造相应的表示对象，或给出足以保证存在性的额外条件。\Cref{b4:thm:A-general-adjoint-functor}会在完备、局部小的范畴上加上解集条件，证明左伴随存在；这里仅使用已经构造出的伴随，不把保持极限单独作为充分条件。
...
\ProseParagraphBreak
单子记录可以复合的运算，而由伴随得到它时，另一范畴的全部信息未必都被记录。\Cref{b4:thm:A-em-adjunction}构造单子代数的自由与遗忘伴随；\cref{b4:thm:A-beck-monadicity}再用保守性和分裂余等化子的保持性，判断原范畴何时能从这些代数恢复。这是另外的构造与判据，并非伴随本身已经保证的结论。
