...
\]
这一双射在 \(A\) 与 \(F\) 上自然：设 \(a:A\rightarrow B\) 为 \(\mathcal C\) 中的态射，\(h_B=\operatorname{Hom}_{\mathcal C}(-,B)\)，\(h_a:h_A\Rightarrow h_B\) 为后复合 \(a\) 得到的自然变换。若 \(G:\mathcal C^{\mathrm{op}}\rightarrow\mathbf{Set}\) 为函子，\(\alpha:h_B\Rightarrow F\)、\(\beta:F\Rightarrow G\) 为自然变换，则
\[
\begin{aligned}
