...
设 \(\mathcal C\) 为局部小范畴。若函子 \(F:\mathcal C\rightarrow\mathbf{Set}\) 存在对象 \(A\in\mathcal C\) 和自然同构 \(\theta:h^A\xrightarrow{\sim}F\)，则称 \(F\) 为\term[kebiao-hanzi]{可表函子}[representable functor]，称 \((A,\theta)\) 为 \(F\) 的一个表示，\(A\) 为表示对象。对于 \(F:\mathcal C^{\mathrm{op}}\rightarrow\mathbf{Set}\)，相应要求为自然同构 \(\theta:h_A\xrightarrow{\sim}F\)。
\end{definition}
...
设 \(\mathcal C\) 为局部小范畴，\(F:\mathcal C\rightarrow\mathbf{Set}\) 为函子。给定 \(A\in\mathcal C\) 及 \(u\in F(A)\)，公式
\[
...
设 \(\mathcal C\) 为局部小范畴，\(F:\mathcal C\rightarrow\mathbf{Set}\) 为函子。若 \((A,u)\)、\((B,v)\) 都满足\cref{b4:prop:universal-element}中的存在唯一性，则有唯一同构 \(s:A\rightarrow B\)，使 \(F(s)(u)=v\)。对于反变函子，亦有唯一同构 \(s:A\rightarrow B\)，使 \(F(s)(v)=u\)。
\end{proposition}
\begin{proof}
