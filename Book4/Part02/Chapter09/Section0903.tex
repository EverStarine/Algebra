...
设 \(R\) 为主理想整环，\(C\) 为自由 \(R\) 模链复形，\(A\) 为 \(R\) 模。令上链复形 \(\operatorname{Hom}_R(C,A)^n=\operatorname{Hom}_R(C_n,A)\)，微分为 \(f\mapsto f\mathrm{d}_{C,n+1}\)。则对每个整数 \(n\)，有自然的短正合列
\[
...
在 \(\mathbb Z\) 上取 \(C_1=\mathbb Za\oplus\mathbb Zb\)、\(C_0=\mathbb Zc\)，\(\mathrm{d}(a)=2c\)、\(\mathrm{d}(b)=0\)，其余项为零。取系数 \(A=\mathbb F_2\)。链自同构
\[
