...
本节及后两节用链下标书写同调公式：\(C_n=C^{-n}\)，微分为 \(d:C_n\to C_{n-1}\)。这只是第五章上链约定的重新编号。对张量积，\((-1)^{-n}=(-1)^n\)，所以符号规则保持不变。

\ProseParagraphBreak
对于右 \(R\)-模链复形 \(C\) 与左 \(R\)-模链复形 \(D\)，\cref{b4:def:tensor-complex}的张量复形在次数 \(n\) 为
\[
 (C\otimes_R D)_n=\bigoplus_{p+q=n}C_p\otimes_R D_q,
 \qquad d(c\otimes e)=d_Cc\otimes e+(-1)^pc\otimes d_De.
\]
这里一般只得到交换群复形；若 \(R\) 交换，才有自然的 \(R\)-模结构。\cref{b4:def:tensor-complex}已经核验两个混合项相消，因此 \(d^2=0\)。直和意味着一个元素只涉及有限多个 \((p,q)\)，不能无说明改成乘积。

\begin{proposition}\label{b4:prop:tensor-homology-map}
设 \(R\) 为环，\(C\) 为右 \(R\)-模链复形，\(D\) 为左 \(R\)-模链复形。圈的张量积诱导自然同态
\[
...
\end{proposition}
\begin{proof}
若 \(c,e\) 都是圈，则微分公式说明 \(c\otimes e\) 是圈。把 \(c\) 换成 \(c+d_Cu\)，差为 \(d(u\otimes e)\)；把 \(e\) 换成 \(e+d_Dv\)，差为 \((-1)^p d(c\otimes v)\)。所以对同调类良定义。双加性和 \(R\)-平衡性来自张量积，故诱导所写同态。链映射保持这些代表元公式，给出自然性。
\end{proof}
...
设 \(R\) 为环，\(C\) 是下有界的平坦右 \(R\)-模链复形，\(f:D\to E\) 是下有界左 \(R\)-模链复形之间的拟同构。则
...
\end{proposition}
\begin{proof}
写 \(Z_n=\ker d_n\)、\(B_n=\operatorname{im}d_{n+1}\)。把两列
\[
