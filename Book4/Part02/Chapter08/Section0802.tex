...
设 \(R\) 是含幺环，\(M,N\) 是左 \(R\) 模，\(P_\bullet\to M\) 为投射消解，\(N\to I^\bullet\) 为内射消解。取
\[
\operatorname{Hom}_R(P_\bullet,N)^n=\operatorname{Hom}_R(P_n,N),
\qquad \delta^n(f)=f\,d_{n+1}.
\]
则有自然同构
\[
...
\[
d_h(f)=f d_P,\qquad d_v(f)=(-1)^p d_I f.
\]
两次水平或竖直微分为零；一水平一竖直的两种复合相差一个负号，故 \(d_h d_v+d_vd_h=0\)。总次数为 \(p+q\)，每条对角线有限，因而总复形只用有限直和。
...
为了方便表示上同调类，本定理使用无符号的 \(\delta(f)=fd\)。若把 \(P_n\) 放在上链次数 \(-n\)，按一般内部 Hom 规则会得到 \(\delta(f)=(-1)^{n+1}fd\)；每个次数 \(n\) 乘 \((-1)^{n(n+1)/2}\) 给出这两种复形的同构。本节以后用前一种计算约定。
