...
删去两个基向量的两种次序符号相反，且 \(x_ix_j=x_jx_i\)，所以 \(\mathrm{d}^2=0\)。外幂及楔积的定义与基已在第二卷第九章建立。第三卷定理15.3.2已把正则列的 Koszul 消解作为交换代数中的主要结果处理；下面给出基于本卷同调长正合列的第二证明，只证明消解所需的方向，不在这里重做第三卷的深度理论。

\begin{proposition}\label{b4:prop:koszul-resolution}
设 \(R\) 是交换含幺环，\(r\ge0\) 为整数，\(x_1,\ldots,x_r\in R\)。假设 \((x_1,\ldots,x_r)\ne R\)，且对每个 \(1\le i\le r\)，乘 \(x_i\) 在 \(R/(x_1,\ldots,x_{i-1})\) 上单射。则 \(R\) 上的 Koszul 复形 \(K_\bullet(x_1,\ldots,x_r;R)\) 连同零次增广 \(R\to R/(x_1,\ldots,x_r)\) 是 \(R/(x_1,\ldots,x_r)\) 的有限自由消解。
\end{proposition}
\begin{proof}
...
\]
故有短正合复形列，左端为 \(K'\)，商复形在次数 \(n\) 为 \(K'_{n-1}\)，微分为 \(\mathrm{d}'\)。其连接态射在同调上是乘 \(x_r\)，差至多为显式的符号 \((-1)^{n-1}\)。归纳假设使 \(K'\) 只在零次有同调。因此长正合列给出 \(H_n(K)=0\) 对 \(n\ge2\)，并给出
\[
...
\]
两次删除的项成对抵消，故 \(\mathrm{d}^2=0\)。在底层 \(k\) 模上令 \(s_n(g_0,\ldots,g_n)=(1,g_0,\ldots,g_n)\)、\(s_{-1}(1)=(1)\)，则 \(\mathrm{d}s+s\mathrm{d}=1\)，因为删除新插入的首项给出原元组，其余项与 \(s\mathrm{d}\) 抵消。因此增广复形正合，是平凡左 \(kG\) 模 \(k\) 的自由消解。\(s\) 通常不具 \(G\) 等变性；这不妨碍正合，却不证明它作为 \(kG\) 复形可缩。
\end{example}
