...
选定逐次双积表示以后，微分写成
\[
d_n^B=\begin{pmatrix}d_n^A&h_n\\0&d_n^C\end{pmatrix},
\qquad d_{n-1}^Ah_n+h_{n-1}d_n^C=0.
\]
第二个等式就是 \((d^B)^2=0\) 的非对角分量，显示原来的扩张如何影响消解。它也解释为什么逐次分裂并不推出短正合列作为复形分裂。

\subsection{导出长正合列的构造}
\label{b4:subsec:0703:03}

\begin{proposition}\label{b4:prop:horseshoe-functor-sequence}
设 \(F:\mathcal A\to\mathcal B\) 是交换范畴间的加性函子。\cref{b4:thm:horseshoe}中的消解短正合列经 \(F\) 作用后仍逐次短正合。因此其同调对象有自然连接态射及长正合列。
\end{proposition}
\begin{proof}
...
\[
d_n^{Q^A}y_n=r_n,
\qquad r_n=u_{n-1}h_n+y_{n-1}d_n^{P^C}-h'_nv_n.
\]
其余三块由 \(u,v\) 是链映射及零块自动满足。将 \(d_{n-1}^{Q^A}\) 作用于 \(r_n\)，利用 \(d^Ah+hd^C=0\)、\(d^{Q^A}h'+h'd^{Q^C}=0\)、\(u,v\) 的链等式及上一次数的 \(y\) 等式，三组项逐对抵消，结果为零；\(n=1\) 时将该微分换成左端增广，用已满足的零次增广等式，结论相同。因此 \(r_n\) 落入目标的核，正合性与 \(P_n^C\) 投射使它提升为 \(y_n\)。归纳给出消解短正合列之间的严格交换图。
