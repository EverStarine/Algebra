...
两个方向各有一个微分时，必须确定它们反交换的符号以及每条对角线怎样求和。无界情形中，直和总化与直积总化可能产生不同的同调。
...
设 \(E\) 是交换范畴中的双复形，每条对角线仅有有限个非零项。若每个纵列 \((E^{p,\bullet},\mathrm{d}_v)\) 正合，则 \(\operatorname{Tot}(E)\) 正合。交换横纵两个方向后，同一结论成立。
\end{lemma}
\begin{proof}
...
\[
\mathrm{d}_v(a_p)=b_p,\qquad \mathrm{d}_h(a_p)=-b_{p+1},\qquad
 \mathrm{d}_v(b_p)=\mathrm{d}_h(b_p)=0.
\]
