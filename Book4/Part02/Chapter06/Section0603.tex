...
设 \(\mathcal A\) 是交换范畴，\(f:C\to D\) 是复形映射。则 \(f\) 是拟同构，当且仅当 \(\operatorname{Cone}(f)\) 无同调。可复合映射 \(f,g\) 中，若 \(f,g,gf\) 的任意两个为拟同构，则第三个也是拟同构。
\end{proposition}
\begin{proof}
锥的长正合列中有
\[
...
令 \(i:H(C)\to C\) 为中间分量嵌入，\(p:C\to H(C)\) 为中间投影，则 \(pi=1\)。令 \(h^n\) 在 \(B^n\) 上为 \(d:W^{n-1}\to B^n\) 的逆，在另外两个分量上为零，就有 \(dh+hd=1-ip\)。这给出所需同伦等价。
