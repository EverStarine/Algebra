% !TeX root = ../../Book4.tex
% 纲要标题：### 11.4 杯积、群扩张与连续上同调
% 纲要位置：计划纲要.md，第 2761 行。

\section{杯积、群扩张与连续上同调}
\label{b4:sec:1104}

系数乘法使不同次数的上同调类可以相乘，群扩张又把两个群的上同调联系起来。连续余链的例子提醒我们：改变群的拓扑以后，允许使用的余链也必须改变。
...
\ProseParagraphBreak
连续群上同调也可表为离散连续 \(G\) 模范畴中不变量函子的右导出函子。\Cref{b4:thm:C-continuous-derived}会构造这个范畴的内射嵌入，并从有限商公式证明导出识别。这个范畴与全部抽象 \(\mathbb ZG\) 模的范畴不同；若用 Ext 表示，必须标明所在范畴。文献中的对应说明见 Weibel\cite[Definition 6.11.8, Lemma 6.11.10, Profinite Cohomology 6.11.11, Theorem 6.11.13]{Weibel1994HomologicalAlgebra}。更具体的投射循环群计算见\cref{b4:prop:C-procyclic-h1}和\cref{b4:prop:C-procyclic-h2}，其中保留了有限系数和膨胀映射的条件。系数若带有非离散拓扑，例如 \(p\)-进系数模，还需要另行规定连续上同调的范畴与极限条件，不能直接套用本节结论。
