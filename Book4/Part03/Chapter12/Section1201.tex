...
设 \(A\) 为结合含幺 \(k\) 代数。其反代数 \(A^{\mathrm{op}}\) 有相同的向量空间，而乘法次序反转。代数
\[
...
设 \(A\) 为结合含幺 \(k\) 代数，\(M\) 为 \(A\) 双模，\(n\ge0\)。把 \(A\) 本身看作左 \(A^e\) 模，定义
\[
...
\]
其中 \(d_n\) 在 \(n\ge1\) 时使用；增广 \(\varepsilon:B_0(A)\to A\) 为乘法。左右 \(A\) 作用分别放在最外面的两个因子上。

\begin{proposition}\label{b4:prop:algebra-bar-resolution}
设 \(A\) 为结合含幺 \(k\) 代数。以上增广链复形 \(B_\bullet(A)\to A\) 是左 \(A^e\) 模的自由消解，称为 \(A\) 的双模 bar 消解。
\end{proposition}
\begin{proof}
两次合并不相邻因子时，两种次序得到同一张量而符号相反；合并三个连续因子时，结合律使两项相等而符号相反。因此 \(d^2=0\)，增广与微分也相容。
...
\]
\(d_{n+1}s_n\) 的首项是原张量；其余合并项与 \(s_{n-1}d_n\) 逐项抵消，所以 \(ds+sd=1\)，增广处为 \(\varepsilon s_{-1}=1\)。故底层向量空间复形正合，也即双模复形正合。这里的 \(s_n\) 通常不是双模同态；底层可缩保证正合，并没有声称 \(A\) 是投射双模。
\end{proof}
