...
第零页只看伴随分次，后续微分则测量代表元还能向深层提升多少次。正合偶提供统一构造，并保证每一新页由前一页取同调得到。
...
\]
若 \(x\) 代表此页的类，便有 \(\mathrm{d}x\in F^{p+1}\)。再取 \(\mathrm{d}x\) 在下一层的类，就得到 \(\mathrm{d}_1[x]\)。例如两项复形 \(\mathbb Z\xrightarrow{2}\mathbb Z\) 的源放在滤过层零、靶放在滤过层一，第零页微分为零，第一页才出现乘 \(2\) 的映射。
...
\]
故 \(jk\) 正是 \(\mathrm{d}_1\)。反复导出正合偶时，每次把 \(D\) 替成包含映射的像；表示连接像所需的提升每次多经过一层滤过，得到次数 \((r,1-r)\) 的 \(\mathrm{d}_r\)。这些页与\cref{b4:eq:spectral-page-formula}一致：连接同态以 \(\mathrm{d}x\) 表示，而提升到第 \(r\) 层正要求 \(\mathrm{d}x\in F^{p+r}\)；更换提升产生的两类差正是该式分母。下一小节直接核对页间关系，因而不把正合偶记号当作省略计算的理由。
