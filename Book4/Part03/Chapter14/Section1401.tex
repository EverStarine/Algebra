...
设 \((C,F)\) 是模的递减滤过复形。若对每个总次数 \(n\)，有
\[
 \bigcup_pF^pC^n=C^n,\qquad \bigcap_pF^pC^n=0,
\]
分别称滤过穷尽、分离。若对每个 \(n\) 存在整数 \(a_n<b_n\)，使 \(F^{a_n}C^n=C^n\)、\(F^{b_n}C^n=0\)，则称滤过逐度有限。若 \(a_n,b_n\) 可独立于 \(n\) 选取，则称滤过一致有界。
\end{definition}
...
总次数写作 \(n=p+q\)，其中 \(p\) 是滤过次数，\(q\) 只是补足总次数的第二指标。一个 \(x\in F^pC^{p+q}\) 若满足 \(dx\in F^{p+r}C^{p+q+1}\)，便表示它的微分直到相隔 \(r\) 层才可能被看见。于是第 \(r\) 次出现的微分必须具有方向
\[
...
设 \(R\) 为含幺环。一个从第零页开始的上同调型\term[pu-xulie]{谱序列}[spectral sequence]，由双分次左 \(R\)-模 \(E_r^{p,q}\)、上述次数的微分 \(d_r\) 及指定同构
\[
 d_r^2=0,\qquad E_{r+1}^{p,q}\simeq
 \frac{\ker(d_r:E_r^{p,q}\to E_r^{p+r,q-r+1})}
 {\operatorname{im}(d_r:E_r^{p-r,q+r-1}\to E_r^{p,q})}
\]
组成。也可从某个较晚页开始给出这些数据。谱序列态射在各页保持双次数、与微分及指定同构相容。
\end{definition}
\symidx{\(E_r^{p,q},d_r\)：谱序列的页与微分}
