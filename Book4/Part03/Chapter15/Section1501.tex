...
\]
给定总次数，非零项有限，两种滤过逐度有限。对 \(F\)，伴随分次第零微分为 \(\mathrm{d}_v\)，故第一页是 \(H_v^q(D^{p,\bullet})\)。反交换关系保证 \(\mathrm{d}_h\) 把纵余圈映为纵余圈、把纵余边映为纵余边，诱导第一页微分；再取同调即为第一条公式。对 \(G\)，第零微分为 \(\mathrm{d}_h\)，第一页微分由 \(\mathrm{d}_v\) 诱导，给出第二条。两种微分在交换指标后仍满足反交换关系，不须另添符号。\cref{b4:thm:finite-filtration-convergence}分别给出强收敛和自然性。
\end{proof}
