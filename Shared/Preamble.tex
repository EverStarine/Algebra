...
\DeclareSymbolFont{AlgebraAMSb}{U}{msb}{m}{n}
\DeclareMathSymbol{\AlgebraRoundEmptySet}{\mathord}{AlgebraAMSb}{"3F}
\AtBeginDocument{%
  \let\varnothing\AlgebraRoundEmptySet
  \let\emptyset\AlgebraRoundEmptySet
}
...
\newif\ifAlgebraHasBibliography
\AtDataInput{\global\AlgebraHasBibliographytrue}
\NewDocumentCommand{\PrintBookBibliography}{}{%
  \ifAlgebraHasBibliography
...
\definecolor{AlgebraBlue}{HTML}{244A64}
\definecolor{AlgebraBlueLight}{HTML}{EEF4F7}
\definecolor{AlgebraGray}{HTML}{4F5962}
\definecolor{AlgebraGrayLight}{HTML}{F4F5F6}

\ctexset{
  part = {
    % 编题页分两行：第一行“第X编”，与封面“第一卷”同款（大号无衬线灰色）；
    % 第二行编名，与封面“代数学”同款（大号无衬线蓝色粗体），字号保持原样。
    % 第一行与行距由 number／aftername 给出，format 只负责编名的字体，
    % 以免字号被重置或被重复输出。
    format = {\centering\fontsize{30}{38}\selectfont\sffamily\bfseries\color{AlgebraBlue}},
    name = {,},
    number = {\centering\normalfont\normalsize{\fontsize{24}{30}\selectfont\sffamily\color{AlgebraGray}第\chinese{part}编}},
  ...
  chapter = {
    format = \raggedright\Huge\bfseries\color{AlgebraBlue},
    name = {第,章},
  ...
  section = {
    format = \Large\bfseries\color{AlgebraBlue},
    beforeskip = 2.4ex plus .6ex minus .2ex,
    afterskip = 1.1ex plus .2ex
  },
  subsection = {
    format = \large\bfseries\color{AlgebraGray},
...
\newlength{\AlgebraProseParagraphGap}
\AtBeginDocument{%
  \setlength{\AlgebraProseParagraphGap}{0.40\baselineskip}%
}
\NewDocumentCommand{\ProseParagraphBreak}{}{%
  \par\addvspace{\AlgebraProseParagraphGap}%
}
...
\pagestyle{fancy}
\fancyhf{}
\fancyhead[LE]{\small\color{AlgebraGray}\thepage\quad\leftmark}
% 奇数页采用页前最后有效的节标记，避免页中开始新节时提前更换页眉。
% 新节或习题的标记从后续页生效；章首页使用 plain，不显示上一章的标记。
\fancyhead[RO]{\small\color{AlgebraGray}\TopMark{2e-right}\quad\thepage}
...
\fancypagestyle{plain}{
  \fancyhf{}
  \fancyfoot[C]{\small\color{AlgebraGray}\thepage}
  \renewcommand{\headrulewidth}{0pt}
}

\usepackage[unicode,colorlinks=true,linkcolor=AlgebraBlue,
  citecolor=AlgebraBlue,urlcolor=AlgebraBlue,pdfborder={0 0 0}]{hyperref}
\usepackage{bookmark}
...
% 又使写入和回读的标签文本稳定。
\newcommand{\AlgebraPdfAlpha}{α}
\newcommand{\AlgebraPdfGamma}{Γ}
\newcommand{\AlgebraPdfLambda}{λ}
\newcommand{\AlgebraPdfMu}{μ}
\newcommand{\AlgebraPdfPi}{π}
\newcommand{\AlgebraPdfSigma}{σ}
\newcommand{\AlgebraPdfTheta}{θ}
\newcommand{\AlgebraPdfXi}{ξ}
\newcommand{\AlgebraPdfZeta}{ζ}
...
\newtheoremstyle{algebraplain}
  {1.2ex}{1.2ex}{\normalfont\upshape\kaishu}{}
  {\color{AlgebraBlue}\bfseries}{.}{0.6em}{}
\newtheoremstyle{algebradefinition}
  {1.2ex}{1.2ex}{\normalfont\kaishu}{}
  {\color{AlgebraBlue}\bfseries}{.}{0.6em}{}
\newtheoremstyle{algebraremark}
  {1.0ex}{1.0ex}{\normalfont\kaishu}{}
  {\color{AlgebraGray}\bfseries}{.}{0.6em}{}

\theoremstyle{algebraplain}
...
\newcommand{\AlgebraOptionalMark}{\texorpdfstring{\textsuperscript{*}}{*}}
% 选做习题沿用同一编号星号。可选参数仍只表示习题自身的简短题名，
% 不得再写“选读”或“选做”；星号由本环境自动附在题号右侧。
\NewDocumentEnvironment{optionalexercise}{o}{%
  \begingroup
  \let\AlgebraSavedTheExercise\theexercise
  \renewcommand{\theexercise}{\AlgebraSavedTheExercise\AlgebraOptionalMark}%
...
\NewDocumentCommand{\OptionalChapter}{O{}mm}{%
  \begingroup
  \let\AlgebraSavedTheChapter\thechapter
  \renewcommand{\thechapter}{\AlgebraSavedTheChapter\AlgebraOptionalMark}%
  \ctexset{chapter={name={第,章\AlgebraOptionalMark},
...
\NewDocumentCommand{\OptionalSection}{mm}{%
  \begingroup
  \let\AlgebraSavedTheSection\thesection
  \renewcommand{\thesection}{\AlgebraSavedTheSection\AlgebraOptionalMark}%
...
\NewDocumentCommand{\OptionalSubsection}{mm}{%
  \begingroup
  \let\AlgebraSavedTheSubsection\thesubsection
  \renewcommand{\thesubsection}{\AlgebraSavedTheSubsection\AlgebraOptionalMark}%
...
\NewDocumentCommand{\BookPart}{mm}{%
  \cleardoublepage
  \begingroup
  \def\AlgebraPartLabel{#2}%
  % \part 会在返回前翻页，故须在 \@endpart 开始处记录题页标签。
  \pretocmd{\@endpart}{\label{\AlgebraPartLabel}}{}%
    {\PackageError{algebra}{无法为编题页写入标签}{请检查当前文档类的 \string\@endpart 定义。}}%
  \part{#1}%
  \endgroup
  \thispagestyle{empty}%
  \null
...
\NewDocumentCommand{\BookPartGuide}{O{本编导读}}{%
  \chapter*{#1}%
  \pdfbookmark[1]{#1}{AlgebraPartGuide@\thepage}%
}
...
\NewDocumentCommand{\BookAppendixPart}{m}{%
  \cleardoublepage
  \begingroup
  \def\AlgebraPartLabel{#1}%
  % 在 part* 的题页尚未翻走时建立唯一的目录与书签目标。
  \pretocmd{\@endpart}{%
    \bookmarksetup{startatroot}%
    \phantomsection
    \addcontentsline{toc}{part}{附录}%
    \label{\AlgebraPartLabel}%
  }{}{\PackageError{algebra}{无法为附录编题页写入目录与标签}{请检查当前文档类的 \string\@endpart 定义。}}%
  \part*{附录}%
  \endgroup
  \thispagestyle{empty}%
  \null
...
\makeatletter
\zref@newprop{algebrachapternum}{}
% 在 \chapter 之后调用，参数即该章标签；把本章汉字号存为 zref 属性
\NewDocumentCommand{\BookChapterNumber}{m}{%
  \zref@setcurrent{algebrachapternum}{\chinese{chapter}}%
  \zref@labelbyprops{zch@#1}{algebrachapternum}%
}
% 取出某章标签对应的汉字号（\zref@extract 可展开，能在 \edef 中安全使用）
\NewDocumentCommand{\AlgebraChineseChapterOf}{m}{%
  \zref@extract{zch@#1}{algebrachapternum}}
\makeatother
...
\NewDocumentCommand{\chapref}{m}{%
  ...
  \edef\AlgebraChapterDisp{第\AlgebraChineseChapterOf{#1}章}%
  \hyperref[#1]{\AlgebraChapterDisp}%
  \endgroup}
...
\newtcolorbox{learninggoals}{
  enhanced,breakable,colback=AlgebraBlueLight,colframe=AlgebraBlue,
  boxrule=.45pt,arc=1mm,left=1.2mm,right=1.2mm,top=1mm,bottom=1mm,
  title=本章目标,fonttitle=\bfseries
}
\newtcolorbox{partguide}{
  enhanced,breakable,colback=AlgebraGrayLight,colframe=AlgebraGray,
...
% 索引采用稍小字号与自然栏底，避免少量词条被挤到单独一页。
\indexsetup{othercode={\small\raggedcolumns}}
...
\NewDocumentCommand{\term}{s O{} m o}{%
  \IfBooleanT{#1}{%
    \PackageError{algebra}{\string\term* 已取消}{请改用 \string\term，并保留末尾的原文注解。}%
  }%
  {\songti\textbf{\textup{#3}}}%
  \IfNoValueTF{#4}{%
    \PackageError{algebra}{术语缺少原文注解}{每个术语首次正式定义或引入时都必须写成 \string\term[排序键]{中文主称}[原文]。}%
  }{%
    \textnormal{(\textup{#4})}%
  }%
...
\newcounter{algebraSymbolIndexOrder}
\NewDocumentCommand{\symidx}{m}{%
  \stepcounter{algebraSymbolIndexOrder}%
  \edef\AlgebraSymbolSortKey{%
    \number\numexpr10000+\value{algebraSymbolIndexOrder}\relax}%
  \index[symbols]{\AlgebraSymbolSortKey@#1}%
}
...
% 正规闭包的双尖括号在正文与符号索引中采用同一间距。
\NewDocumentCommand{\NormalClosure}{m}{\langle\!\langle #1\rangle\!\rangle}
...
\NewCommandCopy{\AlgebraOriginalCref}{\cref}
\NewCommandCopy{\AlgebraOriginalCrefCapital}{\Cref}
\ExplSyntaxOn
\cs_new_protected:Npn \algebra_cref_maybe_cjk_glue:
  { \peek_after:Nw \algebra_cref_maybe_cjk_glue_aux: }
\cs_new_protected:Npn \algebra_cref_maybe_cjk_glue_aux:
  {
...
\RenewDocumentCommand{\cref}{s m}
  {
    \IfBooleanTF{#1}
      { \AlgebraOriginalCref*{#2} }
      { \AlgebraOriginalCref{#2} }
    \algebra_cref_maybe_cjk_glue:
  }
\RenewDocumentCommand{\Cref}{s m}
  {
    \IfBooleanTF{#1}
      { \AlgebraOriginalCrefCapital*{#2} }
      { \AlgebraOriginalCrefCapital{#2} }
    \algebra_cref_maybe_cjk_glue:
  }
...
\newcommand{\MakeBookTitle}[1][封面]{%
  ...
  \bookmarksetup{startatroot}%
  \pdfbookmark[0]{#1}{AlgebraCover}%
  \begin{titlepage}
    \centering
    \vspace*{2.2cm}
    {\fontsize{36}{46}\selectfont\sffamily 代数学\par}
    \vspace{0.7cm}
    {\Large\sffamily\color{AlgebraGray}\BookVolumeName\par}
    \vspace{1.0cm}
    {\fontsize{24}{32}\selectfont\bfseries\color{AlgebraBlue}\BookTitle\par}
    \vfill
...
\newcommand{\MakeBookContents}{%
  \cleardoublepage
  \bookmarksetup{startatroot}%
  \pdfbookmark[0]{目录}{AlgebraContents}%
  \tableofcontents
}
...
\newcommand{\MakeBookListOfTables}{%
  \cleardoublepage
  \bookmarksetup{startatroot}%
  \pdfbookmark[0]{表目录}{AlgebraListOfTables}%
  \begingroup
...
\makeatletter
\let\Algebra@orig@schapter\@schapter
\def\@schapter#1{%
  \markboth{#1}{#1}%
  \Algebra@orig@schapter{#1}%
}
% 目录按层级着色：编为黑色、小节为深灰；章与节保持默认链接蓝。
\pretocmd{\l@part}{\hypersetup{linkcolor=black}}{}{%
  \PackageWarning{algebra}{无法为目录编条目着色}}
\pretocmd{\l@chapter}{\hypersetup{linkcolor=AlgebraBlue}}{}{%
  \PackageWarning{algebra}{无法为目录章条目着色}}
\pretocmd{\l@section}{\hypersetup{linkcolor=AlgebraBlue}}{}{%
  \PackageWarning{algebra}{无法为目录节条目着色}}
\pretocmd{\l@subsection}{\hypersetup{linkcolor=AlgebraGray}}{}{%
  \PackageWarning{algebra}{无法为目录小节条目着色}}
% 编的目录条目与 PDF 书签须用纯文本“第X编”。ctex 的 \CTEX@part@tocline
% 取 \CTEXthepart，而该宏带上编题页的颜色与字体设置，会把格式代码写进目录，
% 并使书签出现 “=AlgebraGray第一编” 之类乱码。此覆盖须置于 \ctexset 之后，
