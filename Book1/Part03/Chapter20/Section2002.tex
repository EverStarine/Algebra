...
这两个定理将来可以用\term[yi-shangxunhuan]{一上循环}[1-cocycle]与\term[yi-shangbianjie]{一上边界}[1-coboundary]的语言统一。这里只说明对应的方程，不要求预先学习上同调。
...
\]
则称它为乘法\term[yi-shangxunhuan]{一上循环}[1-cocycle]。如果存在 \(b\in L^\times\)，使 \(c_g=b/g(b)\) 对每个 \(g\in G\) 都成立，则称这族元素为\term[yi-shangbianjie]{一上边界}[1-coboundary]。当 \(G=\langle\sigma\rangle\) 时，循环由 \(a=c_\sigma\) 决定：
\[
