...
循环扩张只由一个生成自同构控制，却已经包含两种重要的方程类型。本章先用 Hilbert 第90定理把范数为一的元素写成共轭比值，把迹为零的元素写成共轭差。随后在含有所需单位根时建立 Kummer 配对与有限扩张的对应；在特征 \(p\) 中则用 Artin--Schreier 方程描述循环 \(p\) 次扩张。这些结果为下一章把可解群转化为逐层求根提供工具。
\end{chapterabstract}
