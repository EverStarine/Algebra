...
从平面上 \((0,0)\)、\((1,0)\) 出发，有限次画过已知两点的直线、以已知点为圆心且以已知两点间距离为半径的圆，并取这些直线与圆的交点，能够得到的点称为\term[keguitu-dian]{尺规可作图点}[constructible point]。若 \((a,0)\) 可作图，则实数 \(a\) 称为\term[keguitu-shu]{可作图数}[constructible number]。
\end{definition}

标准的作垂线、平行线与转移线段长度均由上述操作实现。因此一个点可作图当且仅当它的两个坐标都是可作图数：从点向坐标轴作垂线得到坐标，反过来在两轴上标出坐标再作平行线，交点即为所需点。
...
\qquad [K_i:K_{i-1}]=2,
\]
使 \(a\in K_r\)。允许 \(r=0\)。特别地，可作图数在 \(\mathbb Q\) 上的次数必为 \(2\) 的幂。
\end{theorem}
\begin{proof}
...
设 \(n\geq3\)。正 \(n\) 边形可尺规作图，当且仅当 \(\varphi(n)\) 为 \(2\) 的幂；等价地，
\[
n=2^a p_1\cdots p_s,
\]
其中 \(a\geq0\)，\(p_i\) 是两两不同的\term[Fermat-sushu]{Fermat 素数}[Fermat prime]，即形如 \(2^{2^r}+1\) 的素数。
\end{theorem}
\begin{proof}
...
化圆为方要求作出与单位圆等面积的正方形，其边长必须为 \(\sqrt\pi\)。这里使用一个本卷不证明的外部结果：\term[Lindemann-yuanzhoulv-chaoyuexing]{Lindemann 的圆周率超越性定理}[Lindemann's theorem on the transcendence of pi]断言 \(\pi\) 不满足任何非零有理系数多项式。其原始论文为 Lindemann 的《Ueber die Zahl \(\pi\)》\cite{Lindemann1882Pi}。

\ProseParagraphBreak
若 \(\sqrt\pi\) 可作图，由\cref{b1:thm:constructible-quadratic-tower}，它在 \(\mathbb Q\) 上代数；平方后 \(\pi\) 也应代数，与所引定理矛盾。因此化圆为方不可能。代数在这里把作图问题归结为代数性，而 \(\pi\) 的超越性需要另外的理论；本节没有从 Galois 对应推出这一超越性结论。
