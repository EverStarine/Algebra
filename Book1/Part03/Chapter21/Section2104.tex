...
从平面上 \((0,0)\)、\((1,0)\) 出发，有限次画过已知两点的直线、以已知点为圆心且以已知两点间距离为半径的圆，并取这些直线与圆的交点，能够得到的点称为\term[keguitu-dian]{尺规可作图点}[constructible point]。若 \((a,0)\) 可作图，则实数 \(a\) 称为\term[guiju-shu]{规矩数}[constructible number]，也称可构造数、可作图数。\termidx[kegouzao-shu]{可构造数}\termidx[kezuotu-shu]{可作图数}
\end{definition}
...
化圆为方要求作出与单位圆等面积的正方形，其边长必须为 \(\sqrt\pi\)。所需的外部输入是\term[Lindemann-Weierstrass-dingli]{Lindemann--Weierstrass 定理}[Lindemann--Weierstrass theorem]的下述单变量推论；本卷不证明这一超越性结果。
