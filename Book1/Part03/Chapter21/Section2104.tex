...
化圆为方要求作出与单位圆等面积的正方形，其边长必须为 \(\sqrt\pi\)。这里要引用\term[Lindemann-Weierstrass-dingli]{Lindemann--Weierstrass 定理}[Lindemann--Weierstrass theorem]的下述单变量推论；本卷不证明这一超越性结果。

\begin{theorem}[Lindemann--Weierstrass 的单变量推论]\label{b1:thm:lindemann-weierstrass-special-case}
若 \(\alpha\) 是非零代数数，则 \(e^\alpha\) 是超越数。
\end{theorem}
这里仅列出尺规作图所需的特例；一般定理断言，互异代数数的指数在代数数域上线性无关。圆周率超越性的原始文献是 Lindemann 的《Ueber die Zahl \(\pi\)》\cite{Lindemann1882Pi}。
...
\end{proof}
代数在这里把作图问题归结为代数性，而所引超越性结果与复指数函数的 Euler 恒等式是独立输入；本节没有从 Galois 对应推出这些结论。
