...
证明将在\subsecref{b1:subsec:2102:04}给出。两个方向需要不同的工作：从可解群出发，应先加入必要的单位根，再把素数阶循环商转为开方；从根式塔出发，则应把所有共轭同时补进来，才能研究一个真正的 Galois 群。只检查原塔中每一步的次数，不能替代后一项工作。
...
\ProseParagraphBreak
在特征 \(p\) 中，若只允许指数与 \(p\) 互素的根式，前一引理仍可按同一方式构造可解的正规包络。但循环 \(p\) 次扩张由 \(X^p-X-a\) 产生，不能调用\cref{b1:thm:kummer-cyclic}把这一步变为 \(p\) 次开方。另一方面，\(X^p-a\) 的根给出纯不可分扩张，其自同构群可能为平凡群，却不能用有限 Galois 对应来分析。正特征下需要把 Kummer 步骤、Artin--Schreier 步骤和纯不可分步骤分别处理。

\subsection{特征零的证明与正特征情形}
\label{b1:subsec:2102:04}

\begin{proof}[\cref{b1:thm:radical-solvability}的证明]
...
这份证明也给出正特征下应如何修改构造的提示：若可解 Galois 群的某个素数阶因子等于特征，使用\cref{b1:thm:artin-schreier-classification}，而不是开方。得到的是由两类方程组成的塔；本书不把它与特征零定义下的纯根式塔混称。
