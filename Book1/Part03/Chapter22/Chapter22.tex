...
无限代数扩张仍由有限子扩张组成，但各有限层上的自同构必须相容。Galois 群可表示为有限商群的逆极限，Krull 拓扑正是相应的逆极限拓扑。本章就地建立所需拓扑与紧性，证明中间域和闭子群的对应，并以有限域的 Frobenius 自同构说明为什么取闭包不可省略。有限层检测还将给出连续表示及正规商群的具体判别。
\end{chapterabstract}
...
本章的对应定理主要使用有限 Galois 对应、一般代数扩张的嵌入延拓、正规性与可分性；读完\secref{b1:sec:1901}和\secref{b1:sec:1902}即可继续，不要求先学习根式可解性或尺规作图。有限域的 Frobenius 计算用于\secref{b1:sec:2204}的典型例子。拓扑术语会在使用前定义，不预设完整点集拓扑课程；任意指标下的紧性证明明确使用 Zorn 引理。实数域的例子使用\cref{b1:thm:fundamental-theorem-algebra}，其证明调用实分析的中间值定理及附录 E 的实闭域刻画；连续群上同调只在末尾说明后续方向。


\ProseParagraphBreak
本章可结合 Lang 的《Algebra》\cite[Chapter VI, \S 14]{Lang2002}学习无限 Galois 扩张。若希望进一步研究绝对 Galois 群及其连续上同调，可读 Gille 与 Szamuely 的《Central Simple Algebras and Galois Cohomology》\cite[Sections 4.1--4.2]{GilleSzamuely2017CentralSimpleAlgebras}；这需要在本章的拓扑群知识之外补充系数模和群上同调。有限群的线性表示可先从 Serre 的《Linear Representations of Finite Groups》\cite[Part I, Chapters 1--2, pp. 3--24]{Serre1977FiniteRepresentations}学起，先了解线性表示与特征标。
...
直接构造时，如果一个字在 \(T\) 中具有任意长的延长字，就称它可以无限延长。空字有此性质。若一个字有此性质，它的两个一步延长至少一个仍有此性质；否则两个分支的长度各有上界，取较大上界便产生矛盾。规定能选 \(0\) 分支时就选 \(0\)，否则选 \(1\)，递归产生所需无限序列。这个规则每步唯一确定选择，因而这里不需要调用正文处理任意指标积时所用的极大真滤子构造；两种证明的适用范围不同。
\end{solution}
