...
本章的对应定理主要使用有限 Galois 对应、一般代数扩张的嵌入延拓、正规性与可分性；读完\secref{b1:sec:1901}和\secref{b1:sec:1902}即可继续，不要求先学习根式可解性或尺规作图。有限域的 Frobenius 计算用于\secref{b1:sec:2204}的典型例子。拓扑术语会在使用前定义，不预设完整点集拓扑课程；任意指标下的紧性证明明确使用 Zorn 引理。实数域的例子还要引用本书未予证明的代数基本定理，连续群上同调只在末尾说明后续方向。
