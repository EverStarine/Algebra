...
设 \(G\) 为拓扑群，\(H\trianglelefteq G\) 为正规子群，\(q:G\rightarrow G/H\) 为自然映射。在抽象商群 \(G/H\) 上规定：集合 \(V\subseteq G/H\) 开，当且仅当 \(q^{-1}(V)\) 在 \(G\) 中开。由此得到的拓扑称为 \(G/H\) 上的\term[shang-tuopu]{商拓扑}[quotient topology]。本节应用时，\(G=\operatorname{Gal}(L/K)\)，\(H\) 还是闭子群，并令 \(E=L^H\)。
