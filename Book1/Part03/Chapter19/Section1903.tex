...
\[
\underbrace{L}_{\text{特征零分裂域}}
\supseteq\underbrace{\mathbb Z[\alpha_1,\ldots,\alpha_n]}_{A}
\longrightarrow\underbrace{A/\mathfrak m}_{\text{有限剩余域 }k}.
\]
先构造可约化的整环，再把有限域的 Frobenius 提升为 \(L\) 的自同构，最后比较两侧根的轨道；不能把模 \(p\) 的根直接当作特征零中的根。
...
\textbf{第三步：提升 Frobenius 并比较根的轨道。}
