...
有限 Galois 理论把中间域结构转化为有限群的子群结构。本章先从正规性和可分性辨认自同构何时达到扩张次数，再独立证明 Artin 不动域定理。精确的次数等式使不动域与固定群互为逆运算，得到 Galois 对应及正规子群的商群解释。二次、三次和四次方程提供可以逐个写出自同构的例子；中间域枚举、合成域与交域的计算，则说明如何应用这一对应。
\end{chapterabstract}
...
本章用到的域论结果包括\cref{b1:thm:embedding-extension}的嵌入延拓、\cref{b1:thm:normal-embedding-criterion}的正规性判据、\cref{b1:thm:separable-embeddings}的嵌入计数，以及\cref{b1:cor:embedding-independence}的独立性。群论使用子群、指数、正规子群、商群和置换作用。先完成不动域扩张次数等于群阶的证明，再建立对应，避免在两者之间循环论证。

\ProseParagraphBreak
建立对应定理只需第18章中的嵌入独立性，迹、范数和判别式的全部计算可以稍后再读。进入\secref{b1:sec:1903}的具体群计算时，再按所用方法补充工具：判别式判别使用\secref{b1:sec:1803}；模素数分解与循环型的联系使用中国剩余定理、有限域的 Frobenius 以及\secref{b1:sec:1804}已证明的\cref{b1:lem:integral-closure-elementary}。循环型判据在\subsecref{b1:subsec:1903:04}完整证明，\secref{b1:sec:2103}将直接引用它处理具体五次方程。
