...
设 \(\sigma:K\rightarrow K'\) 为域同构，\(\alpha\) 是某个域扩张中在 \(K\) 上代数的元素，最小多项式为 \(m\in K[x]\)。对一个包含 \(K'\) 的域 \(\Omega\)，将 \(\sigma\) 延拓为域嵌入 \(K(\alpha)\rightarrow\Omega\) 的方式，与 \(\sigma(m)\) 在 \(\Omega\) 中的不同根 \(\beta\) 一一对应；对应的嵌入把 \(\alpha\) 送到 \(\beta\)。这里 \(\sigma(m)\) 表示对系数施行 \(\sigma\)。
\end{lemma}
\begin{proof}
...
设 \(f\in K[x]\) 为非零多项式，\(L/K\) 是 \(f\) 的分裂域，\(\sigma:K\rightarrow K'\) 是域同构，而 \(L'/K'\) 是 \(\sigma(f)\) 的分裂域。则 \(\sigma\) 可以延拓为域同构 \(L\rightarrow L'\)。特别地，同一多项式的两个分裂域在保持基域的同构意义下唯一。
\end{proposition}
\begin{proof}
