...
\begin{samepage}
下表按适用范围汇合本章的嵌入工具；下一章再以可分性判断何时达到次数上界。
\begin{center}
\small
...
所处理的对象 & 嵌入的作用与新增条件 \\
\midrule
单扩张 & \cref{b1:lem:simple-embedding-extension}：生成元的像由最小多项式的根决定。 \\
一般代数扩张 & \cref{b1:thm:embedding-extension}：相容的部分嵌入须经延拓论证，不能只选一个根。 \\
正规扩张 & \cref{b1:thm:normal-embedding-criterion}：基域嵌入把扩张映回自身。 \\
有限扩张的嵌入数 & \cref{b1:prop:embedding-degree-bound}先给次数上界；可分性将在下一章给出达到上界的条件。 \\
\bottomrule
\end{tabularx}
\end{center}
\end{samepage}
