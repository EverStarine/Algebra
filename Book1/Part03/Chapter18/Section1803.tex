...
有限扩张 \(L/K\) 的\term[ji-peidui]{迹配对}[trace pairing]是双线性映射
\[
L\times L\longrightarrow K,\qquad (x,y)\longmapsto\operatorname{Tr}_{L/K}(xy).
\]
若对每个非零 \(x\) 都存在 \(y\) 使配对值非零，称此配对非退化。
\end{definition}
...
设 \(b=(b_1,\ldots,b_n)\) 是 \(L/K\) 的一组基。它的\term[ji-panbieshi]{基的判别式}[discriminant of a basis]定义为
\[
...
=\prod_{i<j}(a_j-a_i)^2.
\end{equation}
右式也称首一最小多项式的\term[duoxiangshi-panbieshi]{判别式}[discriminant of a polynomial]；对任意首一多项式，按全部根及重数作同样定义。它在根重合时为零。对 \(\mathbb Q(\sqrt d)\) 的基 \(1,\sqrt d\)，迹矩阵为 \(\operatorname{diag}(2,2d)\)，判别式为 \(4d\)。基的判别式依赖所选基，不能直接把任意一组有理基的判别式称为数域整数环的判别式。
