...
\qquad r=[L:K(a)].
\]
在 \(1,a,\ldots,a^{d-1}\) 这组基下，\(m_a\) 将前 \(d-1\) 个基向量依次移向后一个，将末项送到 \(-c_0-\cdots-c_{d-1}a^{d-1}\)。相应伴随矩阵的特征多项式为 \(f\)：沿 \(tI-m_a\) 的末列展开，系数依次为 \(c_0,c_1t,\ldots,c_{d-1}t^{d-1},t^d\)。再取 \(L\) 的一组 \(K(a)\) 基，便得到 \(r\) 个这样的对角块。因此
