...
\ProseParagraphBreak
下面的\cref{b1:lem:integral-closure-elementary}还有一个后续用途：\subsecref{b1:subsec:1903:04}会用根生成环的有限性取得有限剩余域，并将 Frobenius 的作用提升到原分裂域。只学习有限 Galois 对应时可以暂缓这一计算，但使用模素数分解法前须补读此引理；后面的整数环与判别式例子则可另作延伸阅读。
\begin{lemma}\label{b1:lem:integral-closure-elementary}
设 \(A\) 是域 \(L\) 的子环，\(a_1,\ldots,a_s\in L\) 都在 \(A\) 上整。则子环 \(A[a_1,\ldots,a_s]\) 由有限多个元素的 \(A\) 系数线性组合组成，即它是有限生成 \(A\) 模；其中每个元素都在 \(A\) 上整。特别地，\(L\) 中在 \(A\) 上整的元素的和、差与积仍在 \(A\) 上整。
\end{lemma}
\begin{proof}
...
\end{proof}
整元素的逆未必整，例如二是代数整数而 \(1/2\) 不是；迹、范数均为整数也不能在任意次数下反推元素整，因为它们只给出特征多项式的两个系数。对于二次扩张，这两个系数已经是全部非首项系数，所以结论恰好可以反推；章末习题将比较这两种情形。
