...
在有限维向量空间中，从一个线性无关组出发，不断加入线性包以外的向量，经过有限步便能得到一组基。若空间没有预先给定的有限维数，这个办法就留下了一个问题：即使每一步都能继续添加，怎样知道存在一个已经不能再扩大的线性无关集？Zorn 引理回答的正是这一类存在性问题。本节先说明它所依据的选择原则，再用基的扩充把论证中的每一步写清楚。
...
集合 \(P\) 上的二元关系是 \(P\times P\) 的一个子集，记 \(x\leq y\) 表示 \((x,y)\) 属于该关系。如果它满足以下三条，就称为\term[pianxu]{偏序}[partial order]，并称 \((P,\leq)\) 为偏序集：
\begin{enumerate}
...
设 \((P,\leq)\) 为偏序集，\(C\subseteq P\)。若 \(u\in P\) 满足对每个 \(c\in C\) 都有 \(c\leq u\)，称 \(u\) 为 \(C\) 在 \(P\) 中的\term[shangjie]{上界}[upper bound]。元素 \(m\in P\) 称为\term[jidayuan]{极大元}[maximal element]，若对每个 \(x\in P\)，\(m\leq x\) 都蕴含 \(x=m\)，即在 \(P\) 中不能再严格增大 \(m\)。若每个 \(x\in P\) 都有 \(x\leq m\)，则称 \(m\) 为\term[zuidayuan]{最大元}[greatest element]。
\end{definition}
...
\ProseParagraphBreak
在通常的集合论公理体系中，Zorn 引理与选择公理等价。本书把 Zorn 引理作为已采用选择公理的一个标准形式；从选择公理推出它的集合论证明不在这里展开。反向推导可以直接用部分选择函数完成，而且能说明为什么极大性与“同时选取”有关。
...
\begin{proof}
设 \(P\) 为所有包含 \(A_0\) 的线性无关子集组成的集合，按包含关系排序。这里“线性无关”是指：任何有限个互不相同的向量之间的线性关系，都只能具有全零系数。集合 \(A_0\) 属于 \(P\)，所以 \(P\) 非空。
...
由\cref{b1:thm:A-zorn}，取 \(P\) 中的极大元 \(B\)。剩下要证明 \(B\) 生成 \(V\)。若有 \(v\in V\) 不属于 \(B\) 的线性包，则 \(B\cup\{v\}\) 仍线性无关。事实上，若
\[
...
一个全序集 \((I,\leq)\) 若满足：对任意非空子集 \(J\subseteq I\)，都存在 \(j_0\in J\)，使每个 \(j\in J\) 都有 \(j_0\leq j\)，则称这个次序为\term[liangxu]{良序}[well-order]，并称 \(I\) 为良序集。
\end{definition}
...
良序原理也与选择公理等价，本书采用它而不在此证明等价性的全部方向。其中一个方向容易看见：若把 \(\bigcup_iX_i\) 良序，则对每个非空 \(X_i\) 取它在该次序中的最小元，就得到选择函数。良序原理保证能安排这种次序，并不声称给定的通常次序已经是良序，也不提供求出该次序的算法。
...
\ProseParagraphBreak
通常用\term[xushu]{序数}[ordinal]表示良序的次序类型。两个良序集次序类型相同，是指它们之间有保持次序的双射；序数只记录这样的排列方式，不记录元素的原来名称。有限良序的次序类型为 \(0,1,2,\ldots\)，其中 \(0\) 对应空集；自然数通常次序的类型记为 \(\omega\)。在全部自然数之后再添一个新位置，得到 \(\omega+1\)；上面两串元素的次序类型则为 \(\omega+\omega\)。它们可以作为集合有相同大小，却具有不同的排列次序。
