...
自由积的约化字既描述群元素，也给出一类可执行的计算方法。本附录先把这一构造推广到沿公共子群的合并，证明标准形和因子的嵌入；再用自由积的陪集树解释有限子群为什么来自因子。随后区分等式证书、判定算法和不可判定性，最后说明有限单群分类在群结构理论中的地位。
\end{chapterabstract}
...
\item 分清标准形的存在性与有效计算，理解字问题的肯定证书和停机要求。
\item 说明有限单群分类回答了什么问题，以及合成因子为何不足以确定原群。
\end{itemize}
\end{learninggoals}

本附录以\chapref{b1:ch:08}的自由群、群展示和自由积为主要先修；树作用部分还使用\chapref{b1:ch:04}的群作用语言，分类概览使用\chapref{b1:ch:07}的合成列。图论所需的路径和树在使用处说明，不预设代数拓扑。合并积标准形、陪集树与有限群不动点均在本附录证明；字问题不可判定性和有限单群分类只作明确的外部陈述，不作为本卷主干的证明输入。
...
\ProseParagraphBreak
展示中的等式有有限证书，但枚举证书只保证最终识别表示单位的输入。一个判定算法还必须在所有非单位输入上停止，标准形也只有在能够有效求出时才提供这种算法。有限单群分类解决了另一类结构问题：它确定有限群可能出现的简单因子，因子之间的组合仍需扩张理论。
\end{chaptersummary}
