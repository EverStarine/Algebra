...
群展示给出对象的定义，但读出一个字是否为单位，是另一个问题。一个定义可以很短，而由它判断等式的任务仍然困难。\subsecref{b1:subsec:0804:03}已经给出自由群和二面体群的算法；这里解释这些算法为何有效，以及有限展示本身为什么不保证类似算法存在。

\subsection{等式的有限证书}
\label{b1:subsec:C03:01}

固定有限展示 \(G=\langle S\mid R\rangle\)。输入是字母表 \(S\sqcup S^{-1}\) 上的有限字 \(w\)，要判断它在 \(G\) 中是否等于单位。判断两个字 \(u,v\) 是否相等，等价于对 \(uv^{-1}\) 作这项判断。\cref{b1:def:group-presentation}说明，\(w=1\) 当且仅当 \(w\) 在自由群 \(F(S)\) 中属于 \(R\) 的正规闭包。

\begin{proposition}\label{b1:prop:C-word-certificate}
对于有限展示 \(\langle S\mid R\rangle\)，一个字 \(w\) 表示单位，当且仅当存在有限个字 \(u_i\)、关系字 \(r_i\in R\) 及符号 \(\varepsilon_i\in\{1,-1\}\)，使
...
称给定有限生成集下的字问题\term[kepanding]{可判定}[decidable]，是指存在一个算法，对每个有限输入字都在有限步后停止，并正确回答它是否表示单位。这个要求同时包括肯定与否定两类输入；它也没有规定算法必须很快。

以下\term[Novikov-Boone-dingli]{Novikov--Boone 定理}[Novikov--Boone theorem]说明，这种终止保证不可能对所有有限展示的群成立。

\begin{theorem}[Novikov--Boone，外部引用]\label{b1:thm:C-novikov-boone}
存在一个有限展示的群，其字问题不可判定。
\end{theorem}
这一结果在本附录不作证明，证明需要把不可判定的计算过程编码进群关系。可参阅 Rotman 的原文陈述\cite[Chapter 12, Theorem 12.8, p. 431]{Rotman1995TheoryGroups}；该章随后完成所用 Boone 引理的证明。定理中的群和有限展示是固定的，变化的只有输入字；结论因此不只是说“找不到同时处理所有群展示的统一程序”。
