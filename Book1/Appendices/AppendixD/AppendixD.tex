...
本附录使用多项式除法、分裂域、有限 Galois 对应和分圆域。完备性在第一节按赋值定义；微分方程一节只用形式求导和矩阵乘法。赋值部分限于两个局部域模型、单根提升和一个分歧扩张模型；微分部分限于基本解矩阵、自同构的常数矩阵表示及一个二次扩张算例。一般局部类域论与微分 Galois 对应属于后续专题。有序域、实闭域及实根计数集中放在\appref{b1:app:E}，进一步的实代数与半代数几何转第三卷附录 G。
