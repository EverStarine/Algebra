...
“映射”这一要求同时包含两件事：每一对元素都有唯一确定的运算结果。因此整数上的除法不是二元运算：除数可能为零，即使除数非零，结果也未必是整数。相反，整数减法确实是二元运算，只是不满足结合律。例如 \((3-2)-1=0\)，而 \(3-(2-1)=2\)。不满足某条公理与根本没有定义成运算，应当分开判断。

\ProseParagraphBreak
运算的公理是对任意元素都成立的等式或条件；计算几个例子只能发现反例，不能验证无限集合上的全称命题。对于有限集合，可以把二元运算列成表格，但结合律仍需对每三个元素检验，不能只从表格对称性判断。表格关于主对角线对称表达的是交换律。
