...
设 \(r\geq1\)，\(G_1,\ldots,G_r,K\) 为群，并给定群同态 \(u_i:G_i\rightarrow K\)。存在群同态
\[
...
设 \((A_i)_{i\in I}\) 是一族交换群，采用加法记号，单位写成 \(0\)。它们的\term[zhihe]{直和}[direct sum]是所有仅有有限多个非零坐标的族所组成的群，记为
\[
...
加法逐坐标定义，零元是零族，负元逐坐标取负。坐标嵌入 \(\iota_i:A_i\rightarrow\bigoplus_{j\in I}A_j\) 将 \(a\) 放在第 \(i\) 个坐标，其余坐标均置为 \(0\)。当 \(I=\varnothing\) 时，直和是仅含空族的零群。
\end{definition}
