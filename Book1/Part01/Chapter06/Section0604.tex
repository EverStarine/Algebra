...
先比较表中前两条扩张。把两条扩张的核都识别为 \(C_4=\langle z\rangle\)，其中 \(D_8\) 中令 \(z=r\)，\(Q_8\) 中令 \(z=i\)；把商群写成 \(C_2=\{1,q\}\)。在 \(D_8\) 中选归一化截面 \(\sigma_D(q)=s\)，在 \(Q_8\) 中选 \(\sigma_Q(q)=j\)，则相应因子集满足
\[
c_D(q,q)=s^2=1,
\qquad
...
\ProseParagraphBreak
这个平方差同样不依赖归一化截面的选择。设商元素 \(h\) 满足 \(h^2=1\)，并把截面改为 \(\sigma'(h)=b(h)\sigma(h)\)，其中 \(b(h)\in\langle z\rangle\)、\(b(1)=1\)。中心扩张的诱导作用平凡，且二阶核中 \(b(h)^2=1\)，所以
\[
c'(h,h)=b(h)^2c(h,h)b(1)^{-1}=c(h,h).
\]
取 \(h=(0,1)\)，上式说明 \(D_8\) 中的 \(c(h,h)=1\) 与 \(Q_8\) 中的 \(c(h,h)=z\) 都不会随截面改变；两个模型中第二个提升的平方差不能消去。

\ProseParagraphBreak
还可核对两群的共轭类大小分布。\(Q_8\) 的中心元素给出单元素共轭类 \(\{1\}\) 与 \(\{-1\}\)。由四元数关系，\(jij^{-1}=-i\)；而 \(\pm1,\pm i\) 与 \(i\) 交换，\(\pm j,\pm k\) 不与 \(i\) 交换，所以
\[
C_{Q_8}(i)=\langle i\rangle.
\]
这个中心化子有四个元素，故 \(i\) 的共轭类有 \(8/4=2\) 个元素，恰为 \(\{i,-i\}\)。对 \(j,k\) 同理，再加上两个中心元素，得到 \(Q_8\) 的全部共轭类
\[
\{1\},\quad\{-1\},\quad\{i,-i\},\quad
\{j,-j\},\quad\{k,-k\}.
\]
因此其共轭类大小依次为 \(1,1,2,2,2\)，与\secref{b1:sec:0402}中算出的 \(D_8\) 共轭类大小分布相同。

\ProseParagraphBreak
不过，直接数元素阶仍可区分这两个抽象群：\(D_8\) 中四个反射和 \(r^2\) 都为二阶，共五个；\(Q_8\) 中只有 \(-1\) 为二阶，其余六个非单位元素为四阶。因此二者不同构；中心、中心商和共轭类大小分布这些相同的不变量不足以区分它们。
...
\ProseParagraphBreak
\textbf{进一步解释。}第四卷第十一章“群的同调与上同调”将系统研究这种计算。固定交换群 \(A\)、群 \(H\) 及 \(H\) 在 \(A\) 上的平凡作用后，满足上述归一化条件与因子集恒等式的函数称为\term[guiyihua-er-yuquan]{归一化 \(2\)-余圈}[normalized 2-cocycle]。这些函数按逐点相加组成交换群：零函数是单位元，逐点取负给出逆元，而因子集恒等式对加法封闭。对任意满足 \(b(1)=0\) 的函数 \(b:H\rightarrow A\)，函数
\[
(h,k)\longmapsto b(h)+b(k)-b(hk)
\]
称为\term[er-yubian]{\(2\)-余边}[2-coboundary]；它正是改变归一化截面所产生的两个余圈之差。映射 \(b\mapsto\delta b\) 保持逐点加法，所以全部 \(2\)-余边组成归一化 \(2\)-余圈群的子群。把相差 \(2\)-余边的归一化 \(2\)-余圈视为同一类，所得商群就是第二上同调群 \(H^2(H,A)\)，它分类上述中心扩张的等价类。这个分类解释属于后续内容；本小节的两种低阶扩张已经通过乘法和具体映射得到，不需要预先使用群上同调。
