...
设 \(G\) 为群，\(N\trianglelefteq G\)、\(H\leq G\)。如果 \(N\cap H=\{1_G\}\) 且 \(NH=G\)，则将 \(G\) 称为 \(N,H\) 的\term[nei-banzhiji]{内半直积}[internal semidirect product]，并将 \(H\) 称为 \(N\) 的一个\term[buqun]{补群}[complement]。直积是作用平凡的半直积：当 \(\alpha_h(n)=n\) 时，乘法恢复为逐坐标乘法。反过来，若此内分解中的 \(H\) 也正规，则满足\cref{b1:prop:internal-direct-product}的全部条件；该命题证明中的计算给出 \(hnh^{-1}n^{-1}\in N\cap H=\{1\}\)，故 \(\alpha_h(n)=n\)，作用便平凡。
