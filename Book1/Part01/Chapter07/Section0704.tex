...
\end{enumerate}
这些情形穷尽了支持度四至六的偶置换：由\cref{b1:thm:sign}的轮换符号公式，\(k\) 轮换的符号为 \((-1)^{k-1}\)，所以六轮换、三个换位，以及三轮换乘一个换位都是奇置换；含固定点的轮换型应按实际支持度计入前面的情形。因此 \(N\) 含有三轮换。\cref{b1:lem:three-cycles-generate}说明所有三轮换在 \(A_n\) 中共轭，而 \(N\trianglelefteq A_n\) 保证这些共轭全部属于 \(N\)；再用该引理的生成结论，得到 \(N=A_n\)。最后，\((123)(124)\) 把 \(1\) 送到 \(3\)，而 \((124)(123)\) 把 \(1\) 送到 \(4\)，所以两个三轮换不交换，\(A_n\) 非阿贝尔。
\end{proof}
...
\end{corollary}
\begin{proof}
式\eqref{b1:eq:commutator-image}说明自同构保持导出子群，因此 \(A_n'\trianglelefteq A_n\)。由\cref{b1:thm:alternating-simple}，\(A_n\) 非阿贝尔且为单群：前者说明存在不交换的元素，故 \(A_n'\ne1\)；后者迫使 \(A_n'=A_n\)。所以导出列各项均为 \(A_n\)，不能下降到单位群，\(A_n\) 不可解。若 \(S_n\) 可解，\cref{b1:thm:solvable-closure}用于子群 \(A_n\le S_n\) 会推出 \(A_n\) 可解，矛盾。低阶时，式\eqref{b1:eq:small-symmetric-derived}已具体给出 \(S_3''=1\)、\(S_4'''=1\)；而 \(S_1=1\)、\(S_2\cong C_2\) 直接阿贝尔，故均可解。
\end{proof}
