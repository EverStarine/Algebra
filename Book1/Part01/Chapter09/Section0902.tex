...
整数格中的非零元素都有无限阶，循环群 \(C_n\) 的元素则都被某个正整数倍消去。一般阿贝尔群可能同时含有这两部分，因此分类前应先提取有限阶元素，再研究剩下的商群。有限阶部分还可按素数分别处理。这里的“无挠”暂时只限制元素的阶，并不保证群自由；本节末尾的无限群例子将说明有限生成条件的作用。
...
设 \(A\) 为阿贝尔群。若 \(mx=0\)、\(ny=0\)，则 \(mn(x-y)=0\)，所以有限阶元素有希望组成子群。这个简单计算同时提示：所得子群依赖加法的交换性。

\begin{definition}\label{b1:def:torsion-subgroup}
设 \(A\) 是加法记号的阿贝尔群。若元素 \(x\in A\) 满足 \(mx=0\)，其中 \(m\) 为某个正整数，则称 \(x\) 为\term[naoyuan]{挠元}[torsion element]。全部挠元组成的集合记为
\[
