...
设 \(m,n\) 为正整数，\(M\) 为 \(m\times n\) 整数矩阵。对 \(1\le k\le\min(m,n)\)，令 \(\Delta_k(M)\) 为 \(M\) 的全部 \(k\) 阶子式的非负最大公因数，称为第 \(k\) 个\term[xinglieshi-yinzi]{行列式因子}[determinantal divisor]；全部相应子式都为零时约定其最大公因数为零，并令 \(\Delta_0(M)=1\)。若 \(M\) 的秩为 \(t\)，即有 \(t\) 个非零 Smith 对角元，则
\[
\begin{aligned}
