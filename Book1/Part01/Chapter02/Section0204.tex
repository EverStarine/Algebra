...
同构定理不是几个看起来相似的分式恒等式。每个同构都由一个具体映射产生，分母描述该映射消去了什么，分子描述实际可到达哪些元素。先写映射、再求核和像，是使用这些定理最可靠的顺序。
...
\end{theorem}
\begin{proof}
核正规，由商群泛性质，所给映射良定义且为同态。它按像的定义为满射；若 \(f(g)=f(h)\)，则 \(h^{-1}g\in\ker f\)，故
\(g\ker f=h\ker f\)，得到单射。双射同态的逆也是同态，因此它是同构。
\end{proof}
...
\]
而 \((hn)^{-1}=n^{-1}h^{-1}=h^{-1}(hn^{-1}h^{-1})\in HN\)，故是子群。\(N\) 在 \(HN\) 中仍正规。把商映射限制到 \(H\)，得同态 \(h\mapsto hN\)，其核是 \(H\cap N\)，像是 \(HN/N\)，因为 \(hnN=hN\)。核的正规性与第一同构定理同时给出其余结论。
\end{proof}
...
\end{theorem}
\begin{proof}
同态 \(G\rightarrow G/K\) 的核为 \(K\)，包含 \(N\)，故诱导出满同态
\(G/N\rightarrow G/K\)，\(gN\mapsto gK\)。它的核恰为
\(\{gN\mid g\in K\}=K/N\)。核正规，再用第一同构定理即得结果。特别地，两层陪集公式的良定义已经由这两个具体映射保证，无须在嵌套括号中猜测代表元。
\end{proof}
...
\end{theorem}
\begin{proof}
同态像是子群；同态下子群的原像含单位元，并对 \(ab^{-1}\) 封闭，也是子群。原像必含 \(\ker q=N\)。若 \(H\supseteq N\)，则
\(q^{-1}(q(H))=H\)：确实，\(q(g)=q(h)\) 蕴含 \(h^{-1}g\in N\subseteq H\)，从而 \(g\in H\)。另一方面，\(q\) 满射给出
\(q(q^{-1}(U))=U\)。这证明互逆；包含保持由像与原像的基本性质得到。

若 \(H\) 正规，则任意商群元素 \(gN\) 对 \(H/N\) 的共轭仍落在其中。反之，若 \(H/N\) 正规，则对 \(h\in H,g\in G\)，有
\(q(ghg^{-1})\in H/N\)，取原像得 \(ghg^{-1}\in H\)，故 \(H\) 正规。指数公式来自左陪集的双射
\(hK\mapsto(hN)(K/N)\)：两边的相等条件都等价于 \(h_2^{-1}h_1\in K\)。最后，保持包含关系的互逆对应必保持最大下界与最小上界；具体说，交是最大的公共子群，生成子群是最小的共同上界，故它们在对应下仍具有同一描述。
\end{proof}

例如要找 \(\mathbb Z/12\mathbb Z\) 中由 \([4]\) 生成的子群，先在 \(\mathbb Z\) 中取包含 \(12\mathbb Z\) 与 \(4\) 的子群 \(4\mathbb Z\)。它的像是 \(\{[0],[4],[8]\}\)，指数为 \([\mathbb Z:4\mathbb Z]=4\)。若原子群不含核，例如 \(5\mathbb Z\)，其像的原像会变大：模 \(12\) 时 \([5]\) 生成全部余数，所以原像是 \(\mathbb Z\)，不是 \(5\mathbb Z\)。
