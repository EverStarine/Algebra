...
若 \(g\) 的有限阶为 \(n\geq1\)，则对任意 \(a\in\mathbb Z\)，有
\[
...
循环群的子群和商群均为循环群。若 \(G=\langle g\rangle\) 的有限阶为 \(n\geq1\)，则对每个正因子 \(d\mid n\)，恰有一个 \(d\) 阶子群，即 \(\langle g^{n/d}\rangle\)，其商群阶为 \(n/d\)。
\end{theorem}
\begin{proof}
...
设 \(C_m=\langle x\rangle\)、\(C_n=\langle y\rangle\)，其中 \(m,n\geq1\)。任一同态由 \(x\) 的像 \(y^a\) 唯一决定；指定这个像能够给出同态，当且仅当 \(n\mid ma\)。若 \(d=\gcd(m,n)\)，则模 \(n\) 的全部可取指数为
\[
