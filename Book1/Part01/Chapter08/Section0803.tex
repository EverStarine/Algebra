...
上述论证首先证明的是抽象群元素的约化标准形存在且唯一。若要把从左到右的约化过程实现为处理有限输入的有效算法，还必须能够有效表示各因子的元素、计算同一因子内的乘积，并判定所得乘积是否为单位元；标准形引理本身并未提供这些因子内部的计算与判定方法。该引理还说明，只使用 \(G\) 中元素写成的字在 \(G*H\) 中表示单位元，当且仅当它在 \(G\) 中的乘积为单位元。因此，不能仅凭自由积具有唯一约化形式，就无条件断言任意自由积的字问题可解。
...
\]
其中右端是\cref{b1:def:normal-closure}定义的、在环境群 \(G*H\) 中取得的正规闭包。
\end{proposition}
\begin{proof}
