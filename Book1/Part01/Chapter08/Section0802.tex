...
\,\right\}.
\]
其中 \(m=0\) 时右端为单位元，即空乘积。两个这样的有限乘积相乘后仍属其中；取逆只须颠倒因子顺序并把各 \(\varepsilon_i\) 变号，所以右端是子群。用 \(v\in K\) 共轭时，只须把每个 \(u_i\) 换成 \(vu_i\)，因此它正规。这个子群包含 \(R\)，而任何包含 \(R\) 的正规子群都必须包含这些共轭因子及其有限乘积，所以它恰好等于定义中的交。
...
等式关系 \(v=w\) 是关系字 \(vw^{-1}\) 的简写。

\ProseParagraphBreak
任何群都有展示。给定生成集 \(S\)，\cref{b1:thm:free-group-universal}把包含映射 \(S\to G\) 延拓为同态 \(\varphi:F(S)\to G\)；它的像包含生成集，故满射。取 \(R=\ker\varphi\)。因为核本身正规，所以 \(\NormalClosure{R}_{F(S)}=R\)，而\cref{b1:thm:first-isomorphism}给出 \(F(S)/R\cong\operatorname{im}\varphi=G\)，即 \(G\cong\langle S\mid R\rangle\)。这并不保证关系集有限，也没有自动提供判别两个字是否相等的算法。
...
\[
\text{关系成立}\Longrightarrow\text{同态存在},\qquad
\text{像生成目标}\Longrightarrow\text{满射},\qquad
\text{核平凡}\Longrightarrow\text{同构}.
\]
...
t(xy)t^{-1}=y(xy)=x.
\]
所以 \(t\) 的共轭保持 \(V\)。子群 \(V\) 还被自身的元素共轭保持，而 \(P\) 由 \(x,y,t\) 生成，因此 \(V\trianglelefteq P\)。商群 \(P/V\) 由 \(tV\) 生成，且 \((tV)^3=V\)，故 \([P:V]\leq3\)。于是
\[
