...
稳定子群也称稳定化子。\termidx[wendinghuazi]{稳定化子}它由所有固定 \(x\) 的群元素组成；讨论单个群元素时，直接说“固定 \(x\) 的元素 \(g\)”。
...
\]
当 \(X=\varnothing\) 时，右端是空族子群之交，约定为整个群 \(G\)，这与空集上唯一作用的核为 \(G\) 一致。作用的核是同态的核，因而是 \(G\) 的正规子群。由\cref{b1:prop:kernel-fibers}，忠实等价于 \(\rho\) 单射；自由则等价于每个稳定子群都平凡。
