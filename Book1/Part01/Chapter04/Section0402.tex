...
\ProseParagraphBreak
中心化子 \(C_G(H)\) 要求共轭固定 \(H\) 的每个元素，正规化子 \(N_G(H)\) 则只要求共轭保持 \(H\) 整体不变。每个 \(h\in H\) 都满足 \(hHh^{-1}=H\)，故 \(H\leq N_G(H)\)；再按正规化子的定义，便有 \(H\trianglelefteq N_G(H)\)。此外，\(H\trianglelefteq G\) 当且仅当 \(N_G(H)=G\)。中心已在\cref{b1:def:center}之后证明为正规子群，此处不再重复该证明。
...
\subsection{有限 \texorpdfstring{\(p\)}{p} 群的不动点与中心}
\label{b1:subsec:0402:03}

\begin{definition}\label{b1:def:p-group}
设 \(p\) 为素数，\(a\geq0\) 为整数。如果有限群 \(G\) 的阶为 \(p^a\)，则将 \(G\) 称为有限\term[p-qun]{\(p\)-群}[p-group]。当 \(a\geq1\) 时，\(G\) 非平凡。
\end{definition}

由\cref{b1:thm:orbit-stabilizer}，有限 \(p\)-群 \(P\) 的每个轨道大小为某个稳定子群的指数 \([P:P_x]\)，因而整除 \(|P|\)，是 \(p\) 的幂。这给出一个将在 Sylow 理论中反复使用的计数方法。
...
\ProseParagraphBreak
类方程把共轭作用的单元素轨道与其他轨道分开。对于有限 \(p\)-群，同样的分法可用于任意有限作用集合：非单元素轨道的大小都被 \(p\) 整除，模 \(p\) 计数时只留下共同不动点的贡献。
...
单位元已经给出共轭作用的一个共同不动点。同余还说明共同不动点的总数 \(\lvert Z(P)\rvert\) 是 \(p\) 的倍数；由于这个数为正，必有 \(\lvert Z(P)\rvert\geq p\)，所以除单位元外还有其他共同不动点，这才得到中心非平凡。若 \(x_1,\ldots,x_r\) 是非中心共轭类的代表，同一计数机制也可直接从类方程读出：
\[
|P|-|Z(P)|
=\sum_{i=1}^{r}[P:C_P(x_i)]
\equiv0\pmod p.
\]
因此，应用一般不动点同余与对类方程模 \(p\) 计数是同一种轨道分解方法。一般有限群的中心则可以平凡，如 \(S_3\) 所示。
