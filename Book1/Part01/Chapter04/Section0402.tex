...
对共轭作用使用\cref{b1:thm:orbit-stabilizer}，其中点 \(x\) 的稳定子为 \(C_G(x)\)，得到
\[
...
\end{definition}
由\cref{b1:thm:orbit-stabilizer}，有限\term[p-qun]{\(p\)-群}[p-group] \(P\) 的每个轨道大小为某个稳定子的指数 \([P:P_x]\)，因而整除 \(|P|\)，是 \(p\) 的幂。这给出一个将在 Sylow 理论中反复使用的计数方法。
