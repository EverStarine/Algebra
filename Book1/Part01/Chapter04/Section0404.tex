...
设当前作用同态为 \(\rho:G\rightarrow S_m\)，并令 \(K=\ker\rho\)。由\cref{b1:thm:first-isomorphism}，
\[
G/K\xrightarrow{\sim}\rho(G)\leq S_m.
\]
要由这个作用得到 \(G\) 本身到 \(S_m\) 的嵌入，必须证明 \(K=\{1\}\)。未证明忠实性时，只能把当前作用的像识别为商群 \(G/K\)，不能把当前作用直接当作 \(G\) 的嵌入；这并不排除同一个抽象群通过另一个作用嵌入 \(S_m\)。另一方面，发现核非平凡也常是所需结论：它直接提供正规子群。在下一章中，Sylow 子群上的共轭作用将同时发挥这两种作用。
\end{remark}
