...
一个非零交换含幺环 \(A\) 称为\term[jubu-huan]{局部环}[local ring]，如果它恰有一个极大理想 \(\mathfrak m\)。常以 \((A,\mathfrak m)\) 同时注明环和它的唯一极大理想；商域 \(A/\mathfrak m\) 称为它的\term[shengyu-yu]{剩余域}[residue field]。
\end{definition}
...
设 \(\mathfrak p\) 为 \(R\) 的素理想，则 \(R_{\mathfrak p}\) 是局部环，唯一极大理想为
\[
...
对 \(f\in R\)，令 \(S=\{\,f^n\mid n\geq0\,\}\)，其中 \(f^0=1\)。相应局部化记为 \(R_f\) 或 \(R[1/f]\)，其每个元素都能写成 \(a/f^n\)。
...
对任意交换含幺环 \(R\) 和 \(f\in R\)，有保持 \(R\) 中元素像的同构
\[
R_f\cong R[t]/(ft-1),
\]
...
域 \(k\) 上的一元 Laurent 多项式环的单位恰为 \(ct^r\)，其中 \(c\in k^\times\)、\(r\in\mathbb Z\)。
\end{proposition}
\begin{proof}
