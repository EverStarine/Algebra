...
本章使用\chapref{b1:ch:10}的交换环、理想与商环知识，并从\cref{b1:prop:fraction-field-basic}接过已经构造的分式域。局部化本身不要求原环具有唯一分解性，也不以 \(R\rightarrow S^{-1}R\) 为单射作为先决条件。\chapref{b1:ch:11}和\chapref{b1:ch:12}的整除与多项式计算，为本章提供具体例子；模的局部化留待第三卷第2章在模论基础上展开。
...
分式域使整环中所有非零元素可逆；局部化则允许指定一个乘法集。一般环中的等式必须容许再乘一个分母集元素，才能适应零因子。零判据、单位判据和同态延拓的泛性质，分别控制哪些元素消失、哪些元素获得逆元，以及如何把局部化映入其他环。
