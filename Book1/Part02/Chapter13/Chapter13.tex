...
\ProseParagraphBreak
局部化中的全部理想都来自原环，但扩张后再收缩得到的是原理想的饱和。在单个元素 \(f\) 处，这个饱和是 \(I:f^\infty=\bigcup_{n\geq0}(I:f^n)\)；Noether 条件保证相应升链最终稳定。与分母集不相交的素理想已经饱和，因此与局部化的素理想一一对应。在某一个局部环中成立的结论未必全局成立；对全部极大理想逐一检查，则能够检测元素相等、理想成员以及可逆性。向模和几何对象推广时，哪些性质可以这样检测，仍须分别证明。
\end{chaptersummary}
...
\]
称为 \(I\) \term[lixiang-baohe]{关于理想的饱和}[saturation with respect to an ideal]，这里指关于 \(J\) 的饱和。
