...
把非零整数作为分母得到有理数，是局部化最熟悉的例子。一般交换环中，允许哪些元素作分母，会同时影响元素的相等、单位以及理想。本章从分式域出发，构造任意乘法集上的局部化，证明其泛性质，并通过整数环、素理想处的局部环和 Laurent 多项式环说明局部观点的含义。最后用理想商描述饱和，建立理想与素理想的对应，并证明理想成员和元素相等可由所有极大理想处的局部化检测。
\end{chapterabstract}
...
\item 用理想商计算单元素饱和，说明 Noether 条件如何保证相应升链稳定。
\item 运用扩张、收缩和饱和分析理想，理解局部判据为什么需要检查全部极大理想。
\end{itemize}
\end{learninggoals}
...
\ProseParagraphBreak
局部化的基础可参阅 Atiyah 与 Macdonald 的《Introduction to Commutative Algebra》\cite[Chapter 3]{AtiyahMacdonald1969CommutativeAlgebra}。Matsumura 的《Commutative Ring Theory》\cite[Section 4]{Matsumura1989CommutativeRingTheory}把局部化与环的素谱一起讨论，适合了解本章之后的几何解释；Nagata 的《Local Rings》\cite[Sections 5--6]{Nagata1975LocalRings}则可供继续学习局部环与分式环时查阅。后两书需要更熟悉交换环与模的基本性质。
...
\ProseParagraphBreak
局部化中的全部理想都来自原环，但扩张后再收缩得到的是原理想的饱和；改变允许的分母，会有选择地保留或消去理想信息。在单个元素 \(f\) 处，这个饱和是 \(I:f^\infty=\bigcup_{n\geq0}(I:f^n)\)；Noether 条件保证相应升链最终稳定。与分母集不相交的素理想已经饱和，因此与局部化的素理想一一对应。这与第一编的商构造相呼应：两种构造都必须逐项核验映射与运算，但一个施加等价关系，另一个让指定分母变成单位。在某一个局部环中成立的结论未必全局成立；对全部极大理想逐一检查，则能够检测元素相等、理想成员以及可逆性。后卷将研究模的局部化、正合性及更系统的局部性质，不把这些结论视为本章已证。
\end{chaptersummary}
...
\end{exercise}
\begin{solution}
设 \(J\neq(0)\) 为 \(\mathbb Z_{(p)}\) 的理想。取非零分式 \(a/b\in J\)，乘以 \(b/1\) 得非零整数 \(a/1\in J\)，所以收缩 \(I\subseteq\mathbb Z\) 非零。取 \(I\) 中最小正整数 \(d\)。任意 \(c\in I\) 作整数除法 \(c=qd+r\)、\(0\leq r<d\)，则 \(r\in I\)，最小性迫使 \(r=0\)，故 \(I=d\mathbb Z\)。
...
设 \(I,J\) 为交换含幺环 \(R\) 的理想，约定 \(J^0=R\)。把
\[
I:J^\infty\coloneqq\bigcup_{n\geq0}(I:J^n)
=\{\,a\in R\mid aJ^n\subseteq I\text{ 对某个整数 }n\geq0\,\}
\]
这个构造称为\term[lixiang-baohe]{关于理想的饱和}[saturation with respect to an ideal]；在上述记号中，\(I:J^\infty\) 是 \(I\) 关于 \(J\) 的饱和。
\symidx{\(I:J^\infty\)：理想 \(I\) 关于理想 \(J\) 的饱和}
证明它是理想；若 \(J=(f_1,\ldots,f_r)\)、\(r\geq1\)，证明
\[
I:J^\infty=\bigcap_{i=1}^{r}(I:f_i^\infty).
\]
最后在 \(k[x,y]\) 中取 \(I=(xy)\)、\(J=(x,y)\)，计算 \(I:J^\infty\)，并与 \(I\) 关于乘法集 \(S=\{\,x^ay^b\mid a,b\geq0\,\}\) 的饱和比较。
\end{exercise}
\begin{solution}
由 \(J^{n+1}\subseteq J^n\)，有 \(I:J^n\subseteq I:J^{n+1}\)。各项都是\cref{b1:prop:ideal-quotient-rules}所给的理想，因此并集也为理想：其中任意两个元素同时属于某个足够靠后的项，相减仍在该项；乘以任意环元素也留在原项。

若 \(a\in I:J^\infty\)，取 \(n\geq0\) 使 \(aJ^n\subseteq I\)。因为 \(f_i^n\in J^n\)，所以 \(af_i^n\in I\)，即 \(a\in I:f_i^\infty\)，这对每个 \(i\) 都成立。

反过来，设 \(a\) 属于右端的交。对每个 \(i\)，取整数 \(n_i\geq0\) 使 \(af_i^{n_i}\in I\)。若某个 \(n_i=0\)，则 \(a\in I\)，结论已经成立。否则全部 \(n_i\geq1\)，令
\[
N=1+\sum_{i=1}^{r}(n_i-1).
\]
理想 \(J^N\) 由所有 \(f_1^{e_1}\cdots f_r^{e_r}\)、\(e_1+\cdots+e_r=N\) 生成，这来自把 \(J\) 的生成元代入 \(N\) 个因子的乘积并按分配律展开。对每个这样的单项式，必有某个 \(e_i\geq n_i\)：否则各 \(e_i\leq n_i-1\)，相加只会得到总和至多 \(N-1\)。因此 \(a\) 乘以该单项式后含有因子 \(af_i^{n_i}\)，属于 \(I\)。再取这些生成元的任意线性组合，可知 \(aJ^N\subseteq I\)。故 \(a\in I:J^\infty\)，证明了所求等式。有限个生成元使这一步能够选取共同的指数 \(N\)。

在给定例子中，\(I:x^\infty=(y)\)、\(I:y^\infty=(x)\)：例如 \(x^na\in(xy)\) 时令 \(y=0\)，得到 \(x^na(x,0)=0\)，整环 \(k[x]\) 中便有 \(a(x,0)=0\)，即 \(y\mid a\)；反向包含由 \(xy\in I\) 立即得到。交换变量给出另一式。于是
\[
