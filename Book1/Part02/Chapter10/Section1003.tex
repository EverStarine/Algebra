...
设 \(I\) 为环 \(R\) 的双边理想。定义 \(I^0=R\)、\(I^1=I\)，并对整数 \(n\geq1\) 递归定义 \(I^{n+1}=I^nI\)。每个 \(I^n\) 是由 \(n\) 个 \(I\) 中元素的乘积形成的有限和，而不是只由某个元素的 \(n\) 次幂组成。交换环的主理想满足 \((a)^n=(a^n)\)，但多生成元时各个混合乘积也必须计算。
...
\ProseParagraphBreak
若希望分解理想而保留幂次信息，就会遇到\term[zhunsu-lixiang]{准素理想}[primary ideal]。设 \(Q\) 为交换环 \(R\) 的真理想；如果对任意 \(a,b\in R\)，条件 \(ab\in Q\) 且 \(a\notin Q\) 总能推出存在正整数 \(m\) 使 \(b^m\in Q\)，就称 \(Q\) 为准素理想。例如整数环中的 \(p^r\mathbb Z\)（\(p\) 为素数，\(r\geq1\)）具有此性质。若 \(p^r\mid ab\) 而 \(p^r\nmid a\)，整数的素数分解说明 \(p\mid b\)，故 \(p^r\mid b^r\)。特别地，
\[
