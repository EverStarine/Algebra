...
交换环 \(R\) 的两个理想 \(I,J\) 若满足 \(I+J=R\)，称为\term[husu-lixiang]{互素理想}[comaximal ideals]。这等价于存在 \(a\in I,b\in J\) 使 \(a+b=1\)。
\end{definition}

\begin{lemma}\label{b1:lem:comaximal-product}
互素理想满足 \(I\cap J=IJ\)。若 \(I_1,\ldots,I_r\) 两两互素，则
...
设交换环 \(R\) 的理想 \(I_1,\ldots,I_r\) 两两互素，\(r\geq1\)。则映射
\[
...
不要求 \(m,n\) 互素时，以上两同余式有解的充要条件是
\(a\equiv b\pmod d\)，其中 \(d=\gcd(m,n)\)。有解时，全部解为模 \(\operatorname{lcm}(m,n)\) 的一个剩余类。
\end{proposition}
\begin{proof}
...
环 \(R\) 中满足 \(e^2=e\) 的元素称为\term[midengyuan]{幂等元}[idempotent element]。一族幂等元若对不同指标满足 \(e_ie_j=e_je_i=0\)，称为两两正交。
\end{definition}

\begin{proposition}\label{b1:prop:idempotent-product}
若交换环 \(R\) 有两两正交的幂等元 \(e_1,\ldots,e_r\)，且 \(\sum_i e_i=1\)，则
\[
R\longrightarrow\prod_{i=1}^r e_iR,\qquad
a\longmapsto(e_ia)_i
\]
为环同构。每个 \(e_iR\) 的单位是 \(e_i\)，不是一般意义下的 \(1_R\)。反之，有限环直积中的各坐标单位给出这样一族幂等元。
\end{proposition}
\begin{proof}
...
\end{proof}
在中国剩余定理的证明中，\(e_i\) 未必在原环中幂等，但它们在商环 \(R/\bigcap I_i\) 中的像就是这些坐标幂等元。例如 \(\mathbb Z/12\mathbb Z\cong\mathbb Z/3\mathbb Z\times\mathbb Z/4\mathbb Z\) 中，\([4]\) 与 \([9]\) 分别对应 \((1,0)\)、\((0,1)\)：它们的平方各等于自身，乘积为零，和为 \([1]\)。
