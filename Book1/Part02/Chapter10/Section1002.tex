...
若 \(I\) 为环 \(R\) 的双边理想，则对任意 \(a,b\in R\)，按
\[
(a+I)+(b+I)=(a+b)+I,\qquad
(a+I)(b+I)=ab+I
\]
规定运算后，加法陪集集合 \(R/I\) 构成\term[shanghuan]{商环}[quotient ring]，单位为 \(1+I\)。自然映射 \(q:R\rightarrow R/I\) 是满环同态，核为 \(I\)。
\end{proposition}
\begin{proof}
...
设 \(R\) 为环，\(I\) 为 \(R\) 的双边理想，\(q:R\rightarrow R/I\) 为自然映射。则 \(q\) 给出包含 \(I\) 的双边理想 \(J\subseteq R\) 与 \(R/I\) 的双边理想之间的保序双射，双向公式为 \(J\mapsto J/I\) 与 \(L\mapsto q^{-1}(L)\)。若 \(I\subseteq J\)，则
\[
(R/I)/(J/I)\cong R/J.
\]
