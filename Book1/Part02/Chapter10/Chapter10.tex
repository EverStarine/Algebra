...
\ProseParagraphBreak
首次学习商环和后续多项式理论时，宜先连贯掌握环及同态的约定、双边理想、商环与同构定理、素理想和极大理想的商环判据，以及理想的基本运算。若暂时只做具体计算，可先跳过一般极大理想存在性的 Zorn 证明；以后需要无条件调用该存在性结论时，仍须补读证明。理想商及准素理想可在掌握上述内容后继续学习；习题中若用到准素理想，应先阅读\cref{b1:def:primary-ideal}。
...
商环要求被抹去的加法子群同时吸收左右乘法，这给出了理想的定义及核的基本性质。交换环的素理想对应整环商，极大理想对应域商；素性与极大性和理想是否非零是不同的问题。理想的和、交、积是三种不同的运算，取根识别哪些元素在商环中成为幂零元，理想商则收集满足指定乘法条件的元素；取零理想作第一项就得到零化子。
...
取右下对角坐标的同态 \(T\to k\) 的核为 \(I\)，取左上对角坐标的同态之核为 \(J\)，故两者都是双边理想。它们相加含 \(E_{11},E_{12},E_{22}\)，所以 \(I+J=T\)；同时属于两者的矩阵恰为严格上三角矩阵，故 \(I\cap J=N\)。直接相乘可得
\[
...
\]
由于 \(E_{11}E_{12}=E_{12}\)，第一类乘积生成整个 \(N\)，故 \(IJ=N\)，而 \(JI=0\)。这说明即使两个双边理想之和为全环，它们在非交换环中的乘积仍可能随次序改变。
\end{solution}
...
\(\epsilon(\sum_g a_gg)=\sum_g a_g\)。证明它为满环同态，并证明其核作为 \(k[G]\) 的双边理想由全部 \(g-1\)、\(g\in G\) 生成。若 \(G\) 为二阶群且 \(\operatorname{char}k=2\)，证明该核的平方为零。
\end{exercise}
\begin{solution}
系数和保持加法与单位；乘积的系数和为
...
\end{exercise}
\begin{solution}
映射 \(\psi:R\to\mathbb F_2\)、\(f\mapsto[f(0)]_2\) 是满环同态，核为 \(I\)。因此 \(I\) 是真素理想，且 \(\sqrt I=I\)。若 \(I=(f)\)，则 \(f\) 整除常数 \(2\)，故 \(f\) 只能是整数常数 \(\pm1\) 或 \(\pm2\)；又因 \(x\in I\)，它须整除 \(x\)，于是 \(f=\pm1\)，与 \(I\ne R\) 矛盾。
...
\begin{enumerate}
\item 设 \(R\) 为交换环，\(Q\subsetneq R\) 为理想。证明 \(Q\) 为准素理想，当且仅当 \(R/Q\) 的每个非零零因子都幂零。
\item 对整数 \(n\geq2\)，判定何时 \(n\mathbb Z\) 是准素理想。用 \(n=12\) 写出直接违反准素性条件的两个整数，并比较 \(\mathbb Z/4\mathbb Z\) 与 \(\mathbb Z/12\mathbb Z\) 中非零零因子的幂零性。
\end{enumerate}
\end{exercise}
\begin{solution}
\begin{enumerate}
\item 若 \(Q\) 准素，设非零类 \(b+Q\) 被非零类 \(a+Q\) 零化，则 \(ab\in Q\)、\(a\notin Q\)。准素性给出某个 \(j\geq1\) 使 \(b^j\in Q\)，即 \(b+Q\) 幂零。反过来，若每个非零零因子都幂零，而 \(ab\in Q\)、\(a\notin Q\)，则当 \(b\in Q\) 时结论已经成立；当 \(b\notin Q\) 时，非零类 \(b+Q\) 被 \(a+Q\ne0\) 零化，故某次幂为零。这正是 \(Q\) 的准素性。

\item 若 \(n=p^r\) 为素数幂，且 \(ab\in n\mathbb Z\)、\(a\notin n\mathbb Z\)，则 \(p^r\mid ab\) 而 \(p^r\nmid a\)，整数的素数分解迫使 \(p\mid b\)。于是 \(b^r\in n\mathbb Z\)，所以 \(n\mathbb Z\) 准素。
