...
\]
使任意 \(a,b\in R\)、\(b\ne0\) 都可写成 \(a=bq+r\)，其中 \(q,r\in R\)，并且 \(r=0\) 或 \(\delta(r)<\delta(b)\)，则称 \(R\) 为\term[euclid-zhenghuan]{Euclid 整环}[Euclidean domain]，\(\delta\) 称为 Euclid 函数。
\end{definition}
...
设 \(R\) 为 Euclid 整环，\(a,b\in R\) 不全为零，且 Euclid 算法给出的商和余数满足 \(r_{j-1}=q_jr_j+r_{j+1}\)。从
\[
...
v_{j+1}=v_{j-1}-q_jv_j,
\]
以求得最后非零余数 \(d=ua+vb\)，这个过程称为\term[kuozhan-euclid-suanfa]{扩展 Euclid 算法}[extended Euclidean algorithm]。若已知一个求商余的具体程序，它就给出了以 \(a,b\) 为输入、以 \(d,u,v\) 为输出的有限计算。
