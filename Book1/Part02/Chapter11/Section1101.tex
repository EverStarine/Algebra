...
整环 \(R\) 中，\(a\mid b\) 当且仅当 \((b)\subseteq(a)\)。此外，以下三条件等价：\(a,b\) 相伴；\(a\mid b\) 且 \(b\mid a\)；\((a)=(b)\)。
\end{proposition}
\begin{proof}
...
设 \(p\) 是整环 \(R\) 中的非零非单位。
\begin{enumerate}
\item 若每当 \(p=ab\) 时，\(a,b\) 中至少一个为单位，则称 \(p\) 为\term[bukeyueyuan]{不可约元}[irreducible element]。
\item 若对任意 \(a,b\in R\)，\(p\mid ab\) 都蕴含 \(p\mid a\) 或 \(p\mid b\)，则称 \(p\) 为\term[suyuan]{素元}[prime element]。
\end{enumerate}
\end{definition}

不可约性限制 \(p\) 自身的分解，素性控制 \(p\) 对任意乘积的整除。两者并非同一个定义。单位被排除，是因为在分解中插入或删去单位不应改变因子的实质。

\begin{proposition}\label{b1:prop:prime-implies-irreducible}
整环中的素元都是不可约元。非零非单位 \(p\) 是素元，当且仅当主理想 \((p)\) 为素理想。
\end{proposition}
\begin{proof}
...
设 \(a,b\) 为整环 \(R\) 的两个元素。元素 \(d\in R\) 称为它们的\term[zuidagongyinzi]{最大公因子}[greatest common divisor]，若 \(d\mid a\)、\(d\mid b\)，并且每个同时整除 \(a,b\) 的元素 \(c\) 都整除 \(d\)。若 \(1\) 是它们的最大公因子，称 \(a,b\) \term[husu-yuansu]{互素}[relatively prime]。
\end{definition}
...
若 \(d\mid a\)、\(d\mid b\)，且存在 \(u,v\in R\) 使
...
形如\eqref{b1:eq:bezout-ring}、其中 \(d\) 是最大公因子的等式称为\term[bezout-guanxi]{Bézout 关系}[Bézout identity]。它比“\(d\) 是最大公因子”多提供了一个线性组合表达式。在 \(\mathbb Z\) 中，\(\gcd(30,18)=6\)，而 \(6=2\cdot18-30\) 给出相应关系。一般整环中，即使最大公因子存在，也未必能如此表示；下面的例子正说明这一点。
...
互素并不蕴含生成单位理想；一个理想仅用两个元素生成，也可能无法缩减成一个生成元。接下来要研究的主理想整环，正是从整体上排除这种现象的整环。
