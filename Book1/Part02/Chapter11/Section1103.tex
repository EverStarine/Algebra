...
主理想整环使\subsecref{b1:subsec:1101:03}所说的“在真主理想中极大”提升为“在全部真理想中极大”；此处用到的正是每个理想都为主理想。
...
\textbf{进一步阅读。}本小节用一个具体环说明“Euclid 整环 \(\Rightarrow\) 主理想整环”的逆命题不成立。初读时可直接进入\secref{b1:sec:1104}，学习一般唯一分解整环的判据及其与主理想整环的区别；需要比较这两种条件的强弱时，再回查本例。主理想整环的唯一分解性已在上一小节证明。设
\[
...
\ProseParagraphBreak
下面的引理用于证明理想的主性。设非零理想 \(I\) 中的非零元素 \(\beta\) 具有最小范数。若 \(\alpha\in I\) 不被 \(\beta\) 整除，不必寻找普通余数 \(\alpha-d\beta\)；只要找到 \(c,d\in R\)，使
\[
0<N(c\alpha-d\beta)<N(\beta),
\]
便会与 \(\beta\) 的选择矛盾，因为 \(c\alpha-d\beta\in I\)。令 \(z=\alpha/\beta\notin R\)，所需不等式就化为 \(0<\lvert cz-d\rvert^2<1\)。乘数 \(c\) 带来的自由正是以下格点估计的用处；下界严格大于零也不可省略。
...
\end{lemma}
\begin{proof}
先将 \(z\) 移入一个基本矩形：减去 \(R\) 中的一个元素，使所得 \(z_0=x+yi\) 满足
\[
...
这个证明不能被误读为“范数不是 Euclid 函数，所以环不是 Euclid 整环”。后一个结论需要排除所有可能的 Euclid 函数，证明的第二段正是为此而作。第一段使用的乘数 \(c\) 足以证明理想主性，却不符合普通带余除法的要求。

\ProseParagraphBreak
关于这个例子，可参见 Keith Conrad 的\href{https://kconrad.math.uconn.edu/blurbs/ringtheory/euclideanrk.pdf}{\emph{Remarks about Euclidean Domains}}：定理 5.18 提炼了非域 Euclid 整环中某个非单位模下每个剩余类可由零或单位代表的必要条件；定理 5.22 讨论本小节的二次环，其中主理想性只给出证明提纲，可与此处的格点引理作方法对照。
