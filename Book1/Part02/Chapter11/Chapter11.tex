...
本章把整数的整除与素因数分解推广到整环。不可约元与素元分别表达分解不能继续和整除乘积的性质；Euclid 算法通过逐步减小余数计算最大公因子，主理想整环要求每个理想由一个元素生成，唯一分解整环则要求每个非零非单位元的不可约分解在相伴和次序的意义下唯一。通过整数、多项式、Gauss 整数及两个二次数环，既建立这些要求之间的蕴含，也说明逆命题为什么失效。
\end{chapterabstract}
...
由前题在 \(p=3\) 时的结论，\(3\) 是 \(\mathbb Z[i]\) 中的素元，因此 \((3)\) 为极大理想，商为域。这里 \(N(3)=9\)，可见在 Gauss 整数中，范数为整数素数只是证明不可约的充分条件，不是必要条件。每个剩余类唯一以 \(u+vi\)、\(u,v\in\{0,1,2\}\) 表示，因为相差 \(3\) 的倍数等价于实部与虚部系数分别相差 \(3\) 的倍数，故商有九个元素。最后
...
设 \(F\) 为域。令 \(A\) 由形式有限和 \(\sum_q a_qT^q\) 组成，其中 \(a_q\in F\)，指数 \(q\) 取形如 \(m/2^n\) 的非负数，\(m,n\in\mathbb Z_{\geq0}\)。两个形式和相等指每个指数的系数相等；加法逐项进行，乘法由 \(T^qT^r=T^{q+r}\) 及分配律规定。证明 \(A\) 是整环，且 \(T\) 不能分解为有限个不可约元的乘积。再证明 \(A\) 的每个有限生成理想都是主理想，并说明为何 \(A\) 仍不是主理想整环。由此区分分解不存在与分解不唯一两种失败。
\end{exercise}
\begin{solution}
...
有限多个元素 \(f_1,\ldots,f_r\in A\) 同属于某个 \(F[U]\)，其中 \(U=T^{1/2^n}\)。由于 \(F[U]\) 是主理想整环，存在 \(d\in F[U]\) 使 \((f_1,\ldots,f_r)F[U]=dF[U]\)。于是每个 \(f_j\) 都是 \(d\) 在 \(A\) 中的倍数，而 \(d\) 是 \(f_1,\ldots,f_r\) 在 \(F[U]\) 中的线性组合，也属于它们在 \(A\) 中生成的理想。因此 \((f_1,\ldots,f_r)A=dA\)；零理想也由 \(0\) 生成。然而 \(T\) 没有不可约分解，\cref{b1:thm:pid-ufd}说明 \(A\) 不可能是主理想整环。具体地，\((T)\subsetneq(T^{1/2})\subsetneq(T^{1/4})\subsetneq\cdots\) 是一条不稳定的理想升链：右边的生成元有更小的正指数，不能属于左边的理想。与\cref{b1:prop:sqrt-minus-five-factorization}中 \(6\) 有两种不等价不可约分解相比，本例失败的是分解存在性。
\end{solution}

\begin{exercise}\label{b1:ex:11-ideal-splitting-three}
在 \(R=\mathbb Z[s]\)、\(s^2=-5\) 中，令 \(I=(2,1+s)\)、\(J=(3,1+s)\)、\(K=(3,1-s)\)。证明 \(J,K\) 是不同的非主极大理想，且 \(JK=(3)\)。再验证 \(IJ=(1+s)\)、\(IK=(1-s)\)，并用这些等式解释 \(6=2\cdot3=(1+s)(1-s)\) 所对应的理想分解。说明这与元素 \(3\) 不可约为何不矛盾。
\end{exercise}
\begin{solution}
...
理想 \(IJ\) 由 \(6,2(1+s),3(1+s),(1+s)^2\) 生成，均为 \(1+s\) 的倍数；又因 \(1+s=3(1+s)-2(1+s)\in IJ\)，有 \(IJ=(1+s)\)。由 \((1+s)-(1-s)=2s\in(2)\) 可得 \(I=(2,1-s)\)。因此 \(IK\) 由 \(6,2(1-s),3(1-s),(1-s)^2\) 生成，均为 \(1-s\) 的倍数；又有 \(1-s=3(1-s)-2(1-s)\in IK\)，故 \(IK=(1-s)\)。结合本节已证的 \(I^2=(2)\) 与这里的 \(JK=(3)\)，两种元素分解都给出同一个理想等式：
\[
(6)=(2)(3)=I^2JK=(IJ)(IK)=(1+s)(1-s).
\]
末项表示主理想 \((1+s)\) 与 \((1-s)\) 的乘积。这些等式只核对当前例子，并未证明一般理想分解的存在性或唯一性。
\end{solution}
