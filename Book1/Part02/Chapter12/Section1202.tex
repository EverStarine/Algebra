...
为了在更大的域中分解多项式，先建立分数运算所需的基础。这也说明本节所用的域从何而来；分式的泛性质和一般局部化将在\secref{b1:sec:1301}中系统讨论。
...
这个定理说明 \(\mathbb Z[x]\) 和 \(K[x,y]\) 都具有唯一分解性，却没有说明它们是主理想整环。唯一分解控制元素的因子；主理想条件要求一个理想中的全部元素由单个元素生成，是更强的要求。多元多项式环中理想的有限生成性，将由本章最后一节的 Hilbert 基定理保证。
