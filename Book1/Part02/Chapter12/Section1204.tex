...
\ProseParagraphBreak
特别地，域上的有限元多项式环中，每个理想都由有限个多项式生成。因此任意一族多项式方程所生成的理想，均可由有限个多项式生成；这些生成元各自又是原方程中有限项的组合，所以甚至可以从原方程族中选出有限多个，生成同一个理想。不过，这个存在性证明没有给出从任意输入有效寻找生成元的算法，也没有证明它们组成某种适于消去的标准形式。第三卷将在引用这里的 Hilbert 基定理后继续讨论这些问题，并把升链条件推广到模及其子模；本章的证明不依赖那些模论结论。
