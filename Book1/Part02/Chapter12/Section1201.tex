...
\ProseParagraphBreak
特征为素数 \(p\) 时，\(f'=0\) 当且仅当所有非零系数项的指数都被 \(p\) 整除：若 \(p\nmid i\)，则 \(i\cdot1_K\ne0\)，从 \(ia_i=0\) 可消去它而得 \(a_i=0\)。因此非常数多项式也可能导数为零。例如在 \(\mathbb F_p[x]\) 中，\(x^p-1=(x-1)^p\)，它的导数为零；等式来自中间二项式系数都被 \(p\) 整除。不能把特征零下“导数为零则为常数”的经验直接搬到这里。

\subsection{Lagrange 插值与代数闭域中的分解}
...
\]
公式\eqref{b1:eq:lagrange-interpolation}中的 \(\ell_i(x)=\prod_{\substack{0\le j\le n\\j\ne i}}\frac{x-a_j}{a_i-a_j}\)，在这个商环中对应第 \(i\) 个坐标幂等元。中国剩余定理说明可以任意指定各点的值；再将多项式除以首一多项式 \(\prod_{i=0}^n(x-a_i)\)，便知每个陪集都有唯一的次数至多为 \(n\) 的代表。
...
对通常的复数域，根的存在还要另作论证。以下结论称为\term[daishu-jiben-dingli]{代数基本定理}[fundamental theorem of algebra]；它不是复数的定义。这里采用实分析的中间值定理，并调用\appref{b1:app:E}中已经证明的实闭域刻画。
...
\end{theorem}
\begin{proof}
实数的通常序使非零平方为正，所以 \(-1\) 不可能是有限个平方的和，\(\mathbb R\) 是形式实域。若 \(a>0\)，多项式 \(X^2-a\) 在 \(0\) 处为负，在充分大的正实数处为正；中间值定理给出 \(a\) 的正平方根。若实系数多项式 \(f\) 的次数为奇数，它的最高次项在充分大的正、负实数处压过其余各项，故两处取值异号；中间值定理又给出一个实根。因此 \(\mathbb R\) 满足\cref{b1:thm:real-closed-characterizations}的第二项，该定理的第二项推出第三项的证明说明 \(\mathbb R(i)=\mathbb C\) 代数闭。这里的域论论证使用 Sylow 定理与有限 Galois 对应，具体证明在所引附录中。最后应用\cref{b1:prop:algebraically-closed-splitting}，即得按重数计的根数与分解。
\end{proof}
